...
export default async function handler(req, res) {
  ...
  try {
    ...
    // 🔒 Verrou anti double-paiement : marquer les participations comme
    // versées en banque (paid_to_bank=true) APRÈS un transfert réussi.
    // - Validation chercheur : studyId réel → on cible study_id + participant_id.
    // - Retrait participant   : studyId="withdrawal" → on solde toutes les
    //   participations dues (paid=true & paid_to_bank=false) de ce participant.
    // Fait en service role ici (pas côté front) pour être à l'abri de la RLS
    // et garantir que le solde retirable retombe à 0 de façon persistante.
    {
      const isWithdrawal = String(studyId || "") === "withdrawal";
      const filter = isWithdrawal
        ? `participant_id=eq.${participantId}&paid=eq.true&paid_to_bank=eq.false`
