...
export default async function handler(req, res) {
  ...
  try {
    ...
    // requête. Pour un retrait, la clé est dérivée de l'ENSEMBLE EXACT des
    // participations soldées (owedIds), PAS du montant : ainsi un re-essai du
    // même retrait est dédupliqué, mais deux retraits distincts du même montant
    // (ex. 5€ puis encore 5€ dans les 24h) ont des clés différentes et passent
    // tous les deux. Pour le chemin par étude (hérité), clé = étude + montant.
    const idempotencyKey = isWithdrawal
      ? `payout_wd_${participantId}_${crypto
          .createHash("sha256")
          .update(owedIds.slice().sort((a, b) => a - b).join(","))
          .digest("hex")
          .slice(0, 32)}`
      : `payout_${participantId}_${studyId}_${participantAmount}`;
    ...
    //   - Retrait : on solde EXACTEMENT les participations payées (par id).
    //   - Par étude (hérité) : study_id + participant_id.
    ...
    let markOk = false;
    {
      ...
      const url = `${SUPABASE_URL}/rest/v1/participations?${filter}`;
      const doMark = () =>
        fetch(url, {
          method: "PATCH",
          headers: { ...svcHeaders, "Content-Type": "application/json", Prefer: "return=minimal" },
        ...
        if (!markOk) {
          console.error(
            `[payout] CRITIQUE: paid_to_bank non marqué après transfert ${transfer.id} ` +
              `(participant ${participantId}, ids ${owedIds || studyId}) — HTTP ${r.status}`
          );
        }
      } catch (e) {
