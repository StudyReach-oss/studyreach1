...
function ResearcherDashboard({onLogout,showOnboarding,onOnboardingDone}){
  ...
  const exportStudyResultsPDF=(study)=>{
    ...
    if(win){
      ...
      const url=URL.createObjectURL(blob);
      const a=document.createElement("a");
      a.href=url;a.download=`resultats-${_safeName(study)}.html`;a.click();
      URL.revokeObjectURL(url);
    }
  };
  ...
  const exportStudyResultsCSV=(study)=>{
    ...
    const header=["Etude","Participant","Email","Statut","Match (%)","Score qualite (%)","Date","Rapport individuel","Transcript"];
    const lines=[header.map(cell).join(",")];
    rows.forEach(t=>{
      const iv=t.aiInterview||{};
      const transcript=(iv.transcript&&iv.transcript.length>0)?iv.transcript.map(m=>`${m.role==="user"?(t.participantName||"Participant"):"IA"}: ${m.content||""}`).join("\n"):"";
      lines.push([
        study.title||"",
    ...
    const url=URL.createObjectURL(blob);
    const a=document.createElement("a");
    a.href=url;a.download=`resultats-${_safeName(study)}.csv`;a.click();
    URL.revokeObjectURL(url);
  };
  ...
  const exportAllStudiesCSV=()=>{
    ...
    a.href=url;a.download=`studyreach-toutes-etudes-${today}.csv`;a.click();
    URL.revokeObjectURL(url);
  };
