      ...
      {showStudyModal&&(
        <Modal onClose={()=>setShowStudyModal(false)} title={`Nouvelle étude — Étape ${nsStep+1}/7`} wide>
          ...
          <div style={{display:"flex",justifyContent:"space-between",marginTop:24,paddingTop:16,borderTop:`1px solid ${C.border}`}}>
            ...
            {nsStep<6?<Btn onClick={()=>{setNsErr("");setNsStep(nsStep+1);}} disabled={(nsStep===0&&(!ns.title||!ns.theme))||(nsStep===1&&!ns.dur)||(nsStep===2&&!ns.studyType)||(nsStep===3&&!ns.mode)||(nsStep===5&&(ns.prescreening||[]).some(q=>!q.question||(q.acceptedAnswers||[]).length===0))}>Continuer →</Btn>:<Btn onClick={publishStudy}>🚀 Publier l'étude</Btn>}
          </div>
        </Modal>
      )}
    ...
    const loadStudies=async()=>{
      ...
      try{
        ...
        if(Array.isArray(data)&&data.length>0){
          ...
          if(token&&userId){
            try{
              const myPartRes=await fetch(`${SUPA_URL}/rest/v1/participations?participant_id=eq.${userId}&select=study_id`,{
                headers:{"apikey":SUPA_KEY,"Authorization":`Bearer ${token}`}
              });
              const myPart=await myPartRes.json();
              if(Array.isArray(myPart))joinedStudyIds=new Set(myPart.map(p=>p.study_id));
