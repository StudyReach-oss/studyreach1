      ...
      if(res.ok&&data.success){
        ...
        if(token&&participation.participationId){
          fetch(`${SUPA_URL}/rest/v1/participations?id=eq.${participation.participationId}`,{
            method:"PATCH",
            headers:{"apikey":SUPA_KEY,"Authorization":`Bearer ${token}`,"Content-Type":"application/json"},
            body:JSON.stringify({paid:true,status:"completed",paid_at:new Date().toISOString()})
          }).catch(e=>console.error(e));
        }
        ...
        alert("Erreur versement Stripe.\n\nDétail : "+detail+"\n\nSi le participant n'a pas encore configuré son compte de paiement (onboarding Stripe), il sera payé à son prochain retrait.");
      }
    }catch(e){
  ...
  useEffect(()=>{
    ...
    const loadMyParticipations=async()=>{
      ...
      try{
        ...
        setEarnings(paidParts.reduce((a,p)=>{const s=studyMap[p.study_id];return a+(s?participantNet(s.cost_per_participant,s.ai):0);},0));
