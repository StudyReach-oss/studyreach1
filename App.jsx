...
function AiInterviewChat({study,profile,matchScore,onComplete,onClose,initialMessages,participationId}){
  ...
  // Progression basée sur le TEMPS écoulé vs durée prévue (cohérent avec la
  // règle de fin côté serveur). Plafonnée à 99% tant que l'entretien n'est pas
  // réellement conclu, pour ne pas afficher 100% pendant la dernière question.
  const progressPct=finished?100:Math.min(99,Math.round((elapsed/Math.max(1,durMinutesNum*60))*100));
          ...
          {tab==="wallet"&&(
            <div style={{maxWidth:800}}>
              ...
              <div style={{display:"flex",gap:14,flexWrap:"wrap",marginBottom:12,fontSize:11,color:C.muted}}>
                <span style={{display:"flex",alignItems:"center",gap:5}}>⏳ <b style={{color:C.text}}>En attente</b> — à valider ou refuser</span>
                <span style={{display:"flex",alignItems:"center",gap:5}}>🎙️ <b style={{color:C.text}}>En cours</b> — entretien en cours</span>
                <span style={{display:"flex",alignItems:"center",gap:5}}>💸 <b style={{color:C.text}}>Payé</b> — participant rémunéré</span>
                <span style={{display:"flex",alignItems:"center",gap:5}}>🚫 <b style={{color:C.text}}>Refusé</b> — non rémunéré</span>
                <span style={{display:"flex",alignItems:"center",gap:5}}><span style={{color:C.white,fontSize:14,fontWeight:900}}>✕</span> <b style={{color:C.text}}>Expiré</b> — a quitté la session sans terminer l'entretien</span>
              </div>
              ...
              <Card style={{overflow:"hidden"}}>
                {(()=>{
                  ...
                  const headerStyle={display:"grid",gridTemplateColumns:txCols,padding:isDesktop?"10px 20px":"10px 14px",borderBottom:`1px solid ${C.border}`,fontSize:11,fontWeight:700,color:C.muted,textTransform:"uppercase",letterSpacing:.6};

                  return(<>
                    <div style={headerStyle}>
                      <span>Description</span>{isDesktop&&<span>Date</span>}<span></span><span style={{textAlign:"right"}}>Montant</span>
                    </div>
                    {allRows.slice(txRowsPage*TX_ROWS_PER_PAGE,txRowsPage*TX_ROWS_PER_PAGE+TX_ROWS_PER_PAGE).map((row,ri)=>{
                      if(row.type==="recharge"){
                        const t=row.data;
                        return(
                          <div key={"r"+ri} style={{display:"grid",gridTemplateColumns:txCols,padding:rowPad,borderBottom:`1px solid ${C.border}`,alignItems:"center",fontSize:13}}>
                      ...
                      const groupStatus=pendingCount>0?`${pendingCount} en attente`:inprogressCount>0?"En cours":paidCount>0?"Payé ✓":rejectedCount>0?"Refusé":"Expiré";
                      return(
                        <div key={"g"+ri}>
                          <div onClick={()=>setExpandedTx(open?null:row.key)} style={{display:"grid",gridTemplateColumns:txCols,padding:rowPad,borderBottom:`1px solid ${C.border}`,alignItems:"center",fontSize:13,cursor:"pointer",transition:"background .15s",background:open?C.accentGlow+"88":"transparent"}}>
                            <div style={{display:"flex",alignItems:"center",gap:8,minWidth:0}}>
                              <span style={{fontSize:18,flexShrink:0}}>{pendingCount>0?"⏳":inprogressCount>0?"🎙️":paidCount>0?"💸":rejectedCount>0?"🚫":<span style={{color:C.white,fontWeight:900}}>✕</span>}</span>
                            ...
                            <Tag color={groupColor}>{groupStatus}</Tag>
                            ...
                            <div style={{background:C.bg,borderBottom:`1px solid ${C.border}`}}>
                              {g.items.map((t,ii)=>(
                                <div key={ii} style={{display:"grid",gridTemplateColumns:txCols,padding:isDesktop?"10px 20px 10px 48px":"10px 14px 10px 28px",borderBottom:ii<g.items.length-1?`1px solid ${C.border}`:"none",alignItems:"flex-start",fontSize:12,gap:8}}>
                                  ...
                                  {(()=>{const st=payoutState(t);
                                    if(st==="paid")return <Tag color={C.green} style={{fontSize:10}}>Payé ✓</Tag>;
                                    if(st==="rejected")return <Tag color={C.red} style={{fontSize:10}}>Refusé</Tag>;
                                    if(st==="abandoned")return <Tag color={C.muted} style={{fontSize:10}}>Expiré</Tag>;
                                    if(st==="inprogress")return <Tag color={C.accent} style={{fontSize:10}}>En cours</Tag>;
...
function ParticipantDashboard({onLogout}){
  ...
  return(
    <div style={{minHeight:"100vh",display:"flex",flexDirection:"column",fontFamily:FONT}}>
      ...
      {showNewMsgModal&&(
        <Modal onClose={()=>setShowNewMsgModal(false)} title="Contacter un chercheur">
          {(()=>{
            const seen=new Set();
            const eligible=studies.filter(s=>{
