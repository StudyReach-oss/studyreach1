...
function ResearcherDashboard({onLogout,showOnboarding,onOnboardingDone}){
  ...
  return(
    <div style={{minHeight:"100vh",display:"flex",flexDirection:"column",fontFamily:FONT}}>
                              ...
                              <div style={{flex:1,minWidth:140}}>
                                ...
                                <div style={{display:"flex",gap:6,flexWrap:"wrap"}}>
                                  ...
                                  {t.aiInterview?.video_urls?.length>0&&(
                                    <button onClick={()=>setShowAiReport({...t.aiInterview,participantId:t.participantId,participantName:t.participantName,matchScore:t.matchScore,defaultTab:"video"})} style={{background:C.greenGlow,border:`1px solid ${C.green}44`,borderRadius:7,color:C.green,fontSize:11,fontWeight:600,cursor:"pointer",padding:"4px 10px"}}>{(t.aiInterview?.ai_response_format?.audio&&!t.aiInterview?.ai_response_format?.video)?"🎙️":"🎥"} Enregistrements</button>
                                  )}
      ...
      {obOpen&&<OnboardingModal role="researcher" onClose={closeOnboarding}/>}
...
function ParticipantDashboard({onLogout,showOnboarding,onOnboardingDone}){
  ...
  return(
    <div style={{minHeight:"100vh",display:"flex",flexDirection:"column",fontFamily:FONT}}>
      ...
      {obOpen&&<OnboardingModal role="participant" onClose={closeOnboarding}/>}
...
function OnboardingModal({role,onClose}){
  ...
  const steps=isPart?[
    ...
    {icon:"💸",title:"Soyez payé",body:"Vous recevez 90% du montant via PayPal sous 24-48h après validation par le chercheur. Retrait possible dès 5€ de gains."},
  ]:[
    {icon:"👋",title:"Bienvenue sur StudyReach",body:"Recrutez des participants qualifiés et obtenez des insights en quelques jours. Voici comment ça marche en 4 étapes."},
    ...
    {icon:"💳",title:"Rechargez votre portefeuille",body:"Vous payez un tarif fixe par participant. Rechargez votre solde, et chaque participant validé est débité automatiquement."},
      ...
      <div style={{textAlign:"center",padding:"4px 4px 0"}}>
        ...
        <div style={{display:"flex",gap:10}}>
          ...
          {!last?(
            ...
            <Btn green={isPart} onClick={onClose} style={{flex:2}}>{isPart?"Découvrir les études →":"Créer ma première étude →"}</Btn>
          )}
        </div>
