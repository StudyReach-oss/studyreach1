...
function AiInterviewChat({study,profile,matchScore,onComplete,onClose,initialMessages,participationId}){
  ...
  const send=()=>{
    ...
    // FIX: Stop l'enregistrement audio proprement s'il est en cours.
    if(recording){
      try{audioRecorderInstRef.current?.stop();}catch(e){}
      setRecording(false);
      stopMicMeter();
    ...
    // FIX: texte de repli calculé directement (pas via setInput, dont la mise
    // à jour serait asynchrone) pour que "Envoyer" ne soit jamais ignoré si
    // l'utilisateur clique pendant qu'un enregistrement audio est en cours.
    const text=input.trim()||(wasRecording?"[Réponse audio enregistrée]":"");
    if(!text)return;
      ...
      {finished?(
        ...
        <div>
          ...
          <div style={{display:"flex",gap:8}}>
            ...
            <Btn onClick={send} disabled={loading||transcribing||(!input.trim()&&!recording)} style={{alignSelf:"flex-end"}}>{transcribing?"Transcription…":"Envoyer"}</Btn>
          </div>
  ...
  return(
    <div style={{marginBottom:20}}>
      <div style={{fontSize:12,fontWeight:700,color:"#a855f7",textTransform:"uppercase",letterSpacing:.8,marginBottom:10}}>
        🎥 Enregistrements ({signedUrls.length})
      </div>
      {signedUrls.length>1&&(
...
function AiReportModal({data,onClose,onViewProfile,onQuality,onTranscript}){
  const tabs=[
    ...
    {id:"video",label:"🎥 Enregistrements",show:!!(data.video_urls?.length>0)},
                              ...
                              <div style={{flex:1,minWidth:140}}>
                                ...
                                <div style={{display:"flex",gap:6,flexWrap:"wrap"}}>
                                  ...
                                  {t.aiInterview?.video_urls?.length>0&&(
                                    <button onClick={()=>setShowAiReport({...t.aiInterview,participantId:t.participantId,participantName:t.participantName,matchScore:t.matchScore,defaultTab:"video"})} style={{background:C.greenGlow,border:`1px solid ${C.green}44`,borderRadius:7,color:C.green,fontSize:11,fontWeight:600,cursor:"pointer",padding:"4px 10px"}}>🎥 Enregistrements</button>
                                  )}
