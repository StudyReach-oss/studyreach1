        ...
        <Modal onClose={()=>setShowStudyDetail(null)} title={showStudyDetail.title} wide>
          ...
          {["video","video_group","inperson","inperson_group"].includes(showStudyDetail.studyType)&&(
            <div style={{marginTop:14}}>
              ...
              <StudyAgenda studyId={showStudyDetail.id}/>
            </div>
          )}
      ...
      try{
        ...
        const participationStudies=parts.filter(p=>p.status!=="rejected").map(p=>{
          ...
          return{
            ...
            incompleteExpiresAt:p.incomplete_expires_at||null,
          };
        }).filter(Boolean);
        ...
        <main className="p-main" style={{flex:1,padding:"20px 16px",overflowY:"auto",background:C.bg,width:"100%",minWidth:0,boxSizing:"border-box",overflowX:"hidden",marginLeft:isDesktop?210:0,transition:"margin-left .25s ease"}}>
          ...
          {tab==="overview"&&(
            <div>
              ...
              {resumeParticipation&&(
                resumeParticipation.studyClosed||resumeParticipation.quotaExpired?(
                  <Card style={{padding:"16px 18px",marginBottom:20,border:`1px solid ${C.muted}44`,background:C.surfaceHigh,display:"flex",justifyContent:"space-between",alignItems:"center",gap:12,flexWrap:"wrap",opacity:.7}}>
                    <div style={{flex:1,minWidth:0}}>
                      <div style={{fontWeight:800,fontSize:14,marginBottom:2,color:C.muted}}>Étude clôturée</div>
                      <div style={{fontSize:13,color:C.muted}}>"{resumeParticipation.study.title}" — le quota de participants a été atteint.</div>
                    </div>
                    <button onClick={()=>setResumeParticipation(null)} style={{background:"none",border:"none",color:C.muted,fontSize:18,cursor:"pointer",padding:"4px 8px",lineHeight:1}}>✕</button>
                  </Card>
                ):resumeParticipation.timedOut?(
                  <Card style={{padding:"16px 18px",marginBottom:20,border:`1px solid ${C.muted}44`,background:C.surfaceHigh,display:"flex",justifyContent:"space-between",alignItems:"center",gap:12,flexWrap:"wrap",opacity:.7}}>
                    <div style={{flex:1,minWidth:0}}>
                      <div style={{fontWeight:800,fontSize:14,marginBottom:2,color:C.muted}}>Temps écoulé</div>
                      <div style={{fontSize:13,color:C.muted}}>"{resumeParticipation.study.title}" — le délai de 24h pour terminer cette étude est dépassé.</div>
                    </div>
                    <button onClick={()=>setResumeParticipation(null)} style={{background:"none",border:"none",color:C.muted,fontSize:18,cursor:"pointer",padding:"4px 8px",lineHeight:1}}>✕</button>
                  </Card>
                ):(
...
function StudyAgenda({ studyId }){
  ...
  React.useEffect(()=>{
    if(!studyId) return;
    const token = Storage.get("sb_token")||"";
    // Récupère tous les slots de l'étude, avec le profil du participant si réservé
    fetch(`${SUPA_URL}/rest/v1/slots?study_id=eq.${studyId}&select=id,datetime,taken,participations(participant_id,profiles(first_name,last_name))&order=datetime.asc`,{
    ...
    .then(data=>{
      ...
      data.forEach(slot=>{
        ...
        if(slot.taken && slot.participations?.profiles){
          ...
          map.get(dt).taken.push(`${p.first_name||""} ${(p.last_name||"").charAt(0)}.`.trim());
        } else {
          map.get(dt).free += 1;
        }
      });
  ...
  return(
    <div style={{display:"flex",flexDirection:"column",gap:6}}>
      {rows.map(row=>{
        ...
        const lines = [
          ...participants.map(name=>({ name, free:false })),
          ...(free>0 ? [{name:`${free} place${free>1?"s":""} libre${free>1?"s":""}`, free:true}] : [])
        ];

        return lines.map((line,i)=>(
          <div key={row.datetime+"-"+i} style={{display:"flex",alignItems:"center",justifyContent:"space-between",background:C.surfaceHigh,borderRadius:8,padding:"9px 12px",border:`1px solid ${C.border}`}}>
            <div style={{fontSize:13,color:C.muted,textTransform:"capitalize",minWidth:160}}>{i===0 ? label : ""}</div>
            <div style={{fontSize:13,fontWeight:line.free?400:600,color:line.free?C.muted:C.text,textAlign:"right"}}>
              {line.free ? <span style={{color:C.dimmed}}>— {line.name}</span> : line.name}
            </div>
          </div>
        ));
      })}
