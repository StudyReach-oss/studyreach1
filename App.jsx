  ...
  const submit=async()=>{
    ...
    try{
      if(isLogin){
        ...
        if(!data.user?.id)throw new Error("Erreur lors de la création du compte. Réessayez.");
        ...
        if(!token){
          ...
          if(!loginData.access_token)throw new Error("Compte créé ! Connectez-vous maintenant avec vos identifiants.");
