  ...
  const submit=async()=>{
    ...
    try{
      if(isLogin){
        ...
        if(data.error||data.error_description||(!data.user&&!data.id)){
          const msg=data.error_description||data.error?.message||data.msg||JSON.stringify(data);
...
// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
function AiInterviewChat({study,onComplete,onClose}){
  ...
  const audioEnabled=!!study.ai_response_format?.audio;
  ...
  const toggleRecording=()=>{
    ...
    recognition.onerror=(e)=>{console.error("Speech recognition error:",e);setRecording(false);};
  ...
  const finishInterview=async()=>{
    setFinishing(true);
    try{
      const transcript=messages.map(m=>`${m.role==="user"?"Participant":"Interviewer"}: ${m.content}`).join("\n");
      const systemPrompt=`Tu es un expert en analyse d'entretiens qualitatifs pour la plateforme StudyReach.\nGénère un rapport synthétique structuré en français, basé sur la transcription d'un entretien.\n\nÉtude : ${study.title||""} — ${study.theme||""}\nDescription : ${study.description||""}\n\nRédige un rapport avec ces sections :\n## Résumé exécutif\n## Points clés identifiés\n## Verbatims significatifs (citations directes du participant)\n## Insights et recommandations\n## Conclusion\n\nSois factuel, analytique et synthétique. Format markdown propre.`;
      const res=await fetch(`${SUPA_URL}/functions/v1/ai-interview`,{
        method:"POST",
        headers:{"Content-Type":"application/json","Authorization":`Bearer ${Storage.get("sb_token")||""}`,"apikey":SUPA_KEY},
        body:JSON.stringify({action:"report",study:{title:study.title,theme:study.theme,description:study.description,ai_focus:study.ai_focus},messages})
      });
      const data=await res.json();
      const report=data.report||"Rapport indisponible.";
      onComplete(study,messages,report,videoUrls);
    }catch(e){
      console.error("Report generation error:",e);
      onComplete(study,messages,"Erreur lors de la génération du rapport.",videoUrls);
    }
    setFinishing(false);
  };
  ...
  return(
    <Modal onClose={onClose} title={`🤖 StudyReach AI — ${study.title}`} wide>
      <div style={{display:"flex",justifyContent:"flex-end",marginBottom:8}}>
        <button onClick={()=>{if(ttsOn)try{window.speechSynthesis?.cancel();}catch(e){}setTtsOn(!ttsOn);}} style={{display:"flex",alignItems:"center",gap:6,background:ttsOn?"#a855f722":C.surfaceHigh,border:`1px solid ${ttsOn?"#a855f744":C.border}`,borderRadius:20,padding:"5px 12px",cursor:"pointer",fontSize:11,fontWeight:700,color:ttsOn?"#a855f7":C.muted}}>
          {ttsOn?"🔊 Lecture audio activée":"🔇 Lecture audio désactivée"}
        </button>
      </div>
      <div ref={scrollRef} style={{maxHeight:420,overflowY:"auto",display:"flex",flexDirection:"column",gap:12,marginBottom:14,paddingRight:4}}>
        ...
        {loading&&messages.length>0&&(
          <div style={{display:"flex",justifyContent:"flex-start"}}>
            <div style={{background:C.surfaceHigh,border:`1px solid ${C.border}`,borderRadius:14,padding:"10px 14px",fontSize:13,color:C.muted}}>StudyReach AI réfléchit…</div>
          </div>
        )}
      </div>
      {finished?(
        <div>
          <div style={{background:"#1ec98a11",border:"1px solid #1ec98a44",borderRadius:10,padding:"12px 14px",fontSize:13,color:C.text,marginBottom:12}}>
            ✅ L'entretien est terminé. Cliquez ci-dessous pour le soumettre — votre rémunération sera créditée après validation du chercheur.
          </div>
          <Btn green full disabled={finishing} onClick={finishInterview}>{finishing?"Génération du rapport…":"Soumettre l'entretien"}</Btn>
        </div>
      ):(
        <div style={{display:"flex",gap:8}}>
          <textarea value={input} onChange={e=>setInput(e.target.value)} onKeyDown={e=>{if(e.key==="Enter"&&!e.shiftKey){e.preventDefault();send();}}} placeholder={recording?"🎙️ Enregistrement en cours…":"Votre réponse…"} rows={2} disabled={loading} style={{flex:1,background:C.bg,border:`1px solid ${recording?"#f0556a":C.border}`,borderRadius:10,padding:"10px 12px",color:C.text,fontSize:13.5,outline:"none",resize:"none",boxSizing:"border-box"}}/>
          {audioEnabled&&(
        ...
        if(user?.id){
          ...
          if(Array.isArray(studiesData)&&studiesData.length>0){
            ...
            if(studyIds.length>0){
              const partRes=await fetch(`https://bwaoxwfkqqpqvtpynwzh.supabase.co/rest/v1/participations?study_id=in.(${studyIds.join(",")})&select=*,profiles(first_name,last_name,paypal_email)`,{
                headers:{"apikey":"eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImJ3YW94d2ZrcXFwcXZ0cHlud3poIiwicm9sZSI6ImFub24iLCJpYXQiOjE3ODA2OTEzMDEsImV4cCI6MjA5NjI2NzMwMX0.utS5lj2nI-Bs0efelpaj9LHT3B_MSib5Ro8ESIz1-q8","Authorization":`Bearer ${token}`}
              });
                              ...
                              {g.items.map((t,ii)=>(
                                <div key={ii} style={{display:"grid",gridTemplateColumns:"2fr 1fr 1fr 1fr",padding:"10px 20px 10px 48px",borderBottom:ii<g.items.length-1?`1px solid ${C.border}`:"none",alignItems:"flex-start",fontSize:12,gap:8}}>
                                  <div>
                                    ...
                                    {t.prescreeningQuestions?.length>0&&t.prescreeningAnswers?.length>0&&(
                                      <div style={{marginTop:6,paddingLeft:12,borderLeft:`2px solid ${C.border}`}}>
                                        {t.prescreeningQuestions.map((q,qi)=>t.prescreeningAnswers[qi]?(
                                          <div key={qi} style={{fontSize:11,color:C.muted,marginBottom:2}}>
                                            <span style={{color:C.text}}>{q.question}</span> → <strong style={{color:C.accentLight}}>{t.prescreeningAnswers[qi]}</strong>
                                          </div>
                                        ):null)}
                                      </div>
                                    )}
      ...
      {showStudyModal&&(
        <Modal onClose={()=>setShowStudyModal(false)} title={`Nouvelle étude — Étape ${nsStep+1}/7`} wide>
            ...
            <div>
              ...
              {ns.mode==="ai"&&(
                <>
                  ...
                  <div style={{marginBottom:4,fontSize:13,fontWeight:700}}>Points spécifiques à aborder <span style={{color:C.muted,fontWeight:400}}>(optionnel)</span></div>
                  ...
                  <div style={{fontSize:11,color:C.muted,marginTop:4}}>Laissez vide pour laisser StudyReach AI gérer entièrement l'entretien selon le thème et les critères définis.</div>
                  ...
                  {[
                    ...
                    {key:"video",icon:"🎥",title:"Réponse vidéo",desc:"Le participant peut répondre face caméra."},
                    ...
                    const checked=!!(ns.ai_response_format||{})[opt.key];
                    return(
                      <label key={opt.key} style={{display:"flex",alignItems:"flex-start",gap:10,cursor:"pointer",background:checked?"#a855f711":C.bg,border:`1px solid ${checked?"#a855f744":C.border}`,borderRadius:10,padding:"10px 14px",marginBottom:8}}>
                        <input type="checkbox" checked={checked} onChange={e=>setNs({...ns,ai_response_format:{...(ns.ai_response_format||{}),[opt.key]:e.target.checked}})} style={{marginTop:2,width:15,height:15,cursor:"pointer",accentColor:"#a855f7"}}/>
                        <div>
            ...
            <div>
              ...
              <div style={{marginBottom:18}}>
                ...
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"10px 0",borderTop:`1px solid ${C.border}`}}>
                  ...
                  <div onClick={()=>setNs({...ns,target_criteria:{...ns.target_criteria,has_camera:!ns.target_criteria.has_camera}})} style={{width:40,height:22,borderRadius:11,background:ns.target_criteria.has_camera?"#1ec98a":"#1c2035",cursor:"pointer",position:"relative",transition:"background .2s",flexShrink:0}}>
                ...
                <div style={{marginBottom:10}}>
                  ...
                  <div style={{display:"flex",gap:8,flexWrap:"wrap"}}>
                    {["Français","Anglais","Espagnol","Allemand","Italien","Arabe","Portugais"].map(v=>(
                      <div key={v} onClick={()=>toggleTC("languages",v)} style={{padding:"5px 12px",borderRadius:20,cursor:"pointer",fontSize:12,fontWeight:600,background:(ns.target_criteria.languages||[]).includes(v)?"#1ec98a22":"#07080e",border:`1.5px solid ${(ns.target_criteria.languages||[]).includes(v)?"#1ec98a":"#1c2035"}`,color:(ns.target_criteria.languages||[]).includes(v)?"#1ec98a":"#606880"}}>{v}</div>
                    ))}
                  </div>
                </div>
                <div style={{display:"flex",gap:10}}>
                  {[{field:"mobile",label:"Mobilité requise"},{field:"long_term",label:"Disponible long terme"}].map(({field,label})=>(
                    <div key={field} style={{flex:1,display:"flex",justifyContent:"space-between",alignItems:"center",padding:"10px 12px",background:C.surface,borderRadius:10,border:`1px solid ${C.border}`}}>
                      ...
                      <div onClick={()=>setNs({...ns,target_criteria:{...ns.target_criteria,[field]:!ns.target_criteria[field]}})} style={{width:40,height:22,borderRadius:11,background:ns.target_criteria[field]?"#1ec98a":"#1c2035",cursor:"pointer",position:"relative",transition:"background .2s",flexShrink:0}}>
                  ...
                  {[{field:"smoker",label:"Fumeur",opts:["Non","Oui","Occasionnel"]},{field:"alcohol",label:"Alcool",opts:["Jamais","Occasionnel","Régulier"]}].map(({field,label,opts})=>(
                    <div key={field}>
                      ...
                      <div style={{display:"flex",gap:6,flexWrap:"wrap"}}>
                        {opts.map(v=>(
                          <div key={v} onClick={()=>setNs({...ns,target_criteria:{...ns.target_criteria,[field]:ns.target_criteria[field]===v?"":v}})} style={{padding:"4px 10px",borderRadius:20,cursor:"pointer",fontSize:11,fontWeight:600,background:ns.target_criteria[field]===v?"#f59e0b22":"#07080e",border:`1.5px solid ${ns.target_criteria[field]===v?"#f59e0b":"#1c2035"}`,color:ns.target_criteria[field]===v?"#f59e0b":"#606880"}}>{v}</div>
                        ))}
                      </div>
                    </div>
                  ))}
                ...
                <div style={{marginBottom:10}}>
                  ...
                  <div style={{display:"flex",gap:8,flexWrap:"wrap"}}>
                    {["< 1 500€","1 500–3 000€","3 000–5 000€","+ 5 000€"].map(v=>(
                      <div key={v} onClick={()=>toggleTC("income",v)} style={{padding:"5px 12px",borderRadius:20,cursor:"pointer",fontSize:12,fontWeight:600,background:(ns.target_criteria.income||[]).includes(v)?"#1ec98a22":"#07080e",border:`1.5px solid ${(ns.target_criteria.income||[]).includes(v)?"#1ec98a":"#1c2035"}`,color:(ns.target_criteria.income||[]).includes(v)?"#1ec98a":"#606880"}}>{v}</div>
                    ))}
                  </div>
                </div>
                <div style={{display:"flex",gap:10}}>
                ...
                <div style={{marginBottom:10}}>
                  ...
                  <div style={{display:"flex",gap:8,flexWrap:"wrap"}}>
                    {["Célibataire","En couple","Marié(e)","Parent"].map(v=>(
                      <div key={v} onClick={()=>toggleTC("family_status",v)} style={{padding:"5px 12px",borderRadius:20,cursor:"pointer",fontSize:12,fontWeight:600,background:(ns.target_criteria.family_status||[]).includes(v)?"#ec489922":"#07080e",border:`1.5px solid ${(ns.target_criteria.family_status||[]).includes(v)?"#ec4899":"#1c2035"}`,color:(ns.target_criteria.family_status||[]).includes(v)?"#ec4899":"#606880"}}>{v}</div>
                    ))}
                  </div>
                </div>
                <div style={{marginBottom:10}}>
                  ...
                  <div style={{display:"flex",gap:8,flexWrap:"wrap"}}>
                    {["Propriétaire","Locataire","Colocation"].map(v=>(
                      <div key={v} onClick={()=>toggleTC("housing_status",v)} style={{padding:"5px 12px",borderRadius:20,cursor:"pointer",fontSize:12,fontWeight:600,background:(ns.target_criteria.housing_status||[]).includes(v)?"#ec489922":"#07080e",border:`1.5px solid ${(ns.target_criteria.housing_status||[]).includes(v)?"#ec4899":"#1c2035"}`,color:(ns.target_criteria.housing_status||[]).includes(v)?"#ec4899":"#606880"}}>{v}</div>
                    ))}
                  </div>
                </div>
              </div>
            ...
            <div>
              ...
              {(ns.prescreening||[]).map((q,i)=>(
                <div key={i} style={{background:C.surface,border:`1px solid ${C.border}`,borderRadius:12,padding:"14px 16px",marginBottom:10}}>
                  ...
                  <input value={q.question} onChange={e=>{const p=[...ns.prescreening];p[i]={...p[i],question:e.target.value};setNs({...ns,prescreening:p});}} placeholder="Ex: Utilisez-vous des applications mobiles au moins une fois par jour ?" style={{width:"100%",background:C.bg,border:`1px solid ${C.border}`,borderRadius:8,padding:"8px 12px",color:C.text,fontSize:13,outline:"none",marginBottom:8,boxSizing:"border-box"}}/>
                  ...
                  {q.type==="Choix multiple"&&(
                    <div style={{marginTop:8}}>
                      ...
                      <input value={q.options||""} onChange={e=>{const p=[...ns.prescreening];p[i]={...p[i],options:e.target.value};setNs({...ns,prescreening:p});}} placeholder="Ex: Jamais, Parfois, Souvent, Toujours" style={{width:"100%",background:C.bg,border:`1px solid ${C.border}`,borderRadius:8,padding:"8px 12px",color:C.text,fontSize:12,outline:"none",boxSizing:"border-box"}}/>
          ...
          <div style={{display:"flex",justifyContent:"space-between",marginTop:24,paddingTop:16,borderTop:`1px solid ${C.border}`}}>
            ...
            {nsStep<6?<Btn onClick={()=>{setNsErr("");setNsStep(nsStep+1);}} disabled={(nsStep===0&&(!ns.title||!ns.theme))||(nsStep===1&&!ns.dur)||(nsStep===2&&!ns.studyType)||(nsStep===3&&!ns.mode)}>Continuer →</Btn>:<Btn onClick={publishStudy}>🚀 Publier l'étude</Btn>}
          </div>
        </Modal>
      )}
    ...
    <Card style={{padding:"18px 22px",display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:full?0:8,cursor:"pointer"}} onClick={onClick}>
      <div style={{flex:1}}>
        <div style={{display:"flex",gap:8,alignItems:"center",marginBottom:6}}>
          ...
          {(s.mode==="IA"||s.ai||s.linkAi)&&<Tag color="#a855f7">🤖 IA</Tag>}
        </div>
...
function ParticipantDashboard({onLogout}){
      ...
      try{
        ...
        if(Array.isArray(data)&&data.length>0){
          const mapped=data.map(s=>({
            ...
            deadline:"30 juin 2026",pay:s.budget?Math.round(s.budget*0.9):27,
      ...
      try{
        ...
        if(user?.id){
          ...
          if(profiles?.[0]){
            const p=profiles[0];
            setProfile({
  paypal:p.paypal_email||"",bio:p.bio||"",
  ...
  const completeAiInterview=async(study,transcript,report,videoUrls)=>{
    const token=Storage.get("sb_token");
    // Vérifier le matching profil vs target_criteria
    const matchScore=computeMatchScore(study,profile);
    const isMatch=matchScore>=60;
    const newStatus=isMatch?"completed":"rejected";
    try{
      // Sauvegarder l'interview
      await fetch(`${SUPA_URL}/rest/v1/interviews`,{
        method:"POST",
        headers:{"apikey":SUPA_KEY,"Authorization":`Bearer ${token}`,"Content-Type":"application/json","Prefer":"return=minimal"},
        body:JSON.stringify({
          study_id:study.id,
      ...
      if(isMatch){
        // Profil OK → paiement automatique
        setPending(p=>p+study.pay);
        setStudies(prev=>prev.map(x=>x.id===study.id?{...x,status:"completed"}:x));
        setNotifs(prev=>[{id:Date.now(),read:false,type:"complete",text:`✅ Entretien "${study.title}" validé automatiquement — ${study.pay}€ en cours de virement.`,time:"maintenant"},...prev]);
        // Déclencher le paiement automatique
        if(token&&profile.paypal){
          fetch(`/api/payout`,{
            method:"POST",
            headers:{"Content-Type":"application/json"},
            body:JSON.stringify({
              participantEmail:profile.paypal,
              studyAmount:study.pay,
              studyId:study.id,
              participantId:userId,
            })
          }).then(r=>r.json()).then(data=>{
            if(data.success){
              // Déduire du wallet chercheur et mettre à jour
              fetch(`${SUPA_URL}/rest/v1/participations?study_id=eq.${study.id}&participant_id=eq.${userId}`,{
                method:"PATCH",
                headers:{"apikey":SUPA_KEY,"Authorization":`Bearer ${token}`,"Content-Type":"application/json"},
                body:JSON.stringify({paid:true,status:"completed"})
              }).catch(e=>console.error(e));
              // Vérifier quota → auto-fermer l'étude si atteint
              fetch(`${SUPA_URL}/rest/v1/participations?study_id=eq.${study.id}&status=eq.completed&select=id`,{
                headers:{"apikey":SUPA_KEY,"Authorization":`Bearer ${token}`}
              }).then(r=>r.json()).then(completed=>{
                if(Array.isArray(completed)&&completed.length>=(study.maxParticipants||study.target||10)){
                  fetch(`${SUPA_URL}/rest/v1/studies?id=eq.${study.id}`,{
                    method:"PATCH",
                    headers:{"apikey":SUPA_KEY,"Authorization":`Bearer ${token}`,"Content-Type":"application/json","Prefer":"return=minimal"},
                    body:JSON.stringify({status:"closed"})
                  });
                  // Rejeter les participations encore en attente
                  fetch(`${SUPA_URL}/rest/v1/participations?study_id=eq.${study.id}&status=eq.pending_validation`,{
                    method:"PATCH",
                    headers:{"apikey":SUPA_KEY,"Authorization":`Bearer ${token}`,"Content-Type":"application/json"},
                    body:JSON.stringify({status:"rejected"})
                  });
                  setStudies(prev=>prev.map(x=>x.id===study.id?{...x,status:"closed"}:x));
                }
              }).catch(e=>console.error("Quota check error:",e));
            }
          }).catch(e=>console.error("Auto payout error:",e));
        }
      }else{
        // Profil ne correspond pas
        setStudies(prev=>prev.map(x=>x.id===study.id?{...x,status:"rejected"}:x));
        setNotifs(prev=>[{id:Date.now(),read:false,type:"reject",text:`❌ Entretien "${study.title}" : votre profil ne correspond pas aux critères de cette étude.`,time:"maintenant"},...prev]);
      }
    }catch(e){console.error("Save interview error:",e);}
    setShowAiChat(null);
  };
  const completeStudy=(id)=>{
    // Marks as "pending_validation" — researcher must validate to trigger payment
    const s=studies.find(x=>x.id===id);
    setStudies(prev=>prev.map(x=>x.id===id?{...x,status:"pending_validation"}:x));
    setPending(p=>p+s.pay);
    setNotifs(prev=>[{id:Date.now(),read:false,type:"complete",text:`Étude "${s.title}" marquée comme terminée. En attente de validation du chercheur.`,time:"maintenant"},...prev]);
  };
  const sendMsg=async()=>{
    if(!newMsg.trim())return;
    const text=newMsg;setNewMsg("");
  ...
  const computeMatchScore=(study, p)=>{
    ...
    <div style={{minHeight:"100vh",display:"flex",flexDirection:"column",fontFamily:FONT}}>
        ...
        <main className="p-main" style={{flex:1,padding:"20px 16px",overflowY:"auto",background:C.bg,width:"100%",minWidth:0,boxSizing:"border-box",overflowX:"hidden"}}>
          ...
          {tab==="history"&&(
            <div>
              ...
              {["joined","interview","pending_validation","completed"].map(st=>(
                <div key={st} style={{marginBottom:24}}>
                  <h3 style={{fontSize:14,fontWeight:700,color:C.muted,textTransform:"uppercase",letterSpacing:.8,marginBottom:10}}>
                    {st==="joined"?"En cours":st==="interview"?"🤖 Entretien IA en cours":st==="pending_validation"?"⏳ En attente de validation chercheur":"Terminées & Payées"}
                  </h3>
                  {studies.filter(s=>s.status===st).length===0?<p style={{color:C.dimmed,fontSize:14}}>Aucune.</p>:studies.filter(s=>s.status===st).map(s=>(
                    <Card key={s.id} style={{padding:"18px 22px",marginBottom:10,display:"flex",justifyContent:"space-between",alignItems:"center",border:st==="pending_validation"?`1px solid ${C.yellow}44`:undefined}}>
                      <div>
                      ...
                      <div style={{display:"flex",gap:12,alignItems:"center"}}>
                        ...
                        <Tag color={st==="joined"||st==="interview"?C.accent:st==="pending_validation"?C.yellow:C.green}>
                          {st==="joined"?"En cours":st==="interview"?"Entretien IA en cours":st==="pending_validation"?"Validation en attente":"Payée ✓"}
                        </Tag>
                        {st==="joined"&&<Btn small green onClick={()=>completeStudy(s.id)}>Marquer terminée</Btn>}
                        {st==="interview"&&<Btn small green onClick={()=>setShowAiChat({study:s,participationId:null})}>Reprendre l'entretien</Btn>}
                      </div>
                    </Card>
                  ))}
                </div>
      ...
      {showStudyDetail&&(
        <Modal onClose={()=>setShowStudyDetail(null)} title={showStudyDetail.title} wide>
          ...
          <div style={{display:"flex",gap:10}}>
            <Btn green full onClick={()=>joinStudy(showStudyDetail.id,showStudyDetail.answers||[])}>Participer à cette étude — {showStudyDetail.pay}€</Btn>
          </div>
        </Modal>
      )}

      {showAiChat&&(
        <AiInterviewChat study={showAiChat.study} onComplete={completeAiInterview} onClose={()=>setShowAiChat(null)}/>
      )}
  ...
  const pages={
    ...
    "pricing":{
      ...
      sections:[
        ...
        {icon:"🤖",title:"Option Entretiens IA — +10€ / étude",body:"Activez l’IA pour conduire les entretiens automatiquement. Vous définissez le guide de questions, l’IA gère tout le reste et vous livre un rapport complet. Disponible pour tous les formats."},
    ...
    "faq":{
      ...
      sections:[
        ...
        {icon:"🤖",title:"Comment fonctionne le mode IA ?",body:"En activant le mode IA (+10€), notre intelligence artificielle conduit l’entretien à votre place selon votre guide de questions. Elle gère les relances et vous livre un rapport complet avec verbatims sous 48h après chaque entretien."},
