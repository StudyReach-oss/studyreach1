...
function ParticipantDashboard({onLogout,showOnboarding,onOnboardingDone}){
  ...
  const joinStudy=async(id,answers)=>{
    ...
    // Sinon (étude lien) : la suite du parcours (choix du créneau obligatoire,
    // bouton "Accéder à l'étude", puis "J'ai terminé") vit dans l'onglet "history".
    // Sans cette redirection, le participant restait sur la liste des études dispo
    // après avoir cliqué "Participer" → "rien ne se passe", et l'étude ne ressortait
    // que via la bannière "Reprendre", court-circuitant l'accès réel au lien.
    if(s.ai){
  ...
  const accessClassicStudy=(s)=>{
    ...
    setShowDoneModal({...s,startedAt:new Date(startedAt).toISOString()});
