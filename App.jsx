...
function ResearcherDashboard({onLogout}){
            ...
            parts.forEach(p=>{
              ...
              if(computeMatchScore(studyForMatch,p)>=60){
                notifyEmail("new_study_match",{
                  ...
                  study_duration:d?.l,
                  study_price:Math.round(studyCost*0.9*100)/100,
  ...
  return(
    <div style={{minHeight:"100vh",display:"flex",flexDirection:"column",fontFamily:FONT}}>
      ...
      {/* MODAL: New Study */}
      {showStudyModal&&(
        <Modal onClose={()=>setShowStudyModal(false)} title={`Nouvelle étude — Étape ${nsStep+1}/6`} wide>
          {nsStep===0&&(
            <div>
          ...
          <div style={{display:"flex",justifyContent:"space-between",marginTop:24,paddingTop:16,borderTop:`1px solid ${C.border}`}}>
            ...
            {nsStep<5?<Btn onClick={()=>{setNsErr("");setNsStep(nsStep+1);}} disabled={(nsStep===0&&(!ns.title||!ns.theme))||(nsStep===1&&!ns.dur)||(nsStep===2&&(!ns.mode||(ns.mode==="link"&&!ns.link)))||(nsStep===3&&ns.mode==="ai"&&!(ns.ai_focus||"").trim())||(nsStep===4&&(ns.prescreening||[]).some(q=>!q.question||(q.acceptedAnswers||[]).length===0))}>Continuer →</Btn>:<Btn onClick={publishStudy}>🚀 Publier l'étude</Btn>}
          </div>
        </Modal>
      )}
        ...
        if(Array.isArray(data)&&data.length>0){
          ...
          const mapped=data
            ...
            .map(s=>({
              ...
              pay:s.cost_per_participant?Math.round(s.cost_per_participant*0.9*100)/100:27,
              ...
              if(studyData){
                setResumeParticipation({
                  ...
                  study:{
                    ...
                    pay:studyData.cost_per_participant?Math.round(studyData.cost_per_participant*0.9*100)/100:27,
                  }
                });
  ...
  useEffect(()=>{
    ...
    const loadMyParticipations=async()=>{
      ...
      try{
        ...
        setEarnings(paidParts.reduce((a,p)=>{const s=studyMap[p.study_id];return a+(s?Math.round(s.cost_per_participant*0.9*100)/100:0);},0));
        setPending(pendingParts.reduce((a,p)=>{const s=studyMap[p.study_id];return a+(s?Math.round(s.cost_per_participant*0.9*100)/100:0);},0));
      }catch(e){console.error("Load my participations error:",e);}
    };
