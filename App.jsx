...
function ResearcherDashboard({onLogout,showOnboarding,onOnboardingDone}){
  ...
  const generateInvoicePDF=(inv)=>{
    const clientName=inv.researcher_name||(researcherProfile.first+(researcherProfile.last?" "+researcherProfile.last:""));
    const clientCompany=inv.researcher_company||researcherProfile.company||"";
    const clientEmail=inv.researcher_email||researcherProfile.email||"";
    const amt=inv.amount_display||inv.amount;
    const w=window.open("","_blank");
    if(!w)return;
    w.document.write(`<!DOCTYPE html><html><head><meta charset="utf-8"><title>Facture ${inv.id}</title>
    <style>*{box-sizing:border-box;}body{font-family:Arial,sans-serif;padding:48px 56px;color:#111;max-width:680px;margin:0 auto;font-size:14px;}.header{display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:40px;}.logo{font-size:22px;font-weight:900;letter-spacing:-0.5px;}.logo span{color:#5b7cfa;}.from{font-size:12px;color:#666;margin-top:6px;line-height:1.6;}.inv-meta{text-align:right;}.inv-meta h2{font-size:22px;margin:0 0 4px;font-weight:800;}.inv-meta .meta{color:#666;font-size:13px;line-height:1.7;}.section{margin-bottom:28px;}.section-title{font-size:11px;font-weight:700;text-transform:uppercase;letter-spacing:.8px;color:#888;margin-bottom:8px;}.client-box{background:#f8f9fb;border:1px solid #e8eaf0;border-radius:8px;padding:14px 18px;font-size:13px;line-height:1.7;}table{width:100%;border-collapse:collapse;margin-top:8px;}thead tr{background:#f5f6fa;}th{padding:10px 14px;text-align:left;font-size:12px;font-weight:700;color:#555;text-transform:uppercase;letter-spacing:.5px;}td{padding:12px 14px;border-bottom:1px solid #eef0f5;font-size:14px;}.total-row{margin-top:20px;text-align:right;font-size:18px;font-weight:800;}.badge{display:inline-block;background:#e6faf2;color:#1a9e6b;font-weight:700;font-size:12px;padding:3px 10px;border-radius:20px;margin-left:10px;}.footer{margin-top:56px;padding-top:16px;border-top:1px solid #eee;font-size:11px;color:#aaa;text-align:center;line-height:1.8;}@media print{body{padding:20px 28px;}}</style></head><body>
    <div class="header"><div><div class="logo">&#9670; Study<span>Reach</span></div><div class="from">getstudyreach.com<br>contact@getstudyreach.com<br>France</div></div><div class="inv-meta"><h2>Facture</h2><div class="meta">N&deg; ${inv.id}<br>Date : ${inv.date}<br><span class="badge">&#10003; Pay&eacute;e</span></div></div></div>
    <div class="section"><div class="section-title">Factur&eacute; &agrave;</div><div class="client-box">${clientName?`<strong>${clientName}</strong><br>`:""}${clientCompany?`${clientCompany}<br>`:""}${clientEmail||""}</div></div>
    <div class="section"><div class="section-title">D&eacute;tail</div><table><thead><tr><th>Description</th><th style="text-align:right;">Montant HT</th><th style="text-align:right;">TVA</th><th style="text-align:right;">Total TTC</th></tr></thead><tbody><tr><td>Recharge portefeuille StudyReach</td><td style="text-align:right;">${amt}</td><td style="text-align:right;color:#888;">0&nbsp;%&nbsp;*</td><td style="text-align:right;font-weight:700;">${amt}</td></tr></tbody></table><div class="total-row">Total TTC&nbsp;: ${amt}</div></div>
    <div class="footer">StudyReach &middot; contact@getstudyreach.com &middot; getstudyreach.com<br>* TVA non applicable, art. 293 B du CGI (auto-entrepreneur)<br>Ce document vaut re&ccedil;u de paiement.</div>
  ...
  // Charge les factures depuis Supabase (persistées par le webhook stripe-webhook)
  const loadInvoices=async()=>{
    const token=Storage.get("sb_token");
    if(!token)return;
    try{
      const res=await fetch(`${SUPA_URL}/rest/v1/invoices?select=*&order=created_at.desc`,{
        headers:{"apikey":SUPA_KEY,"Authorization":`Bearer ${token}`}
      });
      const data=await res.json();
  ...
  // Stripe redirect : reload factures apres retour paiement (webhook peut avoir un delai)
  useEffect(()=>{
    const params=new URLSearchParams(window.location.search);
        ...
        <Modal onClose={()=>setShowStudyModal(false)} title={`Nouvelle étude — Étape ${nsStep+1}/7`} wide>
          ...
          {nsStep===3&&(
            <div>
              ...
              {/* Grille de mode : vidéos + tâche/enquête/journal (lien vs IA StudyReach).
                  Les autres types (présentiel) sont en "lien" d'office. */}
              {["video","video_group","task","survey","diary"].includes(ns.studyType)&&(
              ...
              {["inperson","inperson_group"].includes(ns.studyType)&&(
                <div style={{marginTop:18,paddingTop:16,borderTop:`1px solid ${C.border}`}}>
                  <p style={{fontWeight:700,marginBottom:10}}>Lieu &amp; informations pratiques <span style={{color:C.red}}>*</span></p>
