...
// Valide qu'une chaîne est bien une URL http(s) exploitable (évite les liens
// d'étude texte libre type "Jdjdjdj" qui résolvent silencieusement vers
// getstudyreach.com/Jdjdjdj — 404 sans aucun message pour le participant).
...
function payoutState(t){
  ...
  switch(t&&t.status_raw){
    ...
    case "abandoned":
    case "expired_unclaimed":return "abandoned"; // créneau expiré sans être réclamé : même traitement qu'un abandon, aucun montant dû
    case "joined":
...
async function notifyResearcherPendingValidation({researcherId,studyTitle,participantName,token}){
  if(!researcherId)return;
  try{
    const rRes=await fetch(`${SUPA_URL}/rest/v1/profiles_public?id=eq.${researcherId}&select=email,first_name`,{
      headers:{"apikey":SUPA_KEY,"Authorization":`Bearer ${token||""}`}
    });
  ...
  <div style={{marginBottom:14,...s}}>
    ...
    {isPass?(
      <div style={{position:"relative"}}>
        <input {...p} type={show?"text":"password"} style={{width:"100%",padding:"10px 40px 10px 13px",background:C.bg,border:`1px solid ${C.border}`,borderRadius:9,color:C.text,fontSize:14,outline:"none",boxSizing:"border-box",fontFamily:FONT}}/>
        <button type="button" onClick={()=>setShow(v=>!v)} tabIndex={-1} style={{position:"absolute",right:10,top:"50%",transform:"translateY(-50%)",background:"none",border:"none",cursor:"pointer",padding:4,display:"flex",alignItems:"center",color:C.muted}} aria-label={show?"Masquer le mot de passe":"Afficher le mot de passe"}>
          {show?(
            <svg width="17" height="17" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M17.94 17.94A10.94 10.94 0 0 1 12 20c-7 0-11-8-11-8a18.7 18.7 0 0 1 5.06-5.94M9.9 4.24A10.4 10.4 0 0 1 12 4c7 0 11 8 11 8a18.7 18.7 0 0 1-2.16 3.19m-6.72-1.07a3 3 0 1 1-4.24-4.24"/><line x1="1" y1="1" x2="23" y2="23"/></svg>
          ):(
...
function Landing({onNav}){
  ...
  return(
    <div style={{minHeight:"100vh",display:"flex",flexDirection:"column"}}>
      {/* Nav */}
      <header className="landing-header" style={{display:"flex",justifyContent:"space-between",alignItems:"center",flexWrap:"wrap",rowGap:10,gap:24,padding:"18px 56px",borderBottom:`1px solid ${C.border}`,position:"sticky",top:0,background:C.bg+"ee",backdropFilter:"blur(12px)",zIndex:50}}>
        <div style={{flexShrink:0}}><Logo/></div>
        <div className="landing-nav-wrap" style={{flex:"1 1 auto",minWidth:0,display:"flex",justifyContent:"center",flexWrap:"wrap"}}>
          <nav className="landing-nav" style={{display:"flex",gap:"10px 28px",alignItems:"center",flexWrap:"wrap",justifyContent:"center"}}>
        ...
        <div style={{maxWidth:1040,margin:"0 auto"}}>
          ...
          <div className="landing-section-grid" style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:16}}>
            {[
              ...
              ["💳","Paiements sécurisés","Portefeuille rechargeable, versements Stripe automatisés, validation manuelle. Vous ne payez que les participants validés : budget non utilisé recrédité, candidat refusé non facturé."],
            ].map(([ic,t,d])=>(
...
function ResearcherDashboard({onLogout,showOnboarding,onOnboardingDone}){
        ...
        if(user?.id){
          ...
          if(Array.isArray(studiesData)&&studiesData.length>0){
            ...
            if(studyIds.length>0){
              const partRes=await fetch(`https://bwaoxwfkqqpqvtpynwzh.supabase.co/rest/v1/participations?study_id=in.(${studyIds.join(",")})&select=*,profiles(first_name,last_name,email)`,{
                headers:{"apikey":"eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImJ3YW94d2ZrcXFwcXZ0cHlud3poIiwicm9sZSI6ImFub24iLCJpYXQiOjE3ODA2OTEzMDEsImV4cCI6MjA5NjI2NzMwMX0.utS5lj2nI-Bs0efelpaj9LHT3B_MSib5Ro8ESIz1-q8","Authorization":`Bearer ${token}`}
              });
      ...
      try{
        ...
        if(Array.isArray(data)){
          ...
          if(otherIds.length>0){
            const profilesRes=await fetch(`${SUPA_URL}/rest/v1/profiles_public?id=in.(${otherIds.join(",")})`,{
              headers:{"apikey":SUPA_KEY,"Authorization":`Bearer ${token}`}
            });
  ...
  const requestCloseStudy=async(studyId)=>{
    ...
    if(token){
      try{
        const res=await fetch(`${SUPA_URL}/rest/v1/participations?study_id=eq.${studyId}&select=paid,status,auto_validated`,{
          headers:{"apikey":SUPA_KEY,"Authorization":`Bearer ${token}`}
        });
        const data=await res.json();
        if(Array.isArray(data)){
          ...
          //   - validés (paid=true OU auto_validated=true, paiement déjà garanti) : NON remboursé.
          ...
          const validated=data.filter(p=>p.paid===true||p.auto_validated===true).length;
  ...
  const confirmCloseStudy=async()=>{
    ...
    if(token){
      try{
        const cr=await fetch(`${SUPA_URL}/rest/v1/participations?study_id=eq.${s.id}&select=paid,status,auto_validated`,{
          headers:{"apikey":SUPA_KEY,"Authorization":`Bearer ${token}`}
        });
        const arr=await cr.json();
        if(Array.isArray(arr)){
          // "validés" = payé OU déjà garanti par l'auto-validation J14 — alignement avec
          // la RPC serveur refund_study_close (ces participations ne sont jamais remboursées).
          validated=arr.filter(p=>p.paid===true||p.auto_validated===true).length;
    ...
    if(token){
      // ✅ Fermeture + remboursement du reliquat + rejet des actifs, côté SERVEUR (anti-fraude)
      try{
        const rRes=await fetch(`${SUPA_URL}/rest/v1/rpc/refund_study_close`,{
          method:"POST",
          headers:{"apikey":SUPA_KEY,"Authorization":`Bearer ${token}`,"Content-Type":"application/json"},
          body:JSON.stringify({p_study_id:s.id})
        });
        if(rRes.ok){ const nb=await rRes.json(); if(typeof nb==="number") setWallet(nb); }
        else console.error("refund_study_close error:",rRes.status,await rRes.text());
      }catch(e){console.error("refund_study_close error:",e);}
    }
    ...
    if(token&&researcherId){
      (async()=>{
        try{
          ...
          // ✅ Débit SERVEUR du budget bloqué (le solde n'est plus écrit par le navigateur — anti-fraude)
          try{
            const debitRes=await fetch(`${SUPA_URL}/rest/v1/rpc/block_study_budget`,{
              method:"POST",
              headers:{"apikey":SUPA_KEY,"Authorization":`Bearer ${token}`,"Content-Type":"application/json"},
              body:JSON.stringify({p_study_id:realId})
            });
            if(debitRes.ok){
              const nb=await debitRes.json();
              if(typeof nb==="number") setWallet(nb);
            }else{
        ...
        try{
          const pRes=await fetch(`${SUPA_URL}/rest/v1/profiles_public?role=eq.participant&select=id,email,first_name,birth_date,genre,country,city,status_pro,sector,education,devices,tech_level,has_camera,languages,mobile,long_term,smoker,alcohol,income,has_car,financial_products,family_status,housing_status,themes`,{
            headers:{"apikey":SUPA_KEY,"Authorization":`Bearer ${token}`}
          });
              ...
              try{
                ...
                if(Array.isArray(joinedParts)&&joinedParts.length>0){
                  ...
                  for(const jp of joinedParts){
                    try{
                      const pRes=await fetch(`${SUPA_URL}/rest/v1/profiles_public?id=eq.${jp.participant_id}&select=email,first_name`,{
                        headers:{"apikey":SUPA_KEY,"Authorization":`Bearer ${token}`}
                      });
  ...
  const rejectParticipant=async(participation)=>{
    ...
    // 🔒 Garde : une participation auto-validée (J14) a une garantie de paiement à
    // J30 même si le chercheur n'a pas encore cliqué "valider" — on ne peut donc
    // plus la refuser non plus (la RPC serveur bloque déjà ce cas, ce message
    // évite juste de laisser remonter une erreur brute à l'écran).
    if(participation.autoValidated){
      alert("Cette participation a été auto-validée (délai de 14 jours dépassé) : le paiement est garanti et ne peut plus être refusé.");
      return;
    }
    if(!confirm(`Refuser la candidature de ${participation.participantName||"ce participant"} pour "${participation.studyTitle}" ?`))return;
    const token=Storage.get("sb_token");
    // ✅ Refus + remboursement + alignement entretien, côté SERVEUR (anti-fraude)
    const study=studies.find(s=>s.id===participation.studyId);
    const refundAmount=study?.budget||participation.budget||0;
    if(token&&participation.participationId){
      try{
        const rRes=await fetch(`${SUPA_URL}/rest/v1/rpc/refund_reject_participation`,{
          method:"POST",
          headers:{"apikey":SUPA_KEY,"Authorization":`Bearer ${token}`,"Content-Type":"application/json"},
          body:JSON.stringify({p_participation_id:participation.participationId})
        });
        if(rRes.ok){ const nb=await rRes.json(); if(typeof nb==="number") setWallet(nb); }
        else console.error("refund_reject_participation error:",rRes.status,await rRes.text());
      }catch(e){console.error("refund_reject_participation error:",e);}
    }
  ...
  const generateInvoicePDF=(inv)=>{
    const buyerName=[researcherProfile.first,researcherProfile.last].filter(Boolean).join(" ")||researcherProfile.email||"";
    const buyerCompany=researcherProfile.company?`${researcherProfile.company}<br/>`:"";
    const buyerEmail=researcherProfile.email||"";
    const html=`<!DOCTYPE html><html><head><meta charset="utf-8"><title>Facture ${inv.id}</title>
    <style>body{font-family:Arial,sans-serif;padding:40px;color:#111;max-width:680px;margin:0 auto;font-size:14px;line-height:1.5;}
    h1{font-size:24px;margin:0 0 2px;}.muted{color:#666;font-size:12px;}
    .row{display:flex;justify-content:space-between;gap:24px;margin-top:24px;}
    .box{font-size:13px;}.box b{display:block;margin-bottom:4px;font-size:11px;text-transform:uppercase;color:#888;letter-spacing:.5px;}
    ...
    .legal{margin-top:36px;font-size:11px;color:#888;border-top:1px solid #eee;padding-top:14px;}
    @media print{.noprint{display:none;}}.noprint{margin-top:28px;}.noprint button{padding:9px 18px;font-size:13px;cursor:pointer;border-radius:8px;border:1px solid #ccc;background:#f5f5f5;}
    </style></head><body>
    <h1>◆ StudyReach</h1><div class="muted">Facture</div>
    <div class="row">
      <div class="box"><b>Émetteur</b>Samira Koibich — Entrepreneur individuel<br/>StudyReach<br/>5 rue Adalbert Simond<br/>78970 Mézières-sur-Seine, France<br/>SIRET : 106 697 063 00019 · APE 8299Z<br/>contact@getstudyreach.com</div>
      <div class="box"><b>Facturé à</b>${buyerCompany}${buyerName}<br/>${buyerEmail}</div>
    </div>
    <div class="row"><div class="box"><b>Facture n°</b>${inv.id}</div><div class="box"><b>Date</b>${inv.date}</div><div class="box"><b>Statut</b>${inv.status}</div></div>
    <table><thead><tr><th>Description</th><th style="text-align:right">Montant</th></tr></thead>
    <tbody><tr><td>Recharge de portefeuille StudyReach</td><td style="text-align:right">${inv.amount}</td></tr></tbody></table>
    <div class="total">Total : ${inv.amount}</div>
    <div class="legal">TVA non applicable, art. 293 B du CGI (franchise en base de TVA).<br/>StudyReach · Samira Koibich · SIRET 106 697 063 00019 · contact@getstudyreach.com</div>
    <div class="noprint"><button onclick="window.print()">Imprimer / Enregistrer en PDF</button></div>
    </body></html>`;
    const w=window.open("","_blank");
    if(w){ w.document.write(html); w.document.close(); w.focus(); setTimeout(()=>{ try{w.print();}catch(e){} },500); }
    else { downloadBlob(`facture-${inv.id}.html`, html, "text/html"); }
  };
  ...
  // 🧾 Charge l'historique des factures (recharges) DEPUIS LA BASE — sinon elles
  // disparaissent au rafraîchissement (elles n'étaient ajoutées qu'au retour de
  // Stripe). Numérotation séquentielle chronologique (obligation légale).
  const loadInvoices=async()=>{
    const token=Storage.get("sb_token");
    if(!token||!researcherId)return;
    try{
      const r=await fetch(`${SUPA_URL}/rest/v1/transactions?user_id=eq.${researcherId}&type=eq.recharge&order=created_at.asc&select=id,amount,created_at,status`,{headers:{"apikey":SUPA_KEY,"Authorization":`Bearer ${token}`}});
      const rows=await r.json();
      if(Array.isArray(rows)){
        const invs=rows.map((t,i)=>({
          id:`F${new Date(t.created_at).getFullYear()}-${String(i+1).padStart(4,"0")}`,
          date:new Date(t.created_at).toLocaleDateString("fr-FR",{day:"2-digit",month:"short",year:"numeric"}),
          amount:`${Number(t.amount).toFixed(2)}€`,
          status:(t.status==="completed"||t.status==="Payée")?"Payée":"En attente",
          rawDate:t.created_at,
        })).reverse();
        setInvoices(invs);

        // Alimente AUSSI l'état `transactions` avec les recharges DEPUIS LA BASE,
        // pour que la liste "Toutes les transactions" et le compteur "Total rechargé"
        // restent corrects même après un rafraîchissement (avant, les recharges
        // n'étaient ajoutées à `transactions` qu'au retour optimiste de Stripe et
        // disparaissaient au reload). On dédoublonne par id de recharge et on
        // préserve les autres types (payout, etc.) déjà présents dans l'état.
        const rechargeTx=rows.map(t=>({
          id:`recharge_${t.id}`,
          type:"recharge",
          ...
          amount:`+${Number(t.amount).toFixed(2)}€`,
          color:C.green,
          status:(t.status==="completed"||t.status==="Payée")?"Confirmé ✓":"En attente",
        }));
        setTransactions(prev=>{
          const nonRecharge=prev.filter(t=>t.type!=="recharge");
          return [...rechargeTx,...nonRecharge].sort((a,b)=>String(b.rawDate||"").localeCompare(String(a.rawDate||"")));
        });
      }
    }catch(e){console.error("loadInvoices error:",e);}
  };
  useEffect(()=>{ loadInvoices(); },[researcherId]);

  // Retour du checkout Stripe : ajout optimiste immédiat + rechargement officiel
  // depuis la base (le webhook y écrit la transaction, d'où le petit délai).
  // L'entrée optimiste est marquée _optimistic pour être remplacée proprement par
  // loadInvoices() (qui recharge les recharges DEPUIS la base) sans laisser de doublon.
  useEffect(()=>{
    const params=new URLSearchParams(window.location.search);
    if(params.get("payment")==="success"&&params.get("amount")){
      ...
      setTransactions(prev=>[{
        id:`recharge_optimistic_${Date.now()}`,type:"recharge",_optimistic:true,
        label:`Recharge portefeuille`,
        date,rawDate:new Date().toISOString(),amount:`+${amt.toFixed(2)}€`,color:C.green,status:"Confirmé ✓"
      },...prev]);
      setInvoices(prev=>[{
        id:`F${new Date().getFullYear()}-en-cours`,date,amount:`${amt.toFixed(2)}€`,status:"Payée"
      },...prev]);
      setTimeout(()=>{ loadInvoices(); },3000);
    }
  },[]);
    ...
    <div style={{minHeight:"100vh",display:"flex",flexDirection:"column",fontFamily:FONT}}>
      ...
      <div style={{display:"block",flex:1,position:"relative",overflow:"hidden"}}>
        ...
        <main className="p-main" style={{flex:1,padding:"20px 16px",overflowY:"auto",background:C.bg,width:"100%",minWidth:0,boxSizing:"border-box",overflowX:"hidden",marginLeft:isDesktop?210:0,transition:"margin-left .25s ease"}}>
                                ...
                                <div style={{display:"flex",flexDirection:"column",alignItems:"flex-end",gap:6,flexShrink:0,marginLeft:"auto"}}>
                                  ...
                                  <div style={{display:"flex",flexDirection:"column",gap:6,alignItems:"flex-end"}}>
                                    <Btn small green onClick={()=>validateParticipant({
                                      pay:t.pay,studyId:t.studyId,studyTitle:t.studyTitle,
          ...
          {/* AGENDA GLOBAL */}
          {tab==="agenda"&&<ResearcherAgenda studies={studies}/>}
                ...
                {invoices.length===0?<div style={{padding:"24px",textAlign:"center",color:C.muted,fontSize:14}}>Aucune facture pour l'instant. Elles apparaîtront après votre première recharge.</div>:invoices.map(inv=>(
                  <div key={inv.id} style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr auto",padding:"14px 20px",borderBottom:`1px solid ${C.border}`,alignItems:"center",fontSize:14}}>
                    ...
                    <div style={{display:"flex",gap:10,alignItems:"center"}}>
                      ...
                      <Btn secondary small onClick={()=>generateInvoicePDF(inv)}>📄 Facture</Btn>
                    </div>
                  </div>
                ))}
              </Card>
...
function ParticipantDashboard({onLogout,showOnboarding,onOnboardingDone}){
  ...
  const [showFinishConfirm,setShowFinishConfirm]=useState(null); // étude s dont le seuil de 80% est dépassé — on demande confirmation avant de soumettre (jamais de bascule silencieuse)
      ...
      try{
        ...
        if(Array.isArray(data)&&data.length>0){
          ...
          if(researcherIds.length>0){
            try{
              const rRes=await fetch(`${SUPA_URL}/rest/v1/profiles_public?id=in.(${researcherIds.join(",")})&select=id,first_name,last_name,company`,{
                headers:{"apikey":SUPA_KEY,"Authorization":`Bearer ${token||""}`}
              });
              const rData=await rRes.json();
          ...
          if(token&&userId&&!showAiChatRef.current){
            ...
            if(resumables.length>0){
              ...
              for(const resumable of resumables){
                // Seul incomplete_expires_at (posé par le blocage quota, cf. plus haut)
                // ou une étude explicitement clôturée signalent un blocage ici.
                // Le vrai abandon 24h est géré par le cron serveur "expire-participations"
                // qui bascule directement le statut en "abandoned" — ces participations-là
                // ne sont donc plus jamais "joined" et n'apparaissent plus dans cette liste.
                ...
                if(studyData){
                  const hasExpiry=!!resumable.incompleteExpiresAt&&new Date(resumable.incompleteExpiresAt).getTime()<=Date.now();
                  built.push({
                    participation:resumable,
                    studyClosed,
                    quotaExpired:!!(studyClosed||hasExpiry), // quota atteint (par d'autres) ou étude clôturée
                    timedOut:false, // plus de source client pour ce cas — géré par le cron serveur
                    study:{
          ...
          // On ne bascule plus JAMAIS en pending_validation en silence — on affiche
          // une demande de confirmation au participant (showFinishConfirm), qui décide.
          if(token&&userId&&!showFinishConfirm){
            for(const[studyId,part]of Object.entries(participationMap)){
              if(part.status==="joined"&&part.started_at){
                ...
                if(elapsed>=threshold){
                  setShowFinishConfirm({id:study.id,title:study.title,dur:study.duration,link:study.link,researcher_id:study.researcher_id,startedAt:part.started_at});
                  break; // une confirmation à la fois
                }
              }
            }
          }

          // NB : le vrai délai d'abandon (24h sans terminer) est géré côté serveur
          // par la Edge Function "expire-participations" (cron horaire) — elle bascule
          // le statut en "abandoned" et rembourse automatiquement le chercheur. On ne
          // duplique plus cette logique ici côté client : ça évite tout risque de
          // double traitement et ça marche même si le participant ne se reconnecte
          // jamais (le cron tourne indépendamment).
      ...
      try{
        ...
        if(user?.id){
          ...
          if(profiles?.[0]){
            const p=profiles[0];
            setProfile({
  email:p.email||"",bio:p.bio||"",phone:p.phone||"",email_verified:!!p.email_verified,
      ...
      try{
        ...
        const participationStudies=parts.filter(p=>p.status!=="rejected").map(p=>{
          ...
          return{
            ...
            // "expired_unclaimed" (créneau expiré non réclamé) n'a pas de rendu dédié
            // côté UI : on le rattache à "abandoned" pour qu'il reste visible dans
            // "Mes participations" au lieu de disparaître silencieusement.
            status:(p.status==="expired_unclaimed"?"abandoned":p.status)||"joined",target_criteria:s.target_criteria||null,
            studyType:s.study_type||"",linkAi:s.link_ai||false,ai:s.ai||false,
            ai_response_format:s.ai_response_format||{audio:false,video:false,tts:false},
  ...
  const MIN_WITHDRAW=0.01; // Retrait possible dès qu'il y a des gains (le plus petit gain étant déjà 9€, aucun seuil bloquant n'a de sens)
  ...
  const resumeNow=async(resumable)=>{
    if(resumable.studyClosed||resumable.timedOut){
      // Quota atteint, ou délai dépassé : pas de reprise possible dans les deux cas.
      // On masque simplement la bannière (garde-fou : même si un autre bouton
      // "Reprendre" venait à être ajouté ailleurs, il ne pourrait pas rouvrir une
      // participation expirée ou clôturée).
  ...
  const accessClassicStudy=(s,opts={})=>{
    // Garde-fou : un lien d'étude mal formé (texte libre au lieu d'une URL) ne doit
    // jamais déclencher window.open ni démarrer le chrono anti-fraude — cela
    // ouvrait silencieusement getstudyreach.com/<texte> (404) sans aucun message.
    if(s.link&&!isValidUrl(s.link)){
      alert("Le lien fourni par le chercheur pour cette étude semble invalide. Merci de le contacter via la messagerie pour signaler le problème.");
      return;
    }
    ...
    // Bonus : polling win.closed (surtout utile desktop). Ne soumet JAMAIS
    // automatiquement — on demande toujours confirmation au participant
    // (cf. showFinishConfirm) pour éviter toute bascule silencieuse.
    if(win){
      ...
      pollRef.current=setInterval(()=>{
        if(win.closed){
          ...
          if(focusListenerRef.current){window.removeEventListener("focus",focusListenerRef.current);focusListenerRef.current=null;}
          if(Date.now()-startedAt>=threshold)setShowFinishConfirm(s);
        }
      },1500);
    }

    // Bonus : listener focus (retour sur l'onglet StudyReach). Ne soumet
    // jamais tout seul — affiche juste une demande de confirmation.
    ...
    const onFocus=()=>{
      if(Date.now()-startedAt>=threshold){
        ...
        if(pollRef.current){clearInterval(pollRef.current);pollRef.current=null;}
        setShowFinishConfirm(s);
      }
    };
    focusListenerRef.current=onFocus;
    window.addEventListener("focus",onFocus);
  };
  // Désarme les détecteurs de fin (poll fenêtre + focus) sans rien soumettre.
  // Utilisé quand le participant dit explicitement "pas fini, je continue plus
  // tard" : tant qu'il n'a pas rouvert l'étude via "Reprendre", rien ne doit
  // se déclencher tout seul.
  const disarmFinishDetection=()=>{
    if(pollRef.current){clearInterval(pollRef.current);pollRef.current=null;}
    if(focusListenerRef.current){window.removeEventListener("focus",focusListenerRef.current);focusListenerRef.current=null;}
  };
  const sendMsg=async()=>{
  ...
  const openNewMsgModal=async()=>{
    ...
    try{
      const res=await fetch(`${SUPA_URL}/rest/v1/profiles_public?id=in.(${ids.join(",")})&select=id,first_name,last_name,company`,{
        headers:{"apikey":SUPA_KEY,"Authorization":`Bearer ${token}`}
      });
      const data=await res.json();
      if(Array.isArray(data)){
  ...
  return(
    <div style={{minHeight:"100vh",display:"flex",flexDirection:"column",fontFamily:FONT}}>
        ...
        <main className="p-main" style={{flex:1,padding:"20px 16px",overflowY:"auto",background:C.bg,width:"100%",minWidth:0,boxSizing:"border-box",overflowX:"hidden",marginLeft:isDesktop?210:0,transition:"margin-left .25s ease"}}>
                ...
                rp.studyClosed||rp.quotaExpired?(
                  ...
                  <Card key={rp.participation.id} style={{padding:"16px 18px",marginBottom:20,border:`1px solid ${C.yellow}44`,background:C.yellow+"11",display:"flex",justifyContent:"space-between",alignItems:"center",gap:12,flexWrap:"wrap"}}>
                    <div style={{flex:1,minWidth:0}}>
                      ...
                      {(()=>{
                        // Indicateur visible du temps restant avant expiration automatique (24h
                        // après le join, géré par le cron serveur) — évite la mauvaise surprise.
                        const startedAt=rp.participation.started_at;
                        if(!startedAt)return null;
                        const hoursLeft=24-((Date.now()-new Date(startedAt).getTime())/3600000);
                        if(hoursLeft<=0)return null;
                        const urgent=hoursLeft<3;
                        return(
                          <div style={{fontSize:11,color:urgent?C.red:C.muted,marginTop:4}}>
                            {urgent?`⚠️ Expire dans ${Math.max(1,Math.round(hoursLeft*60))} min`:`Expire dans ${Math.round(hoursLeft)}h si non terminée`}
                          </div>
                        );
                      })()}
                    </div>
          ...
          {tab==="settings"&&(
            <div style={{maxWidth:600}}>
              ...
              {(()=>{
                const sections=[
                  {label:"Compte",icon:"💰",fields:[{v:profile.bio}]},
              ...
              <Btn green full onClick={async()=>{
                ...
                await fetch(`${SUPA_URL}/rest/v1/profiles?id=eq.${userId}`,{
                  ...
                  body:JSON.stringify({
                    ...
                    themes:profile.themes,phone:profile.phone||null
                  })
                });
      ...
      {showDoneModal&&(()=>{
        ...
        <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,.6)",display:"flex",alignItems:"flex-end",justifyContent:"center",zIndex:200,padding:16}}>
          <div style={{background:C.surface,border:`1px solid ${C.border}`,borderRadius:20,padding:"24px 24px 28px",width:"100%",maxWidth:480,boxShadow:"0 -8px 40px rgba(0,0,0,.4)"}}>
            ...
            <button onClick={()=>{ disarmFinishDetection(); setShowDoneModal(null); }} style={{width:"100%",marginTop:10,background:"none",border:"none",color:C.muted,fontSize:13,cursor:"pointer",padding:"6px 0"}}>
              Continuer plus tard
            </button>
          </div>
        </div>
      ...
      {/* MODAL : demande de confirmation de fin d'étude — jamais de bascule
          silencieuse en pending_validation. Se déclenche quand le seuil de 80%
          de la durée prévue est dépassé (fenêtre fermée, retour de focus, ou
          reconnexion) : on demande toujours au participant de confirmer. */}
      {showFinishConfirm&&(
        <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,.6)",display:"flex",alignItems:"flex-end",justifyContent:"center",zIndex:210,padding:16}}>
          <div style={{background:C.surface,border:`1px solid ${C.border}`,borderRadius:20,padding:"24px 24px 28px",width:"100%",maxWidth:480,boxShadow:"0 -8px 40px rgba(0,0,0,.4)"}}>
            <div style={{display:"flex",alignItems:"center",gap:12,marginBottom:8}}>
              <span style={{fontSize:28}}>🤔</span>
              <div>
                <div style={{fontWeight:800,fontSize:16}}>{showFinishConfirm.title}</div>
                <div style={{fontSize:12,color:C.muted}}>On dirait que vous avez terminé cette étude</div>
              </div>
            </div>
            <div style={{fontSize:14,color:C.muted,marginBottom:20,lineHeight:1.5}}>
              Vous avez atteint {`${Math.round(80)}%`} du temps prévu pour cette étude. Avez-vous terminé ?
            </div>
            <Btn full green onClick={()=>{ triggerPendingValidation(showFinishConfirm); setShowFinishConfirm(null); }} style={{marginBottom:10}}>
              ✅ Oui, j'ai terminé — soumettre
            </Btn>
            <button onClick={()=>setShowFinishConfirm(null)} style={{width:"100%",background:"none",border:`1px solid ${C.border}`,borderRadius:12,color:C.text,fontSize:14,cursor:"pointer",padding:"12px 0"}}>
              Non, pas encore — je continue
            </button>
          </div>
        </div>
      )}
        ...
        <Modal onClose={()=>setShowWithdraw(false)} title={withdrawDone?"":"Retirer mes gains"}>
          {!withdrawDone?(
            <>
              ...
              {earnings<MIN_WITHDRAW&&(
                <div style={{background:C.yellow+"15",border:`1px solid ${C.yellow}55`,borderRadius:10,padding:"12px 14px",marginBottom:14,fontSize:13}}>
                  <span style={{fontWeight:700,color:C.yellow}}>Aucun gain à retirer</span>
                  <p style={{color:C.muted,marginTop:4}}>Vous n'avez pas encore de gains disponibles. Participez à des études pour gagner de l'argent, puis retirez vos gains vers votre compte bancaire.</p>
                </div>
              )}
              ...
              <Btn full green
                ...
                onClick={async()=>{
                  if(earnings<MIN_WITHDRAW){alert(`Vous n'avez aucun gain à retirer pour le moment.`);return;}
                  try{
                    ...
                    if(data.success){
                      ...
                      alert(data.error||"Ce moyen de paiement est déjà associé à un autre compte StudyReach.");
                    }else{
              ...
              >
                {earnings<MIN_WITHDRAW?`Aucun gain à retirer`:`Retirer ${earnings.toFixed(2)}€`}
              </Btn>
            ...
            <div style={{textAlign:"center",padding:"16px 0"}}>
              ...
              <p style={{fontSize:12,color:C.muted,marginBottom:20}}>Compte bancaire (Stripe) · Délai 24–72h</p>
    ...
    const loadAdmin=async()=>{
      ...
      try{
        ...
        if(Array.isArray(uData)){
          ...
          const dupes=[
            ...clusterBy(u=>{const v=(u.payout_fingerprint||"").trim().toLowerCase();return v||null;},"Compte bancaire"),
      ...
      <style>{`
        ...
        *{box-sizing:border-box;margin:0;padding:0;}
        ...
        @media(max-width:640px){
          ...
          .landing-header{padding:14px 16px !important;gap:10px 12px !important;flex-wrap:wrap !important;}
          .landing-nav-wrap{order:3 !important;flex-basis:100% !important;overflow-x:visible !important;justify-content:center !important;}
          .landing-nav{gap:10px 18px !important;flex-wrap:wrap !important;justify-content:center !important;white-space:normal !important;}
          .landing-nav a{font-size:13px !important;}
...
// On exclut : no_show_participant (créneau libéré de toute façon), participation_rejected,
// prescreening_rejected. On inclut completed car un RDV passé complété doit apparaître
// dans l'historique.
const AGENDA_VALID_STATUSES = new Set(["joined","interview","pending_validation","completed"]);

function ResearcherAgenda({ studies }){
  ...
  const studyMeta = React.useMemo(()=>{
    const m = new Map(); agendaStudies.forEach(s=>m.set(s.id,s)); return m;
  },[agendaStudies]);

  async function load(){
    if(studyIds.length===0){ setAllRows([]); return; }
    const token = Storage.get("sb_token")||"";
    const H = { "apikey": SUPA_KEY, "Authorization": `Bearer ${token||SUPA_KEY}` };
    try{
      const slotsData = await fetch(
        `${SUPA_URL}/rest/v1/slots?study_id=in.(${studyIds.join(",")})&taken=eq.true` +
        `&select=id,datetime,study_id,participation_id,participations(participant_id,status,validation_deadline,profiles(first_name,last_name,email,phone))` +
        `&order=datetime.asc`,
        {headers:H}
      ).then(r=>r.json()).catch(()=>[]);
      ...
      const list = (Array.isArray(slotsData)?slotsData:[])
        .filter(slot=>{
          // Ne garder que les participations valides (exclut no_show, rejected, etc.)
          const st = slot.participations?.status||"";
          return AGENDA_VALID_STATUSES.has(st) && slot.participations?.profiles;
        })
        .map(slot=>{
          const s   = studyMeta.get(slot.study_id);
          const p   = slot.participations.profiles;
          const par = slot.participations;
          const stMeta = STUDY_TYPES.find(t=>t.id===s?.studyType);
          const dt  = new Date(slot.datetime);
          const isPast = dt < now;

          // Badge délai : si pending_validation et deadline dépassée
          const deadlineOverdue =
            par.status==="pending_validation" && par.validation_deadline &&
            new Date(par.validation_deadline) < now;

          return {
            id:slot.id, datetime:slot.datetime, dtObj:dt, isPast,
            studyId:slot.study_id,
            studyTitle:s?.title||"Étude",
            studyClosed: s?.status==="closed",
            studyType:s?.studyType||"",
            typeIcon:stMeta?.icon||"📅",
            typeColor:stMeta?.color||C.accent,
            meetingAddress:s?.meeting_address||"",
            participantName:`${p.first_name||""} ${(p.last_name||"").charAt(0)}.`.trim(),
            participantStatus:par.status||"",
            deadlineOverdue,
          };
        });

      setAllRows(list.sort((a,b)=>a.dtObj-b.dtObj));
    }catch(e){ console.error("Agenda global error:",e); setAllRows([]); }
  }

  React.useEffect(()=>{ load(); },[studyIds.join(",")]);

  // Polling toutes les 30s — rafraîchit l'agenda sans dépendance externe.
  React.useEffect(()=>{
    if(studyIds.length===0) return;
  ...
  if(allRows===null) return(
    <div style={{maxWidth:760}}>
      <h1 style={{fontSize:22,fontWeight:800,marginBottom:4}}>Agenda</h1>
      <div style={{color:C.muted,fontSize:14,marginTop:24}}>Chargement…</div>
    </div>
  );

  const todayStart = new Date(); todayStart.setHours(0,0,0,0);
  const upcoming = allRows.filter(r=>r.dtObj>=todayStart);
  const past     = allRows.filter(r=>r.dtObj<todayStart);
  const rows     = showHistory ? past : upcoming;

  // Groupement par jour.
  const byDay = new Map();
  rows.forEach(r=>{
    const day = r.dtObj.toLocaleDateString("fr-FR",{weekday:"long",day:"2-digit",month:"long",year:"numeric"});
    if(!byDay.has(day)) byDay.set(day,[]);
    byDay.get(day).push(r);
  });

  const emptyMsg = showHistory
  ...
  return(
    <div style={{maxWidth:760}}>
      ...
      <div style={{display:"flex",alignItems:"baseline",justifyContent:"space-between",marginBottom:4,flexWrap:"wrap",gap:8}}>
        ...
        <div style={{display:"flex",gap:6}}>
          <button
            ...
            style={{fontSize:12,fontWeight:700,padding:"4px 14px",borderRadius:20,border:"none",cursor:"pointer",
              background:!showHistory?C.accent:"transparent",color:!showHistory?"#fff":C.muted,
              outline:!showHistory?"none":`1px solid ${C.border}`}}>
            À venir {upcoming.length>0&&<span style={{background:C.accent+"33",color:C.accent,borderRadius:10,padding:"1px 6px",marginLeft:4}}>{upcoming.length}</span>}
          </button>
          <button
            onClick={()=>setShowHistory(true)}
            style={{fontSize:12,fontWeight:700,padding:"4px 14px",borderRadius:20,border:"none",cursor:"pointer",
              background:showHistory?C.muted:"transparent",color:showHistory?"#fff":C.muted,
              outline:showHistory?"none":`1px solid ${C.border}`}}>
      ...
      <p style={{color:C.muted,fontSize:13,marginBottom:24}}>
        Tous vos rendez-vous confirmés, toutes études confondues — mis à jour en temps réel.
      </p>

      {rows.length===0?(
        <Card style={{padding:"24px",textAlign:"center",color:C.muted,fontSize:14}}>{emptyMsg}</Card>
      ):(
        [...byDay.entries()].map(([day,items])=>(
          <div key={day} style={{marginBottom:22}}>
            <div style={{fontSize:12,fontWeight:700,color:C.muted,textTransform:"capitalize",letterSpacing:0.5,marginBottom:8}}>{day}</div>
            <Card style={{overflow:"hidden"}}>
              {items.map((r,i)=>(
                <div key={r.id} style={{
                  display:"flex",alignItems:"center",gap:14,padding:"14px 18px",
                  borderBottom:i<items.length-1?`1px solid ${C.border}`:"none",
                  opacity:r.isPast&&!showHistory?0.5:1, // atténue si passé dans vue "à venir" (ne devrait pas arriver)
                }}>
                  {/* Heure */}
                  <div style={{fontWeight:800,fontSize:15,minWidth:52,flexShrink:0}}>
                    {r.dtObj.toLocaleTimeString("fr-FR",{hour:"2-digit",minute:"2-digit"})}
                  </div>

                  {/* Info participant + étude */}
                  <div style={{flex:1,minWidth:0}}>
                    <div style={{display:"flex",alignItems:"center",gap:6,flexWrap:"wrap"}}>
                      <span style={{fontWeight:700,fontSize:14}}>{r.participantName}</span>
                      {/* Badge délai dépassé (chercheur n'a pas validé à temps) */}
                      {r.deadlineOverdue&&(
                        <span style={{fontSize:10,fontWeight:800,background:C.yellow+"22",color:C.yellow,padding:"2px 7px",borderRadius:10,border:`1px solid ${C.yellow}44`}}>
                          ⏰ Délai dépassé
                        </span>
                      )}
                    </div>
                    <div style={{color:C.muted,fontSize:12,marginTop:2,display:"flex",alignItems:"center",gap:6,flexWrap:"wrap"}}>
                      <span>{r.studyTitle}</span>
                      {r.meetingAddress&&["inperson","inperson_group"].includes(r.studyType)&&(
                        <span>· 📍 {r.meetingAddress}</span>
                      )}
                      {/* Badge étude fermée */}
                      {r.studyClosed&&(
                        <span style={{fontSize:10,fontWeight:700,background:C.border,color:C.muted,padding:"1px 6px",borderRadius:8}}>
                          Étude fermée
                        </span>
                      )}
                    </div>
                  ...
                  {/* Badge type d'étude */}
                  <span style={{flexShrink:0,background:r.typeColor+"22",color:r.typeColor,fontSize:11,fontWeight:700,
                    padding:"3px 10px",borderRadius:20,border:`1px solid ${r.typeColor}44`}}>
                    {r.typeIcon} {STUDY_TYPES.find(t=>t.id===r.studyType)?.label||""}
                  </span>
                </div>
              ))}
            </Card>
...
function StudyAgenda({ studyId, studyTitle, studyType, meetingAddress, meetingNotes, maxParticipants }){
  ...
  // Signalement manuel uniquement : clôture la participation en no_show_participant,
  // applique l'action demandée sur le créneau (slotAction), envoie l'email neutre,
  // puis rafraîchit l'agenda.
  ...
  // Études INDIVIDUELLES (video/inperson) : proposer reschedule ou delete
  // Études DE GROUPE (video_group/inperson_group) : SEULEMENT delete (groupe continue N-1)
  ...
  function reportParticipantNoShow(p){
    ...
    if(!isAsync){
      // Vidéo et présentiel : demander au chercheur ce qu'il veut faire du créneau.
      // Distinction groupe/individuel :
    ...
    // Async (task/survey/diary) : pas de "présence" à constater,
