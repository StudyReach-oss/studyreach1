...
// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
function AiInterviewChat({study,profile,matchScore,onComplete,onClose}){
  ...
  const finishInterview=async()=>{
    ...
    try{
      ...
      const res=await fetch(`${SUPA_URL}/functions/v1/ai-interview`,{
        method:"POST",
        headers:{"Content-Type":"application/json","Authorization":`Bearer ${Storage.get("sb_token")||""}`,"apikey":SUPA_KEY},
        body:JSON.stringify({action:"report",study:{title:study.title,theme:study.theme,description:study.description,ai_focus:study.ai_focus,target_criteria:study.target_criteria||null},profile:profile||null,matchScore:typeof matchScore==="number"?matchScore:null,messages})
      });
      const data=await res.json();
...
function ParticipantDashboard({onLogout}){
  ...
  return(
    <div style={{minHeight:"100vh",display:"flex",flexDirection:"column",fontFamily:FONT}}>
      ...
      {showAiChat&&(
        <AiInterviewChat study={showAiChat.study} profile={profile} matchScore={computeMatchScore(showAiChat.study,profile)} onComplete={completeAiInterview} onClose={()=>setShowAiChat(null)}/>
      )}
