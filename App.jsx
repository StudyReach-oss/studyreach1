  ...
  return(
    <div style={{minHeight:"100vh",display:"flex",flexDirection:"column"}}>
      ...
      <section className="landing-section-pad" style={{padding:"60px 56px",borderTop:`1px solid ${C.border}`,background:C.surface}}>
        <div className="landing-section-grid" style={{maxWidth:900,margin:"0 auto",display:"grid",gridTemplateColumns:"1fr 1fr",gap:48,alignItems:"center"}}>
          <div>
  ...
  const submit=async()=>{
    ...
    try{
      if(isLogin){
        ...
        // Attendre un peu que Supabase enregistre le compte
        await new Promise(r=>setTimeout(r,1500));
        // Tenter une connexion directe pour obtenir un token frais
        let loginData=null;
        for(let i=0;i<3;i++){
          ...
          if(loginData.access_token)break;
          await new Promise(r=>setTimeout(r,1000));
        }
        if(!loginData?.access_token)throw new Error(loginData?.error_description||loginData?.error||"Connexion impossible après inscription. Réessayez.");
        const token=loginData.access_token;
        const refresh=loginData.refresh_token||"";
        const finalUserId=userId||loginData.user?.id;
        if(finalUserId){
          await fetch(`${SUPA_URL}/rest/v1/profiles`,{
            method:"POST",
            headers:{"apikey":SUPA_KEY,"Authorization":`Bearer ${token}`,"Content-Type":"application/json","Prefer":"return=representation"},
            body:JSON.stringify({id:finalUserId,email:f.email,first_name:f.first,last_name:f.last,role:isPart?"participant":"researcher",paypal_email:f.paypal||null,profession:f.prof||null,company:f.company||null,wallet:0}),
          });
        }
        Storage.set("sb_token",token);
        Storage.set("sb_refresh",refresh);
      ...
      if(refreshToken){
        try{
          const res=await fetch("https://bwaoxwfkqqpqvtpynwzh.supabase.co/auth/v1/token?grant_type=refresh_token",{
            method:"POST",
            headers:{"apikey":"sb_publishable_SsnkELg6dLx--AjHaW0ShA_N1ISmMKg","Content-Type":"application/json"},
