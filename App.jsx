...
//  1) Envoie un email de confirmation de suppression.
//  2) Appelle l'Edge Function "delete-account" (service_role) qui supprime
//     l'utilisateur auth + ses données en cascade côté serveur.
//  3) Filet de sécurité : si l'Edge Function n'est pas disponible, marque le
...
async function deleteAccount({userId, token, email, firstName, role}){
  // 1) Email de confirmation (fire-and-forget, ne bloque jamais)
  try{ notifyEmail("account_deleted",{email, first_name:firstName||"", role}); }catch(e){}

  let edgeOk=false;
  // 2) Suppression complète côté serveur via Edge Function
  try{
  ...
  // 3) Filet de sécurité : bloquer la reconnexion si l'Edge Function a échoué
    ...
    <Modal onClose={deleting?()=>{}:onClose} title="⚠️ Supprimer mon compte">
      ...
      <div style={{display:"flex",gap:10,marginTop:4}}>
        ...
        <Btn danger full disabled={!ok||deleting} onClick={async()=>{
          setDeleting(true);
          try{ await onConfirm(); }catch(e){ setDeleting(false); }
