    ...
    <div style={{minHeight:"100vh",display:"flex",flexDirection:"column",fontFamily:FONT}}>
              ...
              {(()=>{
                ...
                <div style={{marginBottom:28}}>
                  ...
                  {pendingStudies.map(study=>{
                    const pendingCount=transactions.filter(t=>t.studyId===study.id&&t.status_raw==="pending_validation"&&!t.paid).length;
                    return(
                    <Card key={study.id} style={{padding:"16px 20px",marginBottom:10,border:`1px solid ${C.yellow}33`}}>
                      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                        <div>
                          ...
                          <div style={{fontSize:12,color:C.muted}}>{pendingCount} participant(s) en attente · {study.budget}€/participant</div>
                        </div>
                        <Btn small onClick={()=>setExpandedTx(study.title)} style={{background:C.yellow,color:"#000"}}>Voir & Valider →</Btn>
                      </div>
                    </Card>
                    );
                  })}
      ...
      {showAiReport&&(
        <Modal onClose={()=>setShowAiReport(null)} title="🤖 Rapport StudyReach AI" wide>
          {/* En-tête participant : nom cliquable + pastille match */}
          {(showAiReport.participantName||showAiReport.participantId)&&(
            <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:14,padding:"10px 14px",background:C.surfaceHigh,borderRadius:10,border:`1px solid ${C.border}`}}>
              <span style={{fontSize:13,color:C.muted}}>Participant :</span>
              <span style={{fontWeight:700,color:C.text,cursor:showAiReport.participantId?"pointer":"default",textDecoration:showAiReport.participantId?"underline":"none"}}
                onClick={()=>showAiReport.participantId&&setShowParticipantProfile(showAiReport.participantId)}>
                {showAiReport.participantName||"Voir le profil"}
              </span>
              {typeof showAiReport.matchScore==="number"&&(
                <span style={{fontSize:11,fontWeight:800,padding:"2px 9px",borderRadius:10,background:showAiReport.matchScore>=70?C.green+"22":showAiReport.matchScore>=40?C.yellow+"22":C.red+"22",color:showAiReport.matchScore>=70?C.green:showAiReport.matchScore>=40?C.yellow:C.red,border:`1px solid ${showAiReport.matchScore>=70?C.green:showAiReport.matchScore>=40?C.yellow:C.red}44`}}>
                  {showAiReport.matchScore}% match
                </span>
              )}
            </div>
          )}
