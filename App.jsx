...
function ResearcherDashboard({onLogout,showOnboarding,onOnboardingDone}){
  ...
  const [ns,setNs]=useState({title:"",theme:"",dur:"",mode:"",link:"",ai:false,linkAi:false,ai_focus:"",ai_response_format:{audio:false,video:false,tts:false},studyType:"",maxParticipants:null,description:"",prescreening:[],
  target_criteria:{
  ...
  useEffect(()=>{
    const params=new URLSearchParams(window.location.search);
    if(params.get("payment")) return; // laissé au handler de recharge PayPal
      ...
      (async()=>{
        ...
        try{
          ...
          if(Array.isArray(parts)){
            ...
            const matchedParticipants=parts.filter(p=>p.email&&computeMatchScore(studyForMatch,p)>=60);
  ...
  const validateParticipant=async(participation)=>{
    ...
    try{
      ...
      if(res.ok&&data.success){
        ...
        // ℹ️ Pas de débit du wallet ici : le budget de ce participant a déjà
        // été bloqué à la publication de l'étude (voir publishStudy). Le
        // débiter à nouveau ici causait un double prélèvement. La seule
        // correction de solde liée à cette participation est le crédit en
        // cas de refus (voir rejectParticipant) ou de fermeture d'étude
        // (voir confirmCloseStudy) — la validation ne touche donc plus au wallet.
        const newTx={
          id:Date.now(),
          type:"payout",
        ...
        // 📧 Participation validée + paiement envoyé
      ...
      {showStudyModal&&(
        <Modal onClose={()=>setShowStudyModal(false)} title={`Nouvelle étude — Étape ${nsStep+1}/6`} wide>
          ...
          <div style={{display:"flex",justifyContent:"space-between",marginTop:24,paddingTop:16,borderTop:`1px solid ${C.border}`}}>
            ...
            {nsStep<5?<Btn onClick={()=>{setNsErr("");setNsStep(nsStep+1);}} disabled={(nsStep===0&&(!ns.title||!ns.theme||!ns.maxParticipants||ns.maxParticipants>500))||(nsStep===1&&!ns.dur)||(nsStep===2&&(!ns.mode||(ns.mode==="link"&&!ns.link)))||(nsStep===3&&ns.mode==="ai"&&!(ns.ai_focus||"").trim())||(nsStep===4&&(ns.prescreening||[]).some(q=>!q.question||(q.acceptedAnswers||[]).length===0))}>Continuer →</Btn>:<Btn onClick={publishStudy}>🚀 Publier l'étude</Btn>}
          </div>
        </Modal>
      )}
        ...
        <Modal onClose={()=>setShowStudyDetail(null)} title={showStudyDetail.title} wide>
          ...
          {["video","video_group","inperson","inperson_group"].includes(showStudyDetail.studyType)&&(
            <div style={{marginTop:14}}>
              ...
              <StudyAgenda studyId={showStudyDetail.id} studyTitle={showStudyDetail.title}/>
            </div>
          )}
...
function ParticipantDashboard({onLogout,showOnboarding,onOnboardingDone}){
      ...
      try{
        ...
        if(seenPaidIds.current===null){
          ...
          newlyPaid.forEach(id=>{
            ...
            if(s){
              const net=Math.round(s.cost_per_participant*0.9*100)/100;
              pushNotif(setNotifs,{id:Date.now()+Math.random(),read:false,type:"pay",text:`💸 Paiement de ${net}€ reçu pour "${s.title}" — disponible sur votre PayPal.`});
            }
          });
          seenPaidIds.current=nowPaidIds;
        }
  ...
  const joinStudy=async(id,answers)=>{
    ...
    if(s.ai){
      ...
      const needSlot=await studyNeedsSlot(id,participationId,token);
      if(needSlot){
  ...
  const resumeNow=async(resumable)=>{
    ...
    if(participation.status==="interview"){
      ...
      const token=Storage.get("sb_token");
      const needSlot=await studyNeedsSlot(study.id,participation.id,token);
      if(needSlot){
        setTabPersist("history");
      }else{
  ...
  // Détermine si une étude impose le choix d'un créneau avant l'accès au lien :
  // vrai uniquement si l'étude possède des créneaux ET qu'aucun n'est réservé pour
  // cette participation. Fail-open (false) en cas d'erreur réseau pour ne jamais
  // bloquer à tort l'accès à une étude.
  const studyNeedsSlot=async(studyId,participationId,token)=>{
    if(!token||!participationId)return false;
    try{
      const bRes=await fetch(`${SUPA_URL}/rest/v1/slots?participation_id=eq.${participationId}&select=id`,{
        headers:{"apikey":SUPA_KEY,"Authorization":`Bearer ${token}`}
      });
      const bRows=await bRes.json().catch(()=>[]);
      if(Array.isArray(bRows)&&bRows.length>0)return false; // créneau déjà réservé
      ...
      return Array.isArray(stRows)&&stRows.length>0; // a des créneaux & aucun réservé
    }catch(e){console.error("studyNeedsSlot error:",e);return false;}
  };
              ...
              {["joined","interview","pending_validation","completed","abandoned"].map(st=>{
                ...
                <div key={st} style={{marginBottom:24}}>
                  ...
                  {rows.length===0?<p style={{color:C.dimmed,fontSize:14}}>Aucune.</p>:rows.map(s=>{
                    ...
                    return(
                    <Card key={s.id} style={{padding:"18px 22px",marginBottom:10,display:"flex",justifyContent:"space-between",alignItems:"center",border:st==="pending_validation"?`1px solid ${C.yellow}44`:st==="abandoned"?`1px solid ${C.red}44`:isAutoUnpaid?`1px solid ${C.orange}44`:undefined}}>
                      <div>
                        ...
                        {st==="joined"&&s.link&&(slotBookStatus[s.participationId]&&slotBookStatus[s.participationId].hasSlots&&!slotBookStatus[s.participationId].booked
                          ?<div style={{fontSize:11,color:C.yellow,marginTop:4}}>Choisissez d'abord un créneau ci-dessous pour accéder à l'étude.</div>
                          :<div style={{fontSize:11,color:C.muted,marginTop:4}}>Cliquez sur "Accéder à l'étude", puis déclarez avoir terminé pour transmettre votre participation au chercheur.</div>)}
                          ...
                          <div style={{marginTop:10,width:"100%"}}>
                            <SlotPicker
                              ...
                              onBooked={(slot)=>{
                                ...
                                if(profile?.email) notifyEmail("slot_confirmed",{email:profile.email,first_name:profile.first||"",study_title:s.title,slot_date:fmt.date,slot_time:fmt.time,slot_tz:fmt.tz});
                              }}
                              onEmpty={async()=>{
                      ...
                      <div style={{display:"flex",gap:12,alignItems:"center"}}>
                        <div style={{fontWeight:800,color:st==="abandoned"?C.muted:isAutoUnpaid?C.orange:C.green,fontSize:18}}>{s.pay}€</div>
                        {st==="joined"&&s.link&&(()=>{
                          const ss=slotBookStatus[s.participationId];
                          const needSlot=!!(ss&&ss.hasSlots&&!ss.booked);
                          return <Btn small disabled={needSlot} onClick={()=>{ if(needSlot) return; accessClassicStudy(s); }}>🔗 Accéder à l'étude</Btn>;
                        })()}
                        {st==="joined"&&(()=>{
                          const ss=slotBookStatus[s.participationId];
                          const needSlot=!!(ss&&ss.hasSlots&&!ss.booked);
                          return <Btn small disabled={needSlot} onClick={()=>{ if(needSlot) return; setShowDoneModal(s); }} style={{background:C.yellow+"22",border:`1px solid ${C.yellow}44`,color:C.yellow}}>✓ J'ai terminé</Btn>;
                        })()}
                        <Tag color={st==="joined"||st==="interview"?C.accent:st==="pending_validation"?C.yellow:st==="abandoned"?C.red:isAutoUnpaid?C.orange:C.green}>
                          {st==="joined"?"En cours":st==="interview"?"Entretien IA en cours":st==="pending_validation"?"Validation en attente":st==="abandoned"?"Expirée":isAutoUnpaid?"Auto-validée · paiement en attente":"Payée ✓"}
                        </Tag>
        ...
        <main className="p-main" style={{flex:1,padding:"20px 16px",overflowY:"auto",background:C.bg,width:"100%",minWidth:0,boxSizing:"border-box",overflowX:"hidden"}}>
          ...
          {tab==="multicomptes"&&(()=>{
            ...
            <div>
              ...
              {adminDupes.length>=8&&(
                <SearchBar value={dupeSearch} onChange={setDupeSearch} placeholder="🔍 Rechercher par PayPal, nom ou email…"/>
              )}
              {loadingAdmin?(
...
function OnboardingModal({role,onClose,onStart}){
  ...
  const accent=isPart?C.green:C.accent;
  const steps=isPart?[
    {icon:"👋",title:"Bienvenue sur StudyReach",body:"Donnez votre avis sur des produits et services réels, et soyez payé directement sur PayPal. Voici l'essentiel en un instant."},
    ...
    {icon:"💸",title:"Soyez payé",body:"Le montant affiché pour chaque étude est exactement ce que vous touchez, versé sur votre PayPal sous 24-48h après validation. Retrait possible dès 5€ de gains."},
  ]:[
...
function StudyAgenda({ studyId, studyTitle }){
  ...
  function load(){
    if(!studyId) return;
    const token = Storage.get("sb_token")||"";
    // Récupère tous les slots de l'étude, avec le profil + participation_id du participant si réservé.
    // Les slots dont la participation a déjà été marquée no_show_participant/expired_unclaimed ont
    // été libérés (taken=false, participation_id=null) par l'action correspondante : ils retombent
    // naturellement dans le pool "libre" ci-dessous, pas de filtrage supplémentaire nécessaire.
    fetch(`${SUPA_URL}/rest/v1/slots?study_id=eq.${studyId}&select=id,datetime,taken,participation_id,participations(participant_id,status,profiles(first_name,last_name,email))&order=datetime.asc`,{
      headers:{ "apikey": SUPA_KEY, "Authorization": `Bearer ${token||SUPA_KEY}` }
    })
    .then(r=>r.json())
    .then(data=>{
      if(!Array.isArray(data)){ setRows([]); return; }
      // Regrouper par datetime pour compter les places libres par créneau
      const map = new Map();
      data.forEach(slot=>{
        const dt = slot.datetime;
        if(!map.has(dt)) map.set(dt,{datetime:dt, taken:[], free:0});
        if(slot.taken && slot.participations?.profiles){
          const p = slot.participations.profiles;
          map.get(dt).taken.push({
  ...
  if(rows.length===0) return <div style={{fontSize:12,color:C.muted,padding:"8px 0"}}>Aucun créneau défini pour cette étude.</div>;
  ...
  return(
    <div style={{display:"flex",flexDirection:"column",gap:6}}>
      {rows.map(row=>{
        ...
        // Si le créneau a plusieurs places : on affiche une ligne par participant + "X places libres"
        const lines = [
          ...participants.map(p=>({ ...p, isFreeLine:false })),
          ...(free>0 ? [{name:`${free} place${free>1?"s":""} libre${free>1?"s":""}`, isFreeLine:true}] : [])
        ];

        return lines.map((line,i)=>(
          <div key={row.datetime+"-"+i} style={{display:"flex",alignItems:"center",justifyContent:"space-between",gap:8,background:C.surfaceHigh,borderRadius:8,padding:"9px 12px",border:`1px solid ${C.border}`,flexWrap:"wrap"}}>
            <div style={{fontSize:13,color:C.muted,textTransform:"capitalize",minWidth:160}}>{i===0 ? label : ""}</div>
            <div style={{display:"flex",alignItems:"center",gap:8,marginLeft:"auto"}}>
              <div style={{fontSize:13,fontWeight:line.isFreeLine?400:600,color:line.isFreeLine?C.muted:C.text,textAlign:"right"}}>
                {line.isFreeLine ? <span style={{color:C.dimmed}}>— {line.name}</span> : line.name}
              </div>
              {!line.isFreeLine && slotPassed && (
                <button
...
// Répartition imposée : base = floor(total/nbSlots), le dernier créneau absorbe le reste.
// computeSlotCapacities(20,4) => [5,5,5,5] ; computeSlotCapacities(30,4) => [7,7,7,9]
function computeSlotCapacities(totalParticipants, nbSlots){
  ...
  return Array.from({length:nbSlots},(_,i)=> i===nbSlots-1 ? base+remainder : base);
}
...
function CreationSlotBuilder({ slots, maxParticipants, onChange }){
  ...
  return(
    <div style={{background:C.surfaceHigh,borderRadius:10,padding:14,border:`1px solid ${C.border}`,marginTop:14}}>
      ...
      {slots.length>0&&(
        <div style={{fontSize:11,color:C.accentLight,marginBottom:10}}>Les {maxParticipants||0} participants seront répartis automatiquement sur tes {slots.length} créneau{slots.length>1?"x":""}.</div>
      )}
...
function SlotPicker({ studyId, participationId, token, onBooked, onEmpty, onStatus }){
  ...
  React.useEffect(()=>{
    supaSlot(`slots?participation_id=eq.${participationId}&select=*`,"GET",null,token).then(async data=>{
      if(Array.isArray(data)&&data.length>0){ setBooked(data[0]); setLoading(false); if(onStatus) onStatus({hasSlots:true,booked:true}); return; }
  ...
  async function bookSlot(dt){
    ...
    setBooked(updated[0]);
    setBooking(null);
    if(onStatus) onStatus({hasSlots:true,booked:true});
