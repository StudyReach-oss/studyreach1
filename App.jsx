                ...
                {(()=>{
                  ...
                  return(<>
                    <div style={headerStyle}>
                      <span>Description</span>{isDesktop&&<span>Date</span>}<span>Statut</span><span style={{textAlign:"right"}}>Montant</span>
                    </div>
                      ...
                      return(
                        <div key={"g"+ri}>
                          <div onClick={()=>setExpandedTx(open?null:row.key)} style={{display:"grid",gridTemplateColumns:txCols,padding:rowPad,borderBottom:`1px solid ${C.border}`,alignItems:"center",fontSize:13,cursor:"pointer",transition:"background .15s",background:open?C.accentGlow+"88":"transparent"}}>
                            <div style={{display:"flex",alignItems:"center",gap:8,minWidth:0}}>
                              <span style={{fontSize:18,flexShrink:0}}>{pendingCount>0?"⏳":inprogressCount>0?"🎙️":paidCount>0?"💸":rejectedCount>0?"🚫":"⌛"}</span>
                              ...
                              {g.items.map((t,ii)=>(
                                <div key={ii} style={{display:"grid",gridTemplateColumns:txCols,padding:isDesktop?"10px 20px 10px 48px":"10px 14px 10px 28px",borderBottom:ii<g.items.length-1?`1px solid ${C.border}`:"none",alignItems:"flex-start",fontSize:12,gap:8}}>
                                  ...
                                  {(()=>{const st=payoutState(t);const noMoney=(st==="rejected"||st==="abandoned");
                                    return <span style={{fontWeight:700,color:st==="paid"?C.green:(noMoney||st==="inprogress")?C.muted:C.yellow,textAlign:"right",textDecoration:noMoney?"line-through":"none"}}>{noMoney?"0.00€":t.amount}</span>;
                                  })()}
                                </div>
                              ))}
                            </div>
