...
function EquipmentCheck({needsAudio,needsVideo,onReady,onClose,isResume}){
  ...
  return(
    <Modal onClose={onClose} title={isResume?"🔄 Reprise de l'entretien":"🎙️ Vérification de l'équipement"}>
...
function AiInterviewChat({study,profile,matchScore,onComplete,onClose,initialMessages,participationId}){
  ...
  const toggleVideoRecording=()=>{
    ...
    try{
      ...
      recorder.onstop=async()=>{
        ...
        try{
          const text=await transcribeBlob(blob,actualMime);
  ...
  const startMicMeter=async()=>{
    try{
      ...
      // La reconnaissance vocale capte déjà le micro ; un getUserMedia
      // concurrent l'empêche de recevoir l'audio (→ aucun transcript).
      // On réutilise le flux caméra/enregistrement déjà ouvert s'il existe.
  ...
  const toggleRecording=()=>{
    ...
    // FIX 3: Stop path — _shouldStop flag breaks the auto-restart loop cleanly.
    if(recording){
      recognitionRef.current._shouldStop=true;
      recognitionRef.current?.stop();
      setRecording(false);
      stopMicMeter();
      // Arrêt de l'enregistrement audio parallèle → transcription Whisper (source fiable).
      const ar=audioRecorderRef.current;
      if(ar&&ar.rec){
        ar.rec.onstop=async()=>{
          try{
            const blob=new Blob(audioChunksRef.current,{type:ar.mime});
            try{audioStreamRef.current?.getTracks().forEach(t=>t.stop());}catch(e){}
            audioStreamRef.current=null;
            if(blob.size>0){
              setTranscribing(true);
              try{const text=await transcribeBlob(blob,ar.mime);if(text&&text.trim())setInput(text.trim());}finally{setTranscribing(false);}
            }
          }catch(e){console.error("Audio transcribe error:",e);}
        };
        try{ar.rec.stop();}catch(e){}
    ...
    startMicMeter();
    // Enregistrement audio en parallèle de la reco vocale, pour disposer d'un
    // fichier à transcrire côté serveur (fiable sur tous les appareils).
    (async()=>{
      try{
        ...
        rec.start(1000);
        audioRecorderRef.current={rec,mime:rec.mimeType||"audio/webm"};
      }catch(e){console.error("Audio capture error:",e);audioRecorderRef.current=null;}
    })();
    let finalTranscript="";
    ...
    const startRecognition=()=>{
      ...
      // FIX 3: continuous=true stops automatically after ~5 s on mobile.
      // Using continuous=false + auto-restart on onend keeps the mic alive indefinitely.
      recognition.continuous=false;
      recognition.interimResults=true;
      recognition._shouldStop=false;

      recognition.onresult=(event)=>{
        // FIX: Don't update input after user pressed Stop/Send
        if(recognition._shouldStop)return;
        let interim="";
      ...
      recognition.onerror=(e)=>{
        // 'no-speech' is normal on mobile — just restart silently.
        if(e.error==="no-speech")return;
        console.error("Speech recognition error:",e);
        recognition._shouldStop=true;
        setRecording(false);
        stopMicMeter();
        // Micro indisponible (occupé par l'enregistrement, refusé, etc.) :
        // on prévient l'utilisateur qu'il peut taper sa réponse à la place.
        if(e.error==="not-allowed"||e.error==="audio-capture"||e.error==="service-not-allowed"){
          setInput(prev=>prev||"");
          alert("La transcription vocale n'a pas pu démarrer sur cet appareil. Vous pouvez taper votre réponse dans le champ, puis appuyer sur Envoyer.");
        }
      };
      recognition.onend=()=>{
        // Auto-restart unless the user pressed Stop.
        if(!recognition._shouldStop){
          try{recognition.start();}catch(e){
            // If restart fails, fall back to stopped state
            setRecording(false);
            stopMicMeter();
          }
        }
  ...
  React.useEffect(()=>{
    return()=>{
      ...
      cancelAnimationFrame(micRafRef.current);
      try{micStreamRef.current?.getTracks().forEach(t=>t.stop());}catch(e){}
      try{audioRecorderRef.current?.rec?.stop();}catch(e){}
      try{audioStreamRef.current?.getTracks().forEach(t=>t.stop());}catch(e){}
    };
  },[]);
  ...
  const send=()=>{
    ...
    // FIX: Stop mic cleanly (set _shouldStop so the auto-restart loop doesn't fire)
    if(recording){
      if(recognitionRef.current)recognitionRef.current._shouldStop=true;
      try{recognitionRef.current?.stop();}catch(e){}
      setRecording(false);
      stopMicMeter();
    }
    // FIX: If video is recording, stop it and wait for onstop (which uploads) before continuing.
    // We still send the text answer immediately; the video upload happens in the background.
    if(videoRecording){
      try{mediaRecorderRef.current?.stop();setVideoRecording(false);}catch(e){}
    }
    const text=input.trim();
    // Require at least some text input (video alone isn't sent as a chat message)
    if(!text)return;
      ...
      {finished?(
        ...
        <div>
          ...
          <div style={{display:"flex",gap:8}}>
            ...
            <Btn onClick={()=>{if(recording){if(recognitionRef.current)recognitionRef.current._shouldStop=true;try{recognitionRef.current?.stop();}catch(e){}setRecording(false);stopMicMeter();}send();}} disabled={loading||transcribing||(!input.trim()&&!recording)} style={{alignSelf:"flex-end"}}>{transcribing?"Transcription…":"Envoyer"}</Btn>
          </div>
        </div>
      )}
    </Modal>
...
function StudyCard({s,full,onClick,onClose,validated=0}){
  ...
  const synthPending=isAiStudy&&validated>=(s.target||1)&&!s.global_synthesis;
      ...
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",cursor:"pointer"}} onClick={onClick}>
        <div style={{flex:1}}>
          <div style={{display:"flex",gap:8,alignItems:"center",marginBottom:6,flexWrap:"wrap"}}>
            ...
            {!s.global_synthesis&&!synthPending&&allValidated&&<Tag color={C.green}>Toutes les validations effectuées</Tag>}
          </div>
...
// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
function AiReportModal({data,onClose,onViewProfile}){
  const tabs=[
    {id:"rapport",label:"📋 Rapport",show:!!data.report},
    {id:"video",label:"🎥 Vidéo",show:!!(data.video_urls?.length>0)},
    ...
    <Modal onClose={onClose} title="Rapport individuel" wide>
      ...
      {typeof data.quality_score==="number"&&(
        <div style={{marginBottom:18,padding:"12px 16px",background:C.surfaceHigh,borderRadius:12,border:`1px solid ${C.border}`}}>
          <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",gap:10,marginBottom:data.quality_detail?10:0,flexWrap:"wrap"}}>
            <div style={{fontWeight:800,fontSize:13,color:C.text}}>Qualité des réponses</div>
            <ScoreBadge score={data.quality_score} label="qualité" title="Pertinence et sérieux des réponses"/>
          </div>
          {data.quality_detail&&(
            <>
              <div style={{display:"flex",gap:8,flexWrap:"wrap",marginBottom:data.quality_detail.justification?8:0}}>
                {[["Pertinence","pertinence"],["Profondeur","profondeur"],["Soin","soin"],["Sérieux","serieux"]].map(([lab,key])=>typeof data.quality_detail[key]==="number"?(
                  <div key={key} style={{fontSize:11,color:C.muted,background:C.bg,border:`1px solid ${C.border}`,borderRadius:8,padding:"3px 9px"}}>{lab} <b style={{color:C.text}}>{data.quality_detail[key]}%</b></div>
                ):null)}
              </div>
              {data.quality_detail.justification&&(
                <div style={{fontSize:12,color:C.muted,fontStyle:"italic"}}>« {data.quality_detail.justification} »</div>
              )}
            </>
          )}
      ...
      {activeTab==="video"&&(
        <VideoPlayer storagePaths={data.video_urls}/>
      )}
...
function ResearcherDashboard({onLogout}){
  ...
  const confirmCloseStudy=async()=>{
    ...
    if((s.ai||s.linkAi||s.mode==="IA")&&s.joined>0&&!s.global_synthesis){
      (async()=>{
        try{
          const intRes=await fetch(`${SUPA_URL}/rest/v1/interviews?study_id=eq.${s.id}&status=in.(completed,pending_validation)&select=report,transcript,match_score`,{
            headers:{"apikey":SUPA_KEY,"Authorization":`Bearer ${token}`}
          });
          const interviews=await intRes.json();
          if(Array.isArray(interviews)&&interviews.length>0){
            const reportsText=interviews.map((iv,idx)=>`--- Entretien ${idx+1} (score matching: ${iv.match_score||"N/A"}%) ---\n${iv.report||"(pas de rapport)"}`).join("\n\n");
            const synthesisPrompt=`Tu es un expert en recherche qualitative. Voici les rapports de ${interviews.length} entretiens réalisés dans le cadre de l'étude "${s.title}" (thème: ${s.theme||"non précisé"}).\n\n${reportsText}\n\nGénère une SYNTHÈSE GLOBALE de cette étude en français avec les sections suivantes :\n## Résumé exécutif\n(3-4 phrases résumant les grands enseignements)\n\n## Tendances principales\n(Les patterns récurrents identifiés dans plusieurs entretiens)\n\n## Verbatims clés\n(Les citations les plus représentatives ou marquantes)\n\n## Points de divergence\n(Ce qui varie d'un participant à l'autre)\n\n## Recommandations\n(3-5 recommandations actionnables basées sur les insights)\n\nSois synthétique, factuel et orienté insights.`;
            const aiRes=await fetch("https://api.anthropic.com/v1/messages",{
              method:"POST",
              headers:{"Content-Type":"application/json"},
              body:JSON.stringify({model:"claude-sonnet-4-6",max_tokens:1000,messages:[{role:"user",content:synthesisPrompt}]})
            });
            const aiData=await aiRes.json();
            const synthesis=aiData?.content?.[0]?.text||null;
            if(synthesis){
              await fetch(`${SUPA_URL}/rest/v1/studies?id=eq.${s.id}`,{
                method:"PATCH",
                headers:{"apikey":SUPA_KEY,"Authorization":`Bearer ${token}`,"Content-Type":"application/json","Prefer":"return=minimal"},
        ...
        if(token&&participation.studyId){
          try{
            ...
            if(study&&!quotaReached){
              ...
              setNotifs(prev=>[{id:Date.now(),read:false,type:"complete",text:`✅ Participant validé — ${completed.length}/${target} validés pour "${study.title}".`,time:"maintenant"},...prev]);
            }
            if(quotaReached){
              setNotifs(prev=>[{id:Date.now(),read:false,type:"complete",text:`🎉 Tous les participants sont validés (${completed.length}/${target}) pour "${study.title}" — vous pouvez fermer l'étude quand vous le souhaitez.`,time:"maintenant"},...prev]);

              // 🤖 Si étude IA → générer synthèse globale à partir des rapports individuels
              if(study.ai||study.linkAi||study.mode==="IA"){
                (async()=>{
                  try{
                    const intRes=await fetch(`${SUPA_URL}/rest/v1/interviews?study_id=eq.${study.id}&status=in.(completed,pending_validation)&select=report,transcript,match_score`,{
                      headers:{"apikey":SUPA_KEY,"Authorization":`Bearer ${token}`}
                    });
                    const interviews=await intRes.json();
                    if(Array.isArray(interviews)&&interviews.length>0){
...
Sois synthétique, factuel et orienté insights.`;
        ...
        const detail=data.details||data.error||data.message||raw||("HTTP "+res.status);
        console.error("Payout failed:",res.status,data);
        alert("Erreur paiement PayPal.\n\nDétail : "+detail+"\n\nVérifie la configuration PayPal de la fonction /api/payout sur Vercel (identifiants, mode sandbox/live, Payouts activé).");
      }
    }catch(e){
      console.error("Payout error:",e);
      alert("Erreur réseau lors du paiement : "+(e?.message||e));
    }
  };
  ...
  const rejectParticipant=async(participation)=>{
    ...
    if(refundAmount>0){
      setNotifs(prev=>[{id:Date.now(),read:false,type:"refund",text:`💰 ${refundAmount.toFixed(2)}€ recrédités sur votre portefeuille (candidature refusée).`,time:"maintenant"},...prev]);
    }
  };
  ...
  return(
    <div style={{minHeight:"100vh",display:"flex",flexDirection:"column",fontFamily:FONT}}>
      ...
      <div style={{display:"block",flex:1,position:"relative",overflow:"hidden"}}>
        ...
        {/* Content */}
        <main className="p-main" style={{flex:1,padding:"20px 16px",overflowY:"auto",background:C.bg,width:"100%",minWidth:0,boxSizing:"border-box",overflowX:"hidden",marginLeft:sideOpen?210:0,transition:"margin-left .25s ease"}}>
          ...
          {tab==="studies"&&(
            <div>
              ...
              <div style={{display:"flex",flexDirection:"column",gap:12}}>
                {studies.map(s=>(<StudyCard key={s.id} s={s} full validated={transactions.filter(t=>t.studyId===s.id&&t.paid).length} onClick={()=>setShowStudyDetail(s)} onClose={requestCloseStudy}/>))}
              </div>
            </div>
          )}
          ...
          {tab==="validations"&&(()=>{
            ...
            const counts={
              pending:payoutTx.filter(t=>t.status_raw==="pending_validation"&&!t.paid).length,
            ...
            const filtered=payoutTx.filter(t=>{
              if(valFilter==="pending")return t.status_raw==="pending_validation"&&!t.paid;
              if(valFilter==="paid")return t.paid;
              if(valFilter==="rejected")return t.status_raw==="rejected";
              return true;
            });
            const byStudy={};
            filtered.forEach(t=>{(byStudy[t.studyId]=byStudy[t.studyId]||[]).push(t);});
            ...
            return(
              <div style={{maxWidth:860}}>
                ...
                <div style={{display:"flex",gap:14,marginBottom:20,flexWrap:"wrap",fontSize:12,color:C.muted,background:C.surfaceHigh,border:`1px solid ${C.border}`,borderRadius:10,padding:"10px 14px"}}>
                  <span><b style={{color:C.text}}>% match</b> — adéquation du profil aux critères de ciblage de l'étude.</span>
                  <span><b style={{color:C.text}}>% qualité</b> — pertinence et sérieux des réponses à l'entretien IA. Indicatif : un participant bref mais pertinent peut rester valide.</span>
                </div>
                ...
                {studyGroups.length===0?(
                  <Card style={{padding:"40px 20px",textAlign:"center",color:C.muted}}>
                    ...
                    Aucun participant {valFilter==="pending"?"en attente":valFilter==="paid"?"validé":valFilter==="rejected"?"refusé":""} pour le moment.
                  </Card>
                  ...
                  <div key={study.id} style={{marginBottom:24}}>
                    ...
                    <div style={{display:"flex",flexDirection:"column",gap:10}}>
                      {byStudy[study.id].map((t,i)=>{
                        ...
                        const statusColor=isPaid?C.green:isPending?C.yellow:isRejected?C.red:C.muted;
                        const statusLabel=isPaid?"Payé ✓":isPending?"En attente":isRejected?"Refusé":"En cours";
                        return(
                            ...
                            <div style={{display:"flex",alignItems:"center",gap:10,flexWrap:"wrap"}}>
                              ...
                              <div style={{flex:1,minWidth:140}}>
                                <div style={{display:"flex",alignItems:"center",gap:8,flexWrap:"wrap",marginBottom:5}}>
                                  <span style={{fontWeight:700,fontSize:14}}>{t.participantName||"Participant"}</span>
                                  ...
                                  <ScoreBadge score={t.qualityScore} label="qualité" title="Pertinence et sérieux des réponses à l'entretien IA"/>
                                  <Tag color={statusColor} style={{fontSize:10}}>{statusLabel}</Tag>
                                </div>
                                <div style={{display:"flex",gap:6,flexWrap:"wrap"}}>
                                  {t.participantId&&(
                                    <button onClick={()=>setShowParticipantProfile(t.participantId)} style={{background:C.accentGlow,border:`1px solid ${C.accent}44`,borderRadius:7,color:C.accentLight,fontSize:11,fontWeight:600,cursor:"pointer",padding:"4px 10px"}}>👤 Profil</button>
                                  )}
                                  {t.aiInterview&&(t.aiInterview.report||t.aiInterview.transcript?.length>0)&&(
                                    <button onClick={()=>setShowAiReport({...t.aiInterview,participantId:t.participantId,participantName:t.participantName,matchScore:t.matchScore,defaultTab:"rapport"})} style={{background:"#a855f722",border:"1px solid #a855f744",borderRadius:7,color:"#a855f7",fontSize:11,fontWeight:600,cursor:"pointer",padding:"4px 10px"}}>📋 Rapport individuel</button>
                                  )}
                                  {t.aiInterview?.video_urls?.length>0&&(
                                    <button onClick={()=>setShowAiReport({...t.aiInterview,participantId:t.participantId,participantName:t.participantName,matchScore:t.matchScore,defaultTab:"video"})} style={{background:C.greenGlow,border:`1px solid ${C.green}44`,borderRadius:7,color:C.green,fontSize:11,fontWeight:600,cursor:"pointer",padding:"4px 10px"}}>🎥 Vidéo</button>
                                  )}
                                  {t.aiInterview?.transcript?.length>0&&(
                                    <button onClick={()=>setShowAiReport({...t.aiInterview,participantId:t.participantId,participantName:t.participantName,matchScore:t.matchScore,defaultTab:"transcript"})} style={{background:C.surfaceHigh,border:`1px solid ${C.border}`,borderRadius:7,color:C.muted,fontSize:11,fontWeight:600,cursor:"pointer",padding:"4px 10px"}}>💬 Transcript</button>
                                  )}
                                </div>
                              </div>
                              {isPending&&(
          ...
          {tab==="recaps"&&(
            <div style={{maxWidth:860}}>
              ...
              {(()=>{const aiStudies=studies.filter(s=>s.mode==="IA"||s.ai||s.linkAi);return aiStudies.length===0?(
                <Card style={{padding:"40px 20px",textAlign:"center",color:C.muted}}>
                  <div style={{fontSize:32,marginBottom:10}}>📊</div>
                  ...
                  <Card key={study.id} style={{padding:"20px 22px",marginBottom:16,border:study.global_synthesis?`1px solid #a855f733`:undefined}}>
                    <div style={{marginBottom:16}}>
                      <div style={{display:"flex",gap:8,alignItems:"center",marginBottom:6,flexWrap:"wrap"}}>
                        ...
                        <Tag color={study.status==="active"?C.green:study.status==="closed"?C.red:C.muted}>{study.status==="active"?"Active":study.status==="closed"?"Fermée":"Terminée"}</Tag>
                        {isAiStudy&&<Tag color="#a855f7">🤖 IA</Tag>}
                        {avgMatch!==null&&(
                    ...
                    {study.global_synthesis?(
                      <div style={{background:"linear-gradient(135deg,#1a0a2e,#0e0a1a)",border:"1px solid #a855f744",borderRadius:12,padding:"16px 18px"}}>
                        <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:12}}>
                          ...
                          <span style={{fontWeight:800,fontSize:13,color:"#c084fc"}}>Synthèse collective IA</span>
                        </div>
                        <div style={{fontSize:13,lineHeight:1.75,color:C.text}}>
          ...
          {tab==="wallet"&&(
            <div style={{maxWidth:800}}>
              ...
              {(()=>{
                const pendingStudyIds=new Set(transactions.filter(t=>t.status_raw==="pending_validation"&&!t.paid).map(t=>t.studyId));
                const pendingStudies=studies.filter(s=>pendingStudyIds.has(s.id));
                if(pendingStudies.length===0)return null;
                return(
                <div style={{marginBottom:28}}>
                  ...
                  {pendingStudies.map(study=>{
                    const pendingTxs=transactions.filter(t=>t.studyId===study.id&&t.status_raw==="pending_validation"&&!t.paid);
                    return(
                    <Card key={study.id} style={{padding:"16px 20px",marginBottom:10,border:`1px solid ${C.yellow}33`}}>
                      {/* En-tête étude */}
                      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:pendingTxs.length>0?12:0}}>
                        <div>
                          <div style={{fontWeight:700,fontSize:14,marginBottom:2}}>{study.title}</div>
                          <div style={{fontSize:12,color:C.muted}}>{pendingTxs.length} participant(s) · {study.budget}€/participant</div>
                        </div>
                      </div>
                      {/* Liste des participants en attente */}
                      {pendingTxs.map((t,ii)=>(
                        <div key={ii} style={{display:"flex",alignItems:"center",gap:10,padding:"10px 0",borderTop:`1px solid ${C.border}`,flexWrap:"wrap"}}>
                          {/* Avatar + Nom */}
                          <Avatar initials={(t.participantName||"?").split(" ").map(n=>n[0]).join("").toUpperCase().slice(0,2)} size={34}/>
                          <div style={{flex:1,minWidth:120}}>
                            <div style={{display:"flex",alignItems:"center",gap:8,flexWrap:"wrap"}}>
                              <span
                                style={{fontWeight:700,fontSize:13,cursor:t.participantId?"pointer":"default",color:t.participantId?C.accentLight:C.text,textDecoration:t.participantId?"underline":"none"}}
                                onClick={()=>t.participantId&&setShowParticipantProfile(t.participantId)}
                              >
                                {t.participantName||"Participant"}
                              </span>
                              {typeof t.matchScore==="number"&&(
                                <span style={{fontSize:10,fontWeight:800,padding:"2px 8px",borderRadius:10,background:t.matchScore>=70?C.green+"22":t.matchScore>=40?C.yellow+"22":C.red+"22",color:t.matchScore>=70?C.green:t.matchScore>=40?C.yellow:C.red,border:`1px solid ${t.matchScore>=70?C.green:t.matchScore>=40?C.yellow:C.red}44`}}>
                                  {t.matchScore}% match
                                </span>
                              )}
                              <ScoreBadge score={t.qualityScore} label="qualité" title="Pertinence et sérieux des réponses à l'entretien IA"/>
                            ...
                            <div style={{display:"flex",gap:8,marginTop:5,flexWrap:"wrap"}}>
                              {t.participantId&&(
                                <button onClick={()=>setShowParticipantProfile(t.participantId)} style={{background:C.accentGlow,border:`1px solid ${C.accent}44`,borderRadius:7,color:C.accentLight,fontSize:11,fontWeight:600,cursor:"pointer",padding:"3px 10px"}}>
                                  👤 Voir profil
                                </button>
                              )}
                              {t.aiInterview?.report&&(
                                <button onClick={()=>setShowAiReport({...t.aiInterview,participantId:t.participantId,participantName:t.participantName,matchScore:t.matchScore,defaultTab:"rapport"})} style={{background:"#a855f722",border:"1px solid #a855f744",borderRadius:7,color:"#a855f7",fontSize:11,fontWeight:600,cursor:"pointer",padding:"3px 10px"}}>
                                  📋 Rapport
                                </button>
                              )}
                              {t.aiInterview?.video_urls?.length>0&&(
                                <button onClick={()=>setShowAiReport({...t.aiInterview,participantId:t.participantId,participantName:t.participantName,matchScore:t.matchScore,defaultTab:"video"})} style={{background:C.greenGlow,border:`1px solid ${C.green}44`,borderRadius:7,color:C.green,fontSize:11,fontWeight:600,cursor:"pointer",padding:"3px 10px"}}>
                                  🎥 Vidéo
                                </button>
                              )}
                              {t.aiInterview?.transcript?.length>0&&!t.aiInterview?.report&&(
                                <button onClick={()=>setShowAiReport({...t.aiInterview,participantId:t.participantId,participantName:t.participantName,matchScore:t.matchScore,defaultTab:"transcript"})} style={{background:C.surfaceHigh,border:`1px solid ${C.border}`,borderRadius:7,color:C.muted,fontSize:11,fontWeight:600,cursor:"pointer",padding:"3px 10px"}}>
                                  💬 Transcript
                                </button>
                              )}
                            </div>
                          ...
                          <div style={{display:"flex",gap:6,flexShrink:0}}>
                            <Btn small onClick={()=>validateParticipant({
                              ...
                              participantId:t.participantId,
                            })} style={{background:C.yellow,color:"#000",fontSize:11,padding:"5px 12px"}}>✓ Valider & Payer</Btn>
                            <Btn small onClick={()=>rejectParticipant({
                              ...
                              participantEmail:t.participantEmail,
                              participationId:t.participationId,
                            })} style={{background:"transparent",border:`1px solid ${C.red}66`,color:C.red,fontSize:11,padding:"5px 12px"}}>✕ Refuser</Btn>
                          </div>
                        </div>
                      ))}
                    </Card>
        ...
        <Modal onClose={()=>setShowStudyModal(false)} title={`Nouvelle étude — Étape ${nsStep+1}/7`} wide>
            ...
            <div>
              ...
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:14,marginBottom:16}}>
                {[{id:"link",icon:"🔗",title:"Mon propre lien",desc:"Gérez l'entretien et l'analyse vous-même (Zoom, Typeform, Calendly…)"},{id:"ai",icon:"🤖",title:"Entretiens IA",desc:"Notre IA conduit et synthétise l'entretien",extra:"+10€ / participant"}].map(m=>(
                  <div key={m.id} onClick={()=>setNs({...ns,mode:m.id,ai:m.id==="ai",studyType:""})} style={{background:ns.mode===m.id?C.accentGlow:C.bg,border:`1.5px solid ${ns.mode===m.id?C.accent:C.border}`,borderRadius:12,padding:"20px",cursor:"pointer",textAlign:"center"}}>
          ...
          {nsStep===3&&(
            <div>
              ...
              {ns.mode==="ai"&&<div style={{background:C.accentGlow,border:`1px solid ${C.accent}33`,borderRadius:10,padding:"10px 14px",fontSize:12,color:C.muted,marginBottom:14}}>🤖 Mode IA — seuls les types compatibles sont disponibles.</div>}
              <div style={{display:"grid",gridTemplateColumns:"repeat(2,1fr)",gap:10}}>
                {STUDY_TYPES.filter(t=>ns.mode!=="ai"||["video","survey"].includes(t.id)).map(t=>(
          ...
          <div style={{display:"flex",justifyContent:"space-between",marginTop:24,paddingTop:16,borderTop:`1px solid ${C.border}`}}>
            ...
            {nsStep<6?<Btn onClick={()=>{setNsErr("");setNsStep(nsStep+1);}} disabled={(nsStep===0&&(!ns.title||!ns.theme))||(nsStep===1&&!ns.dur)||(nsStep===2&&(!ns.mode||(ns.mode==="link"&&!ns.link)))||(nsStep===3&&!ns.studyType)||(nsStep===4&&ns.mode==="ai"&&!(ns.ai_focus||"").trim())||(nsStep===5&&(ns.prescreening||[]).some(q=>!q.question||(q.acceptedAnswers||[]).length===0))}>Continuer →</Btn>:<Btn onClick={publishStudy}>🚀 Publier l'étude</Btn>}
          </div>
        </Modal>
      )}
      ...
      {showStudyDetail&&(
        <Modal onClose={()=>setShowStudyDetail(null)} title={showStudyDetail.title} wide>
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:14,marginBottom:20}}>
            {[{l:"Participants",v:`${showStudyDetail.joined} / ${showStudyDetail.target}`,c:C.accentLight},{l:"Budget dépensé",v:showStudyDetail.budget+"€",c:C.yellow},{l:"Statut",v:showStudyDetail.status==="active"?"Active":showStudyDetail.status==="closed"?"Fermée":"Terminée",c:showStudyDetail.status==="active"?C.green:C.muted}].map(s=>(
            ...
            return(
              <div style={{marginBottom:14}}>
                ...
                <div style={{fontSize:11,color:allValidated?C.green:C.muted,marginTop:4,fontWeight:allValidated?700:400}}>
                  {validatedCount}/{target} validés{allValidated?" — synthèse disponible, vous pouvez fermer l'étude quand vous le souhaitez":""}
                </div>
              </div>
            );
          })()}
...
function ParticipantDashboard({onLogout}){
  ...
  useMobileBack({tab,setTab,homeTab:"overview",overlays:[
    ...
    {active:!!resumeParticipation,close:()=>setResumeParticipation(null)},
    {active:!!showStudyDetail,close:()=>setShowStudyDetail(null)},
        ...
        if(Array.isArray(data)&&data.length>0){
          ...
          const mapped=data
            ...
            .map(s=>({
              ...
              pay:s.cost_per_participant?Math.round(s.cost_per_participant*0.9):27,
          ...
          if(researcherIds.length>0){
            try{
              ...
              if(Array.isArray(rData)){
                ...
                setStudies(enriched);
              }else{setStudies(mapped);}
            }catch(e){console.error("Load researcher profiles error:",e);setStudies(mapped);}
          }else{setStudies(mapped);}
      ...
      try{
        ...
        setStudies(prev=>{
          ...
          const updated=prev.map(s=>{
            ...
            return found?{...s,status:found.status,participationId:found.participationId}:s;
          });
        ...
        const pendingParts=parts.filter(p=>p.status==="pending_validation"&&!p.paid);
        setEarnings(paidParts.reduce((a,p)=>{const s=studyMap[p.study_id];return a+(s?Math.round(s.cost_per_participant*0.9):0);},0));
        setPending(pendingParts.reduce((a,p)=>{const s=studyMap[p.study_id];return a+(s?Math.round(s.cost_per_participant*0.9):0);},0));
      }catch(e){console.error("Load my participations error:",e);}
    };
  ...
  const joinStudy=async(id,answers)=>{
    ...
    // 📧 Email de confirmation de participation (une seule fois, à la première inscription)
    if(participationId){notifyEmail("participation_confirmed",{
      email:profile.email,
      first_name:profile.first,
      study_title:s.title,
        ...
        <main className="p-main" style={{flex:1,padding:"20px 16px",overflowY:"auto",background:C.bg,width:"100%",minWidth:0,boxSizing:"border-box",overflowX:"hidden"}}>
                ...
                return(
                  <div onClick={()=>setTab("settings")} style={{marginBottom:20,padding:"14px 16px",borderRadius:14,border:`1px solid ${color}44`,background:color+"11",cursor:"pointer"}}>
                    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8}}>
                      ...
                      <span style={{fontSize:12,color,fontWeight:700}}>Compléter →</span>
                    </div>
                  ...
                  {studies.filter(s=>s.status===st).length===0?<p style={{color:C.dimmed,fontSize:14}}>Aucune.</p>:studies.filter(s=>s.status===st).map(s=>(
                    <Card key={s.id} style={{padding:"18px 22px",marginBottom:10,display:"flex",justifyContent:"space-between",alignItems:"center",border:st==="pending_validation"?`1px solid ${C.yellow}44`:st==="abandoned"?`1px solid ${C.red}44`:undefined}}>
                      ...
                      <div style={{display:"flex",gap:12,alignItems:"center"}}>
                        ...
                        {st==="joined"&&<Btn small onClick={()=>setShowDoneModal(s)} style={{background:C.yellow+"22",border:`1px solid ${C.yellow}44`,color:C.yellow}}>✓ J'ai terminé</Btn>}
          ...
          {tab==="earnings"&&(
            <div style={{maxWidth:580}}>
              ...
              <Card style={{overflow:"hidden"}}>
                {studies.filter(s=>s.status==="completed"||s.status==="pending_validation").length===0?(
                  <div style={{padding:"28px",textAlign:"center",color:C.muted,fontSize:14}}>Aucun paiement pour l'instant.</div>
                ):studies.filter(s=>s.status==="completed"||s.status==="pending_validation").map((s,i)=>(
    ...
    const loadAdmin=async()=>{
      ...
      try{
        ...
        if(Array.isArray(uData)){
          ...
          // partageant un même PayPal, ou un même nom + date de naissance.
          ...
          const dupes=[
            ...clusterBy(u=>{const v=(u.paypal_email||"").trim().toLowerCase();return v||null;},"PayPal"),
          ...
          if(Array.isArray(pData)&&Array.isArray(sData)){
            ...
            pData.forEach(p=>{
              const s=studyMap[p.study_id];
              if(!s)return;
              const fee=Math.round((s.cost_per_participant||0)*0.10*100)/100;
          ...
          {tab==="users"&&(
            <div>
              ...
              <Card style={{overflow:"hidden"}}>
                ...
                {loadingAdmin?<div style={{padding:32,textAlign:"center",color:C.muted}}>Chargement…</div>:adminUsers.length===0?<div style={{padding:32,textAlign:"center",color:C.muted}}>Aucun utilisateur.</div>:adminUsers.slice(usersPage*USERS_PER_PAGE,usersPage*USERS_PER_PAGE+USERS_PER_PAGE).map(u=>(
                ...
                <Pager page={usersPage} setPage={setUsersPage} total={adminUsers.length} pageSize={USERS_PER_PAGE}/>
              </Card>
            </div>
          )}
          {tab==="transactions"&&(
            <div>
              ...
              <Card style={{overflow:"hidden"}}>
                ...
                {loadingAdmin?<div style={{padding:32,textAlign:"center",color:C.muted}}>Chargement…</div>:adminTransactions.length===0?<div style={{padding:32,textAlign:"center",color:C.muted}}>Aucune transaction.</div>:adminTransactions.slice(txPageA*TX_PER_PAGE,txPageA*TX_PER_PAGE+TX_PER_PAGE).map((t,i)=>(
                ...
                <Pager page={txPageA} setPage={setTxPageA} total={adminTransactions.length} pageSize={TX_PER_PAGE}/>
              </Card>
            </div>
          )}
          {tab==="multicomptes"&&(
            <div>
              ...
              <p style={{color:C.muted,fontSize:13,marginBottom:20}}>Comptes partageant un même PayPal, ou un même nom + date de naissance. À vérifier manuellement avant toute sanction — un PayPal partagé peut aussi être une famille.</p>
              {loadingAdmin?(
                <div style={{padding:32,textAlign:"center",color:C.muted}}>Chargement…</div>
              ):adminDupes.length===0?(
                <Card style={{padding:"32px 20px",textAlign:"center",color:C.muted}}>Aucun doublon détecté. ✅</Card>
              ):adminDupes.map((g,gi)=>(
...
// Pastille de score colorée (vert ≥70, jaune ≥40, rouge sinon). Réutilisée
// pour le match (profil) et la qualité des réponses.
function ScoreBadge({score,label,title}){
  if(typeof score!=="number")return null;
  const col=score>=70?C.green:score>=40?C.yellow:C.red;
  return <span title={title||""} style={{fontSize:10,fontWeight:800,padding:"2px 7px",borderRadius:10,background:col+"22",color:col,border:`1px solid ${col}44`,cursor:title?"help":"default"}}>{score}% {label}</span>;
}
...
export default function App(){
  ...
  return(
    ...
    <div style={{background:C.bg,minHeight:"100vh",color:C.text,fontFamily:FONT}}>
      ...
      {view==="researcher"&&<ResearcherDashboard onLogout={logout}/>}
      {view==="participant"&&<ParticipantDashboard onLogout={logout}/>}
