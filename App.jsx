...
function AiInterviewChat({study,profile,matchScore,onComplete,onClose,initialMessages,participationId}){
  ...
  const toggleVideoRecording=()=>{
    ...
    try{
      ...
      const MIME_TYPES=["video/webm;codecs=vp9","video/webm;codecs=vp8","video/webm","video/mp4;codecs=h264","video/mp4",""];
      ...
      {finished?(
        ...
        <div>
          ...
          <div style={{display:"flex",gap:8}}>
            ...
            <Btn onClick={()=>{if(recording){if(recognitionRef.current)recognitionRef.current._shouldStop=true;try{recognitionRef.current?.stop();}catch(e){}setRecording(false);stopMicMeter();}send();}} disabled={loading||(!input.trim()&&!recording)} style={{alignSelf:"flex-end"}}>Envoyer</Btn>
          </div>
        </div>
      )}
    </Modal>
...
function ResearcherDashboard({onLogout}){
  ...
  const confirmCloseStudy=async()=>{
    ...
    if((s.ai||s.linkAi)&&s.joined>0&&!s.global_synthesis){
      (async()=>{
        try{
...
Sois synthétique, factuel et orienté insights.`;
  ...
  const sideItems=[
    ...
    {id:"recaps",icon:"📊",label:"Récapitulatifs études"},
    ...
    <div style={{minHeight:"100vh",display:"flex",flexDirection:"column",fontFamily:FONT}}>
      ...
      <div style={{display:"block",flex:1,position:"relative",overflow:"hidden"}}>
        ...
        <main className="p-main" style={{flex:1,padding:"20px 16px",overflowY:"auto",background:C.bg,width:"100%",minWidth:0,boxSizing:"border-box",overflowX:"hidden",marginLeft:sideOpen?210:0,transition:"margin-left .25s ease"}}>
          ...
          {tab==="validations"&&(()=>{
            ...
            return(
              <div style={{maxWidth:860}}>
                ...
                <p style={{color:C.muted,fontSize:14,marginBottom:20}}>Examinez chaque participant individuellement et validez ou refusez sa participation.</p>
          ...
          {tab==="recaps"&&(
            <div style={{maxWidth:860}}>
              <h1 style={{fontSize:22,fontWeight:800,marginBottom:4}}>Récapitulatifs études</h1>
              <p style={{color:C.muted,fontSize:14,marginBottom:24}}>Vue d'ensemble et synthèse collective de chacune de vos études.</p>
              {studies.length===0?(
                <Card style={{padding:"40px 20px",textAlign:"center",color:C.muted}}>
                  <div style={{fontSize:32,marginBottom:10}}>📊</div>
                  Vous n'avez pas encore créé d'étude.
                </Card>
              ):studies.map(study=>{
                const studyTxs=transactions.filter(t=>t.studyId===study.id&&t.type==="payout");
                const paidTxs=studyTxs.filter(t=>t.paid);
                ...
                const spent=paidTxs.length*(study.budget||0);
                ...
                return(
                  <Card key={study.id} style={{padding:"20px 22px",marginBottom:16,border:study.global_synthesis?`1px solid #a855f733`:undefined}}>
                    ...
                    <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(110px,1fr))",gap:10,marginBottom:16}}>
                      <div style={{background:C.surfaceHigh,borderRadius:10,padding:"10px 12px"}}>
                        <div style={{fontSize:18,fontWeight:900,color:C.accentLight}}>{studyTxs.length}</div>
                        <div style={{fontSize:11,color:C.muted}}>Participants</div>
                      </div>
                      <div style={{background:C.surfaceHigh,borderRadius:10,padding:"10px 12px"}}>
                        <div style={{fontSize:18,fontWeight:900,color:C.green}}>{paidTxs.length}</div>
                        <div style={{fontSize:11,color:C.muted}}>Validés</div>
                      </div>
                      <div style={{background:C.surfaceHigh,borderRadius:10,padding:"10px 12px"}}>
                        <div style={{fontSize:18,fontWeight:900,color:C.yellow}}>{spent.toFixed(0)}€</div>
                        <div style={{fontSize:11,color:C.muted}}>Dépensé</div>
                      </div>
                      {avgMatch!==null&&(
                        <div style={{background:C.surfaceHigh,borderRadius:10,padding:"10px 12px"}}>
                          <div style={{fontSize:18,fontWeight:900,color:C.accentLight}}>{avgMatch}%</div>
                          <div style={{fontSize:11,color:C.muted}}>Match moyen</div>
                        </div>
                      )}
                    </div>
      ...
      {showStudyDetail&&(
        <Modal onClose={()=>setShowStudyDetail(null)} title={showStudyDetail.title} wide>
          ...
          {(()=>{
            const studyTxs=transactions.filter(t=>t.studyId===showStudyDetail.id);
            if(studyTxs.length===0)return null;
            return(
              <div style={{marginTop:20}}>
                <div style={{fontWeight:800,fontSize:14,marginBottom:12,display:"flex",alignItems:"center",gap:8}}>
                  👥 Participants
                  <span style={{background:C.accentGlow,color:C.accentLight,fontSize:11,fontWeight:700,padding:"2px 8px",borderRadius:10}}>{studyTxs.length}</span>
                </div>
                <div style={{display:"flex",flexDirection:"column",gap:10}}>
                  {studyTxs.map((t,i)=>{
                    const isPending=t.status_raw==="pending_validation"&&!t.paid;
                    ...
                    return(
                      <Card key={i} style={{padding:"14px 16px",border:`1px solid ${statusColor}33`}}>
                        <div style={{display:"flex",alignItems:"center",gap:10,flexWrap:"wrap"}}>
                          <Avatar initials={(t.participantName||"?").split(" ").map(n=>n[0]).join("").toUpperCase().slice(0,2)} size={36}/>
                          <div style={{flex:1,minWidth:100}}>
                            <div style={{display:"flex",alignItems:"center",gap:8,flexWrap:"wrap",marginBottom:4}}>
                              <span style={{fontWeight:700,fontSize:13}}>{t.participantName||"Participant"}</span>
                              {typeof t.matchScore==="number"&&(
                                <span style={{fontSize:10,fontWeight:800,padding:"2px 7px",borderRadius:10,background:t.matchScore>=70?C.green+"22":t.matchScore>=40?C.yellow+"22":C.red+"22",color:t.matchScore>=70?C.green:t.matchScore>=40?C.yellow:C.red,border:`1px solid ${t.matchScore>=70?C.green:t.matchScore>=40?C.yellow:C.red}44`}}>{t.matchScore}% match</span>
                              )}
                              <Tag color={statusColor} style={{fontSize:10}}>{statusLabel}</Tag>
                            ...
                            <div style={{display:"flex",gap:6,flexWrap:"wrap"}}>
                              {t.participantId&&(
                                <button onClick={()=>{setShowStudyDetail(null);setShowParticipantProfile(t.participantId);}} style={{background:C.accentGlow,border:`1px solid ${C.accent}44`,borderRadius:7,color:C.accentLight,fontSize:11,fontWeight:600,cursor:"pointer",padding:"4px 10px"}}>
                                  👤 Profil
                                </button>
                              )}
                              {t.aiInterview&&(t.aiInterview.report||t.aiInterview.transcript?.length>0)&&(
                                <button onClick={()=>setShowAiReport({...t.aiInterview,participantId:t.participantId,participantName:t.participantName,matchScore:t.matchScore,defaultTab:"rapport"})} style={{background:"#a855f722",border:"1px solid #a855f744",borderRadius:7,color:"#a855f7",fontSize:11,fontWeight:600,cursor:"pointer",padding:"4px 10px"}}>
                                  📋 Rapport individuel
                                </button>
                              )}
                              {t.aiInterview?.video_urls?.length>0&&(
                                <button onClick={()=>setShowAiReport({...t.aiInterview,participantId:t.participantId,participantName:t.participantName,matchScore:t.matchScore,defaultTab:"video"})} style={{background:C.greenGlow,border:`1px solid ${C.green}44`,borderRadius:7,color:C.green,fontSize:11,fontWeight:600,cursor:"pointer",padding:"4px 10px"}}>
                                  🎥 Vidéo
                                </button>
                              )}
                              {t.aiInterview?.transcript?.length>0&&(
                                <button onClick={()=>setShowAiReport({...t.aiInterview,participantId:t.participantId,participantName:t.participantName,matchScore:t.matchScore,defaultTab:"transcript"})} style={{background:C.surfaceHigh,border:`1px solid ${C.border}`,borderRadius:7,color:C.muted,fontSize:11,fontWeight:600,cursor:"pointer",padding:"4px 10px"}}>
                                  💬 Transcript
                                </button>
                              )}
                            </div>
                          ...
                          {isPending&&(
                            <div style={{display:"flex",gap:6,flexShrink:0}}>
                              <Btn small green onClick={()=>validateParticipant({
                                ...
                                participantName:t.participantName,participantFirstName:t.participantFirstName,
                                participantEmail:t.participantEmail,participationId:t.participationId,
                              })} style={{fontSize:11,padding:"5px 12px"}}>✓ Valider</Btn>
                              <Btn small danger onClick={()=>rejectParticipant({
                                studyId:t.studyId,studyTitle:t.studyTitle,participantId:t.participantId,
                                participantName:t.participantName,participantFirstName:t.participantFirstName,
                                participantEmail:t.participantEmail,participationId:t.participationId,
                              })} style={{fontSize:11,padding:"5px 12px"}}>✕ Refuser</Btn>
                            </div>
                          )}
                        </div>
                      </Card>
      ...
      {showAiReport&&(()=>{
        const tabs=[
          {id:"rapport",label:"📋 Rapport",show:!!showAiReport.report},
          {id:"video",label:"🎥 Vidéo",show:!!(showAiReport.video_urls?.length>0)},
          {id:"transcript",label:"💬 Transcript",show:!!(showAiReport.transcript?.length>0)},
        ].filter(t=>t.show);
        const defaultTab=showAiReport.defaultTab||tabs[0]?.id||"rapport";
        const [activeTab,setActiveTab]=React.useState(defaultTab);
        const score=showAiReport.matchScore;
        const scoreColor=score>=70?C.green:score>=40?C.yellow:C.red;
        return(
        <Modal onClose={()=>setShowAiReport(null)} title="Rapport individuel" wide>
          {/* En-tête participant */}
          <div style={{display:"flex",alignItems:"center",gap:12,marginBottom:18,padding:"12px 16px",background:C.surfaceHigh,borderRadius:12,border:`1px solid ${C.border}`}}>
            <Avatar initials={(showAiReport.participantName||"?").split(" ").map(n=>n[0]).join("").toUpperCase().slice(0,2)} size={38}/>
            <div style={{flex:1}}>
              <div style={{display:"flex",alignItems:"center",gap:10,flexWrap:"wrap"}}>
                <span style={{fontWeight:800,fontSize:14,color:C.text,cursor:showAiReport.participantId?"pointer":"default",textDecoration:showAiReport.participantId?"underline":"none"}}
                  onClick={()=>showAiReport.participantId&&setShowParticipantProfile(showAiReport.participantId)}>
                  {showAiReport.participantName||"Participant"}
                </span>
                {typeof score==="number"&&(
              ...
              {showAiReport.participantId&&(
                <button onClick={()=>setShowParticipantProfile(showAiReport.participantId)} style={{marginTop:5,background:"transparent",border:"none",color:C.accent,fontSize:12,cursor:"pointer",padding:0,fontWeight:600}}>
                  👤 Voir profil complet →
                </button>
              )}
            </div>
          ...
          {activeTab==="rapport"&&showAiReport.report&&(
          ...
          {/* Contenu onglet Transcript */}
          {activeTab==="transcript"&&showAiReport.transcript?.length>0&&(
              ...
              {showAiReport.transcript.map((m,i)=>(
                <div key={i} style={{display:"flex",justifyContent:m.role==="user"?"flex-end":"flex-start"}}>
                  <div style={{maxWidth:"80%",background:m.role==="user"?C.accentGlow:C.surfaceHigh,border:`1px solid ${m.role==="user"?C.accent+"55":C.border}`,borderRadius:12,padding:"10px 14px",fontSize:12.5,lineHeight:1.7,color:C.text,whiteSpace:"pre-wrap"}}>
                    ...
                    {m.role==="user"&&<div style={{fontSize:10,fontWeight:700,color:C.accentLight,marginBottom:4}}>👤 {showAiReport.participantName||"Participant"}</div>}
                    {m.content}
                  </div>
                </div>
              ))}
...
function ParticipantDashboard({onLogout}){
  ...
  useEffect(()=>{
    ...
    const interval=setInterval(loadMyParticipations,30000);
    return()=>clearInterval(interval);
  },[userId]);
      ...
      try{
        ...
        if(Array.isArray(uData)){
          ...
          if(Array.isArray(pData)&&Array.isArray(sData)){
            const studyMap={};
            sData.forEach(s=>{studyMap[s.id]=s;});
            const txs=pData.slice(0,50).map(p=>{
              const s=studyMap[p.study_id];
              return{
          ...
          {tab==="users"&&(
            <div>
              ...
              <Card style={{overflow:"hidden"}}>
                ...
                {loadingAdmin?<div style={{padding:32,textAlign:"center",color:C.muted}}>Chargement…</div>:adminUsers.length===0?<div style={{padding:32,textAlign:"center",color:C.muted}}>Aucun utilisateur.</div>:adminUsers.map(u=>(
          ...
          {tab==="transactions"&&(
            <div>
              ...
              <Card style={{overflow:"hidden"}}>
                ...
                {loadingAdmin?<div style={{padding:32,textAlign:"center",color:C.muted}}>Chargement…</div>:adminTransactions.length===0?<div style={{padding:32,textAlign:"center",color:C.muted}}>Aucune transaction.</div>:adminTransactions.map((t,i)=>(
