                      ...
                      return(
                        <div key={"g"+ri}>
                          <div onClick={()=>setExpandedTx(open?null:row.key)} style={{display:"grid",gridTemplateColumns:"2fr 1fr 1fr 1fr",padding:"14px 20px",borderBottom:`1px solid ${C.border}`,alignItems:"center",fontSize:13,cursor:"pointer",transition:"background .15s",background:open?C.accentGlow+"88":"transparent"}}>
                            <div style={{display:"flex",alignItems:"center",gap:8}}>
                              <span style={{fontSize:18}}>{pendingCount>0?"⏳":inprogressCount>0?"🎙️":paidCount>0?"💸":rejectedCount>0?"✕":"⌛"}</span>
                              <div>
                                <div style={{fontWeight:700}}>{g.label}</div>
                                <div style={{fontSize:11,color:C.accent,marginTop:2}}>
                                  {g.items.length} participant(s){pendingCount>0?` · ${pendingCount} en attente`:""}{inprogressCount>0?` · ${inprogressCount} en cours`:""}{paidCount>0?` · ${paidCount} payé${paidCount>1?"s":""}`:""}{rejectedCount>0?` · ${rejectedCount} refusé${rejectedCount>1?"s":""}`:""}{abandonedCount>0?` · ${abandonedCount} expiré${abandonedCount>1?"s":""}`:""} · {open?"▲ Masquer":"▼ Voir détails"}
                                </div>
                              </div>
                            </div>
