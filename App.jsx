          ...
          if(realId&&!ns.ai&&(ns.slots||[]).length){
            try{
              const slotsRes=await fetch(`${SUPA_URL}/rest/v1/slots`,{
                method:"POST",
                headers:{"apikey":SUPA_KEY,"Authorization":`Bearer ${token}`,"Content-Type":"application/json"},
                body:JSON.stringify((ns.slots||[]).map(iso=>({study_id:realId,datetime:iso,taken:false})))
              });
        ...
        <Modal onClose={()=>setShowStudyModal(false)} title={`Nouvelle étude — Étape ${nsStep+1}/6`} wide>
          ...
          {nsStep===2&&(
            <div>
              ...
              {ns.mode==="link"&&(
                <CreationSlotBuilder slots={ns.slots||[]} onChange={s=>setNs({...ns,slots:s})}/>
              )}
              {ns.mode==="ai"&&(
                <>
                  ...
                  {rows.length===0?<p style={{color:C.dimmed,fontSize:14}}>Aucune.</p>:rows.map(s=>{
                    ...
                    <Card key={s.id} style={{padding:"18px 22px",marginBottom:10,display:"flex",justifyContent:"space-between",alignItems:"center",border:st==="pending_validation"?`1px solid ${C.yellow}44`:st==="abandoned"?`1px solid ${C.red}44`:isAutoUnpaid?`1px solid ${C.orange}44`:undefined}}>
                      <div>
                        ...
                        {st==="joined"&&!s.ai&&s.mode!=="IA"&&(
                          <div style={{marginTop:10,width:"100%"}}>
                            <SlotPicker
                              studyId={s.id}
                              participationId={s.participationId||s.id}
                              token={Storage.get("sb_token")}
                              onBooked={(slot)=>console.log("Créneau réservé",slot)}
                            />
                          </div>
                        )}
...
function CreationSlotBuilder({ slots, onChange }){
  ...
  return(
    <div style={{background:C.surfaceHigh,borderRadius:10,padding:14,border:`1px solid ${C.border}`,marginTop:14}}>
      ...
      <div style={{fontSize:11,color:C.muted,marginBottom:12}}>Chaque participant accepté pourra choisir un créneau. Horaires en {tz}. Tu pourras aussi en ajouter plus tard.</div>
...
function SlotPicker({ studyId, participationId, token, onBooked }){
  ...
  React.useEffect(()=>{
    supaSlot(`slots?participation_id=eq.${participationId}&select=*`,"GET",null,token).then(data=>{
      if(Array.isArray(data)&&data.length>0){ setBooked(data[0]); setLoading(false); return; }
      supaSlot(`slots?study_id=eq.${studyId}&taken=eq.false&order=datetime.asc&select=*`,"GET",null,token)
  ...
  async function bookSlot(slot){
    setBooking(slot.id);
    await supaSlot(`slots?id=eq.${slot.id}`,"PATCH",{taken:true,participation_id:participationId},token);
    setBooked(slot);
    setBooking(null);
    if(onBooked) onBooked(slot);
  }
    ...
    <div style={{background:C.surface,borderRadius:10,padding:16,border:`1px solid ${C.border}`,fontFamily:FONT}}>
      ...
      <div style={{display:"flex",flexDirection:"column",gap:8}}>
        {slots.map(slot=>{
          ...
          return(
            <div key={slot.id} style={{display:"flex",alignItems:"center",justifyContent:"space-between",background:C.surfaceHigh,borderRadius:8,padding:"12px 14px",border:`1px solid ${C.border}`}}>
              <div>
              ...
              <button onClick={()=>bookSlot(slot)} disabled={!!booking} style={{background:isBooking?C.accentGlow:C.accent,color:C.white,border:"none",borderRadius:6,padding:"8px 16px",fontSize:12,fontWeight:700,cursor:"pointer",opacity:booking&&!isBooking?.4:1,minWidth:80,fontFamily:FONT}}>
                {isBooking?"…":"Choisir"}
              </button>
            </div>
          );
