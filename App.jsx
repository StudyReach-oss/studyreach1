      ...
      if(res.ok&&data.success){
        ...
        if(token&&participation.participationId){
          fetch(`${SUPA_URL}/rest/v1/participations?id=eq.${participation.participationId}`,{
            method:"PATCH",
            headers:{"apikey":SUPA_KEY,"Authorization":`Bearer ${token}`,"Content-Type":"application/json"},
            body:JSON.stringify({paid:true,status:"completed",paid_to_bank:true,paid_at:new Date().toISOString()})
          }).catch(e=>console.error(e));
        }
        ...
        alert(`✅ Paiement envoyé ! ${data.participantAmount}€ versé au participant.`);
      }else if(data.needsOnboarding){
        // Le participant n'a pas encore configuré son compte Stripe : aucun
        // virement n'a eu lieu. On marque malgré tout la participation comme
        // validée et DUE (paid=true, paid_to_bank=false) → la somme apparaît
        // dans son solde retirable et il sera payé à son prochain retrait,
        // une fois l'onboarding fait. Pas de double-paiement : paid_to_bank
        // ne passera à true qu'au moment du virement effectif.
        if(token&&participation.participationId){
          fetch(`${SUPA_URL}/rest/v1/participations?id=eq.${participation.participationId}`,{
            method:"PATCH",
            headers:{"apikey":SUPA_KEY,"Authorization":`Bearer ${token}`,"Content-Type":"application/json"},
        ...
        alert("✅ Participant validé. Il n'a pas encore configuré son compte de paiement Stripe : la somme est créditée sur son solde et lui sera versée à son prochain retrait.");
      }else{
        const detail=data.details||data.error||data.message||raw||("HTTP "+res.status);
        console.error("Payout failed:",res.status,data);
        alert("Erreur versement Stripe.\n\nDétail : "+detail);
      }
    }catch(e){
  ...
  const pages={
    "how-it-works":{
      ...
      sections:[
        ...
        {icon:"💸",title:"3. Menez l’entretien et payez",body:"Une fois l’entretien terminé, le paiement est automatiquement versé au participant par virement bancaire sous 24-48h. Vous recevez un rapport si vous avez choisi le mode IA, ou gérez les résultats vous-même avec votre propre lien."},
    ...
    "for-participants":{
      ...
      sections:[
        {icon:"💸",title:"Soyez rémunéré entre 20€ et 40€",body:"Chaque entretien auquel vous participez est rémunéré entre 18€ et 36€ net selon la durée. Le paiement est automatiquement versé sur votre compte bancaire sous 24-48h après validation."},
    ...
    "faq":{
      ...
      sections:[
        {icon:"💰",title:"Comment sont calculés les paiements ?",body:"Les chercheurs paient un tarif fixe par participant (20€, 30€ ou 40€ selon la durée). StudyReach prélève 10% de frais de service. Le participant reçoit donc 90% du montant par virement bancaire sous 24-48h après validation de l’entretien."},
...
function OnboardingModal({role,onClose,onStart}){
  ...
  const steps=isPart?[
    ...
    {icon:"💸",title:"Soyez payé",body:"Le montant affiché pour chaque étude est exactement ce que vous touchez, versé par virement bancaire sous 24-48h après validation. Retrait possible dès 5€ de gains."},
  ]:[
