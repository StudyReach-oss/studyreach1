...
function AuthPage({type,onDone,onNav}){
  ...
  const SUPA_KEY="VOTRE_CLE_ANON_SUPABASE"; // ⚠️ Remplacez par votre clé anon depuis Supabase → Settings → API
    ...
    const loadResearcherProfile=async()=>{
      ...
      try{
        const userRes=await fetch(`https://bwaoxwfkqqpqvtpynwzh.supabase.co/auth/v1/user`,{
          headers:{"apikey":"VOTRE_CLE_ANON_SUPABASE","Authorization":`Bearer ${token}`}
        });
  ...
  const publishStudy=()=>{
    ...
    if(token&&researcherId){
      fetch(`https://bwaoxwfkqqpqvtpynwzh.supabase.co/rest/v1/etudes`,{
        method:"POST",
        headers:{"apikey":"VOTRE_CLE_ANON_SUPABASE","Authorization":`Bearer ${token}`,"Content-Type":"application/json","Prefer":"return=representation"},
      ...
      if(refreshToken){
        try{
          const res=await fetch("https://bwaoxwfkqqpqvtpynwzh.supabase.co/auth/v1/token?grant_type=refresh_token",{
            method:"POST",
            headers:{"apikey":"VOTRE_CLE_ANON_SUPABASE","Content-Type":"application/json"},
