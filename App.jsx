    ...
    try{
      ...
      if(data.success){
        ...
        if(token&&participation.participationId){
          fetch(`https://bwaoxwfkqqpqvtpynwzh.supabase.co/rest/v1/participations?id=eq.${participation.participationId}`,{
            method:"PATCH",
            headers:{"apikey":"eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImJ3YW94d2ZrcXFwcXZ0cHlud3poIiwicm9sZSI6ImFub24iLCJpYXQiOjE3ODA2OTEzMDEsImV4cCI6MjA5NjI2NzMwMX0.utS5lj2nI-Bs0efelpaj9LHT3B_MSib5Ro8ESIz1-q8","Authorization":`Bearer ${token}`,"Content-Type":"application/json"},
            body:JSON.stringify({paid:true,paid_at:new Date().toISOString(),paypal_batch_id:data.batchId})
          }).catch(e=>console.error(e));
        }
            ...
            <div>
              ...
              {(ns.prescreening||[]).map((q,i)=>(
                <div key={i} style={{background:C.surface,border:`1px solid ${C.border}`,borderRadius:12,padding:"14px 16px",marginBottom:10}}>
                  ...
                  <div style={{display:"flex",gap:8}}>
                    {["Oui / Non","Texte libre","Choix multiple"].map(t=>(
...
function ParticipantDashboard({onLogout}){
  ...
  const joinStudy=async(id,answers)=>{
    ...
    for(let i=0;i<questions.length;i++){
      const q=questions[i];
      if(q.type==="Texte libre")continue; // pas de filtrage auto
      const accepted=q.acceptedAnswers||[];
  ...
  const completeAiInterview=async(study,transcript,report,videoUrls)=>{
    setStudies(prev=>prev.map(x=>x.id===study.id?{...x,status:"pending_validation"}:x));
    setPending(p=>p+study.pay);
    setNotifs(prev=>[{id:Date.now(),read:false,type:"complete",text:`Entretien IA "${study.title}" terminé. En attente de validation du chercheur.`,time:"maintenant"},...prev]);
    const token=Storage.get("sb_token");
    if(token){
      try{
            ...
            <div style={{marginBottom:20}}>
              ...
              {showStudyDetail.prescreening.map((q,i)=>(
                <div key={i} style={{background:C.surface,border:`1px solid ${C.border}`,borderRadius:10,padding:"12px 14px",marginBottom:8}}>
                  ...
                  {q.type==="Texte libre"&&(
                    <textarea value={(showStudyDetail.answers||[])[i]||""} onChange={e=>{const a=[...(showStudyDetail.answers||[])];a[i]=e.target.value;setShowStudyDetail({...showStudyDetail,answers:a});}} placeholder="Votre réponse…" style={{width:"100%",background:C.bg,border:`1px solid ${C.border}`,borderRadius:8,padding:"8px 12px",color:C.text,fontSize:13,outline:"none",resize:"vertical",minHeight:60,boxSizing:"border-box"}}/>
                  )}
                  {q.type==="Choix multiple"&&(
                    <div style={{display:"flex",gap:6,flexWrap:"wrap"}}>
                      {(q.options||"").split(",").map(v=>v.trim()).filter(Boolean).map(v=>(
