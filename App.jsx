          ...
          {isLogin&&<div style={{textAlign:"center",marginTop:10}}><span style={{fontSize:13,color:accent,cursor:"pointer"}} onClick={async()=>{
            ...
            try{
              const res=await fetch(`${SUPA_URL}/functions/v1/send-password-reset`,{
                method:"POST",
                headers:{"apikey":SUPA_KEY,"Content-Type":"application/json"},
                body:JSON.stringify({email:f.email,redirect_to:window.location.origin+"/"})
              });
              if(!res.ok){setErr("Envoi impossible pour le moment. Réessayez dans quelques minutes.");return;}
              setErr("");
