...
function ResearcherDashboard({onLogout}){
  ...
  const [ns,setNs]=useState({title:"",theme:"",dur:"",mode:"",link:"",ai:false,linkAi:false,studyType:"",description:""});
  ...
  const publishStudy=()=>{
    ...
    const newStudy={id:Date.now(),title:ns.title||"Nouvelle étude",theme:`${t?.i} ${t?.l}`,dur:d?.l,mode:ns.ai?"IA":"Lien",link:ns.link,target:10,joined:0,budget:studyCost,status:"active",linkAi:ns.linkAi||false,studyType:ns.studyType||"",created:new Date().toLocaleDateString("fr-FR",{day:"2-digit",month:"short",year:"numeric"})};
    ...
    if(token&&researcherId){
      fetch(`https://bwaoxwfkqqpqvtpynwzh.supabase.co/rest/v1/etudes`,{
        ...
        body:JSON.stringify({researcher_id:researcherId,title:ns.title,theme:THEMES.find(x=>x.id===ns.theme)?.l||ns.theme,duration:d?.l,mode:ns.ai?"IA":"Lien",link:ns.link||null,ai:ns.ai,link_ai:ns.linkAi||false,study_type:ns.studyType||"",budget:studyCost,status:"active"})
      }).catch(e=>console.error("Supabase save study error:",e));
    }
    setShowStudyModal(false);setNsStep(0);setNs({title:"",theme:"",dur:"",mode:"",link:"",ai:false,linkAi:false,studyType:"",description:""});
  };
  ...
  return(
    <div style={{minHeight:"100vh",display:"flex",flexDirection:"column",fontFamily:FONT}}>
                              ...
                              {g.items.map((t,ii)=>(
                                <div key={ii} style={{display:"grid",gridTemplateColumns:"2fr 1fr 1fr 1fr",padding:"10px 20px 10px 48px",borderBottom:ii<g.items.length-1?`1px solid ${C.border}`:"none",alignItems:"center",fontSize:12,gap:8}}>
                                  <div style={{display:"flex",alignItems:"center",gap:6}}>
                                    ...
                                    <span style={{color:C.text}}>{t.participantName||"Participant"}</span>
                                  </div>
      ...
      {/* MODAL: New Study */}
      {showStudyModal&&(
        <Modal onClose={()=>setShowStudyModal(false)} title={`Nouvelle étude — Étape ${nsStep+1}/5`} wide>
          {nsStep===0&&(
            <div>
          ...
          <div style={{display:"flex",justifyContent:"space-between",marginTop:24,paddingTop:16,borderTop:`1px solid ${C.border}`}}>
            ...
            {nsStep<4?<Btn onClick={()=>{setNsErr("");setNsStep(nsStep+1);}} disabled={(nsStep===0&&(!ns.title||!ns.theme))||(nsStep===1&&!ns.dur)||(nsStep===2&&!ns.studyType)||(nsStep===3&&!ns.mode)}>Continuer →</Btn>:<Btn onClick={publishStudy}>🚀 Publier l'étude</Btn>}
          </div>
        </Modal>
      )}
...
function ParticipantDashboard({onLogout}){
  ...
  const avail=studies.filter(s=>s.status==="available"&&(filterTheme?s.theme.includes(filterTheme):true)&&(filterDur?s.dur===filterDur:true)&&(filterType?s.studyType===filterType:true)).sort((a,b)=>sortBy==="recent"?((b.created||"")>(a.created||"")?1:-1):0);
