...
const Modal=({children,onClose,title,wide})=>(
  <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,.75)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:200,padding:16}} onClick={e=>{if(e.target===e.currentTarget)onClose();}}>
    <div className="modal-inner" style={{background:C.surface,border:`1px solid ${C.border}`,borderRadius:20,padding:"28px 28px",width:"100%",maxWidth:wide?680:480,maxHeight:"92vh",overflowY:"auto",position:"relative"}}>
...
function Landing({onNav}){
  ...
  return(
    <div style={{minHeight:"100vh",display:"flex",flexDirection:"column"}}>
      ...
      {/* Hero */}
      <section className="landing-hero" style={{textAlign:"center",padding:"80px 24px 60px",position:"relative",overflow:"hidden"}}>
      ...
      {/* For participants */}
      <section className="landing-section-wrap" style={{padding:"60px 56px",borderTop:`1px solid ${C.border}`,background:C.surface}}>
        <div className="landing-section" style={{maxWidth:900,margin:"0 auto",display:"grid",gridTemplateColumns:"1fr 1fr",gap:48,alignItems:"center"}}>
          <div>
      ...
      {/* CTA */}
      <section className="landing-cta" style={{padding:"60px 56px",borderTop:`1px solid ${C.border}`,textAlign:"center"}}>
      ...
      <footer className="landing-footer" style={{borderTop:`1px solid ${C.border}`,padding:"32px 56px",display:"flex",justifyContent:"space-between",flexWrap:"wrap",gap:20}}>
        ...
        <div className="landing-footer-links" style={{display:"flex",gap:48,flexWrap:"wrap"}}>
          {[
...
function ResearcherDashboard({onLogout}){
    ...
    const loadResearcherProfile=async()=>{
      ...
      try{
        const userRes=await fetch(`https://bwaoxwfkqqpqvtpynwzh.supabase.co/auth/v1/user`,{
          headers:{"apikey":"sb_publishable_SsnkELg6dLx--AjHaW0ShA_N1ISmMKg","Authorization":`Bearer ${token}`}
        });
  ...
  const publishStudy=()=>{
    ...
    if(token&&researcherId){
      fetch(`https://bwaoxwfkqqpqvtpynwzh.supabase.co/rest/v1/études`,{
        method:"POST",
        headers:{"apikey":"sb_publishable_SsnkELg6dLx--AjHaW0ShA_N1ISmMKg","Authorization":`Bearer ${token}`,"Content-Type":"application/json","Prefer":"return=representation"},
  ...
  return(
    <div style={{minHeight:"100vh",display:"flex",flexDirection:"column",fontFamily:FONT}}>
      {/* Topbar */}
      <header className="dash-header" style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"12px 28px",borderBottom:`1px solid ${C.border}`,background:C.surface,position:"sticky",top:0,zIndex:40}}>
      ...
      <div style={{display:"block",flex:1,position:"relative",overflow:"hidden"}}>
        ...
        {/* Content */}
        <main className="dash-main dash-main-with-nav" style={{flex:1,padding:"20px 16px",overflowY:"auto",background:C.bg,width:"100%",minWidth:0,boxSizing:"border-box",overflowX:"hidden"}}>
          ...
          {tab==="settings"&&(
            <div style={{maxWidth:560}}>
              ...
              <Card style={{padding:24,marginBottom:16}}>
                ...
                <div className="settings-grid" style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}><Inp label="Prénom" defaultValue=""/><Inp label="Nom" defaultValue=""/></div>
...
function StudyCard({s,full,onClick}){
  return(
    <Card style={{padding:"18px 22px",display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:full?0:8,cursor:"pointer",flexWrap:"wrap",gap:10}} onClick={onClick}>
      <div style={{flex:1}}>
...
function ParticipantDashboard({onLogout}){
  ...
  useEffect(()=>{
    const loadStudies=async()=>{
      const token=Storage.get("sb_token");
      try{
        const res=await fetch(`https://bwaoxwfkqqpqvtpynwzh.supabase.co/rest/v1/études?status=eq.active&select=*`,{
          headers:{"apikey":"sb_publishable_SsnkELg6dLx--AjHaW0ShA_N1ISmMKg","Authorization":`Bearer ${token||""}`}
        });
        const data=await res.json();
    ...
    <div style={{minHeight:"100vh",display:"flex",flexDirection:"column",fontFamily:FONT}}>
      <header className="dash-header" style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"12px 28px",borderBottom:`1px solid ${C.border}`,background:C.surface,position:"sticky",top:0,zIndex:40}}>
        ...
        <div style={{display:"flex",alignItems:"center",gap:14}}>
          <div className="dash-gains" style={{display:"flex",alignItems:"center",gap:8,background:C.greenGlow,border:`1px solid ${C.green}44`,padding:"7px 14px",borderRadius:10}}>
          ...
          <div className="dash-name" style={{display:"flex",alignItems:"center",gap:8,cursor:"pointer"}} onClick={()=>setTab("settings")}>
        ...
        <main className="dash-main dash-main-with-nav" style={{flex:1,padding:"20px 16px",overflowY:"auto",background:C.bg,width:"100%",minWidth:0,boxSizing:"border-box",overflowX:"hidden"}}>
          ...
          {tab==="earnings"&&(
            <div style={{maxWidth:580}}>
              ...
              <div className="earnings-grid" style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:14,marginBottom:24}}>
          ...
          {tab==="settings"&&(
            <div style={{maxWidth:560}}>
              ...
              <Card style={{padding:24,marginBottom:16}}>
                <h3 style={{fontWeight:700,marginBottom:16,fontSize:15}}>Informations personnelles</h3>
                <div className="settings-grid" style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}><Inp label="Prénom" value={profile.first} onChange={e=>setProfile({...profile,first:e.target.value})}/><Inp label="Nom" value={profile.last} onChange={e=>setProfile({...profile,last:e.target.value})}/></div>
      ...
      <nav className="bottom-nav">
        {[
          ...
          <button key={t.id} className="bottom-nav-item" onClick={()=>setTab(t.id)} style={{color:tab===t.id?"#1ec98a":"#606880"}}>
...
export default function App(){
  const _savedRole=Storage.get("sb_role");
  const _savedToken=Storage.get("sb_token");
  const [view,setView]=useState(_savedToken&&_savedRole?_savedRole:"landing");
  const [role,setRole]=useState(_savedToken&&_savedRole?_savedRole:null);

  const nav=(v)=>{
    if(v==="landing"){setRole(null);Storage.remove("sb_token");Storage.remove("sb_role");}
    setView(v);
  };
      ...
      <style>{`
        ...
        *{box-sizing:border-box;margin:0;padding:0;}
        ...
        @media(max-width:600px){
          ...
          .landing-hero h1{font-size:34px !important;letter-spacing:-1px !important;}
          .landing-hero p{font-size:15px !important;}
          .landing-section{padding:36px 16px !important;grid-template-columns:1fr !important;}
          .landing-cta{padding:36px 16px !important;}
          .landing-footer{padding:24px 16px !important;flex-direction:column !important;gap:24px !important;}
          .landing-footer-links{gap:24px !important;}
          .auth-card{padding:24px 16px !important;margin:16px !important;}
          .dash-header{padding:10px 12px !important;flex-wrap:wrap;gap:8px !important;}
          .dash-header-right{gap:8px !important;flex-wrap:wrap;}
          .dash-gains{display:none !important;}
          ...
          .study-card-inner{flex-direction:column !important;gap:10px !important;}
          .study-card-right{text-align:left !important;margin-left:0 !important;}
          .earnings-grid{grid-template-columns:1fr !important;}
          .settings-grid{grid-template-columns:1fr !important;}
          .modal-inner{padding:20px 14px !important;margin:8px !important;}
          .r-header{padding:10px 12px !important;}
          .r-header-right{gap:6px !important;}
          .r-wallet{display:none !important;}
          .r-main{padding:14px 10px !important;}
          .stats-grid{grid-template-columns:1fr 1fr !important;}
          .faq-section{padding:36px 16px !important;}
          .bottom-nav{display:flex !important;}
          .dash-main-with-nav{padding-bottom:70px !important;}
        }
        .bottom-nav{display:none;position:fixed;bottom:0;left:0;right:0;background:#0e1120;border-top:1px solid #1c2035;z-index:50;height:60px;align-items:stretch;}
        .bottom-nav-item{flex:1;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:2px;border:none;background:transparent;cursor:pointer;font-family:inherit;padding:4px 0;}
        .bottom-nav-item span.icon{font-size:18px;}
        .bottom-nav-item span.label{font-size:10px;font-weight:600;letter-spacing:.3px;}
      `}</style>
