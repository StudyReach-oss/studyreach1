...
// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
//  PARTICIPANT PROFILE MODAL (read-only)
// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
function ResearcherDashboard({onLogout}){
  ...
  const [ns,setNs]=useState({title:"",theme:"",dur:"",mode:"",link:"",ai:false,linkAi:false,studyType:"",maxParticipants:10,description:"",prescreening:[],
  target_criteria:{
  ...
  const closeStudy=async(studyId)=>{
    const token=Storage.get("sb_token");
    setStudies(prev=>prev.map(s=>s.id===studyId?{...s,status:"closed"}:s));
    if(token){
      fetch(`${SUPA_URL}/rest/v1/etudes?id=eq.${studyId}`,{
        method:"PATCH",
        headers:{"apikey":SUPA_KEY,"Authorization":`Bearer ${token}`,"Content-Type":"application/json","Prefer":"return=minimal"},
  ...
  const publishStudy=()=>{
    ...
    if(token&&researcherId){
      fetch(`https://bwaoxwfkqqpqvtpynwzh.supabase.co/rest/v1/etudes`,{
        ...
        body:JSON.stringify({researcher_id:researcherId,title:ns.title,theme:THEMES.find(x=>x.id===ns.theme)?.l||ns.theme,duration:d?.l,mode:ns.ai?"IA":"Lien",link:ns.link||null,ai:ns.ai,link_ai:ns.linkAi||false,study_type:ns.studyType||"",budget:studyCost,max_participants:ns.maxParticipants||10,prescreening:ns.prescreening||[],status:"active",target_criteria:ns.target_criteria})
      }).catch(e=>console.error("Supabase save study error:",e));
    }
    setShowStudyModal(false);setNsStep(0);setNs({title:"",theme:"",dur:"",mode:"",link:"",ai:false,linkAi:false,studyType:"",maxParticipants:10,description:"",prescreening:[],target_criteria:{age_min:"",age_max:"",genre:[],country:"",nationality:"",handicap:"",status_pro:[],sector:[],education:[],company_size:[],devices:[],os:[],tech_level:"",social_networks:[],has_participated:"",has_camera:"",languages:[],mobile:"",long_term:"",sport:[],diet:[],smoker:"",alcohol:"",medical_follow:"",chronic_illness:"",income:[],online_purchase_freq:"",has_car:"",subscriptions:[],brand_preference:[],financial_products:"",family_status:[],children_count:"",housing_status:[],housing_type:[],screen_time:[],media_consumption:[],social_frequency:[],creative_hobby:[],themes:[]}});
  };
      ...
      if(data.success){
        ...
        setTransactions(prev=>prev.map(t=>
          t.participationId===participation.participationId
            ?{...t,paid:true,color:C.red,status:"Payé ✓"}
            :t
        ).concat(
          participation.participationId?[]:[{
            ...
            rawDate:new Date().toISOString(),
            amount:`-${totalDeducted.toFixed(2)}€`,
            color:C.red,status:"Payé ✓",paid:true,batchId:data.batchId,
          }]
        ));
          ...
          {tab==="studies"&&(
            <div>
              ...
              <div style={{display:"flex",flexDirection:"column",gap:12}}>
                {studies.map(s=>(<StudyCard key={s.id} s={s} full onClick={()=>setShowStudyDetail(s)} onClose={closeStudy}/>))}
              </div>
            </div>
          )}
                        ...
                        <div key={"g"+ri}>
                          ...
                          {open&&(
                            <div style={{background:C.bg,borderBottom:`1px solid ${C.border}`}}>
                              {g.items.map((t,ii)=>(
                                <div key={ii} style={{display:"grid",gridTemplateColumns:"2fr 1fr 1fr 1fr",padding:"10px 20px 10px 48px",borderBottom:ii<g.items.length-1?`1px solid ${C.border}`:"none",alignItems:"center",fontSize:12,gap:8}}>
                                  <div style={{display:"flex",alignItems:"center",gap:6}}>
                                    <span style={{width:6,height:6,borderRadius:"50%",background:t.paid?C.red:C.yellow,display:"inline-block",flexShrink:0}}/>
                                  ...
                                  {t.paid
                                    ?<Tag color={C.red} style={{fontSize:10}}>Payé ✓</Tag>
                                  ...
                                  <span style={{fontWeight:700,color:t.paid?C.red:C.yellow,textAlign:"right"}}>{t.amount}</span>
                                </div>
                              ))}
                            </div>
                          )}
