  ...
  const submit=async()=>{
    ...
    try{
      if(isLogin){
        ...
        localStorage.setItem("sb_token", data.access_token||"");
        localStorage.setItem("sb_role", isPart?"participant":"researcher");
  ...
  return(
    <div style={{minHeight:"100vh",display:"flex",flexDirection:"column",fontFamily:FONT}}>
      ...
      <div style={{display:"block",flex:1,position:"relative",overflow:"hidden"}}>
        ...
        {/* Sidebar */}
        <nav style={{width:210,borderRight:`1px solid ${C.border}`,padding:"20px 0",display:"flex",flexDirection:"column",gap:2,background:C.surface,position:"fixed",top:0,left:sideOpen?0:-220,height:"100vh",zIndex:40,transition:"left .25s ease",paddingTop:58,overflowY:"auto"}}>
          {sideItems.map(t=>(
    ...
    const loadProfile=async()=>{
      ...
      try{
        ...
        if(user?.id){
          ...
          if(profiles?.[0]){
            const p=profiles[0];
            setProfile({first:p.first_name||"",last:p.last_name||"",email:p.email||"",paypal:p.paypal_email||"",prof:p.profession||"",bio:"",age:"",region:""});
          }
        }
              ...
              <Card style={{padding:24,marginBottom:16}}>
                ...
                <Btn green onClick={async()=>{
                  ...
                  await fetch(`https://bwaoxwfkqqpqvtpynwzh.supabase.co/rest/v1/profiles?id=eq.${userId}`,{
                    method:"PATCH",
                    headers:{"apikey":"sb_publishable_SsnkELg6dLx--AjHaW0ShA_N1ISmMKg","Authorization":`Bearer ${token}`,"Content-Type":"application/json","Prefer":"return=representation"},
                    body:JSON.stringify({first_name:profile.first,last_name:profile.last,email:profile.email,profession:profile.prof})
                  });
...
export default function App(){
  const _savedRole=localStorage.getItem("sb_role");
  const _savedToken=localStorage.getItem("sb_token");
  ...
  const nav=(v)=>{
    if(v==="landing"){setRole(null);localStorage.removeItem("sb_token");localStorage.removeItem("sb_role");}
    setView(v);
  };
