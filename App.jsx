...
function ResearcherDashboard({onLogout}){
  ...
  const confirmCloseStudy=async()=>{
    ...
    // 🤖 Si étude IA, déclencher la synthèse globale
    if((s.ai||s.linkAi)&&s.joined>0){
      (async()=>{
        try{
          const synthRes=await fetch(`${SUPA_URL}/functions/v1/send-email`,{
            method:"POST",
            headers:{"apikey":SUPA_KEY,"Content-Type":"application/json"},
            body:JSON.stringify({type:"generate_synthesis",data:{study_id:s.id,study_title:s.title,researcher_email:researcherProfile.email,researcher_name:researcherProfile.first}})
          });
          ...
          if(synthData?.synthesis){
            await fetch(`${SUPA_URL}/rest/v1/studies?id=eq.${s.id}`,{
              method:"PATCH",
              headers:{"apikey":SUPA_KEY,"Authorization":`Bearer ${token}`,"Content-Type":"application/json","Prefer":"return=minimal"},
              body:JSON.stringify({global_synthesis:synthData.synthesis})
            });
            setStudies(prev=>prev.map(x=>x.id===s.id?{...x,global_synthesis:synthData.synthesis}:x));
          }
