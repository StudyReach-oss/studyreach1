...
function AuthPage({type,onDone,onNav}){
  ...
  const SUPA_KEY="eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImJ3YW94d2ZrcXFwcXZ0cHlud3poIiwicm9sZSI6ImFub24iLCJpYXQiOjE3ODA2OTEzMDEsImV4cCI6MjA5NjI2NzMwMX0.utS5lj2nI-Bs0efelpaj9LHT3B_MSib5Ro8ESIz1-q8"; // ⚠️ Remplacez par votre clé anon depuis Supabase → Settings → API
...
function ResearcherDashboard({onLogout}){
    ...
    const loadResearcherProfile=async()=>{
      ...
      try{
        ...
        if(user?.id){
          ...
          const studiesRes=await fetch(`https://bwaoxwfkqqpqvtpynwzh.supabase.co/rest/v1/etudes?researcher_id=eq.${user.id}&select=*`,{
            headers:{"apikey":"eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImJ3YW94d2ZrcXFwcXZ0cHlud3poIiwicm9sZSI6ImFub24iLCJpYXQiOjE3ODA2OTEzMDEsImV4cCI6MjA5NjI2NzMwMX0.utS5lj2nI-Bs0efelpaj9LHT3B_MSib5Ro8ESIz1-q8","Authorization":`Bearer ${token}`}
          });
          const studiesData=await studiesRes.json();
          if(Array.isArray(studiesData)&&studiesData.length>0){
            setStudies(studiesData.map(s=>({
              id:s.id,title:s.title,theme:s.theme||"",dur:s.duration||"",
              mode:s.mode||"Lien",link:s.link||"",target:s.target||10,
              joined:s.joined||0,budget:s.budget||0,status:s.status||"active",
  ...
  const publishStudy=()=>{
    ...
    if(wallet<studyCost){
      setNsErr(`Solde insuffisant — vous avez ${wallet.toFixed(2)}€ mais il vous faut ${studyCost}€. Rechargez votre portefeuille.`);
      return;
    }
    const newStudy={id:Date.now(),title:ns.title||"Nouvelle étude",theme:`${t?.i} ${t?.l}`,dur:d?.l,mode:ns.ai?"IA":"Lien",link:ns.link,target:10,joined:0,budget:studyCost,status:"active",created:new Date().toLocaleDateString("fr-FR",{day:"2-digit",month:"short",year:"numeric"})};
    setStudies(prev=>[...prev,newStudy]);
    setWallet(prev=>prev-studyCost);
    // Save to Supabase
    const token=Storage.get("sb_token");
    if(token&&researcherId){
      fetch(`https://bwaoxwfkqqpqvtpynwzh.supabase.co/rest/v1/etudes`,{
        method:"POST",
        ...
        body:JSON.stringify({researcher_id:researcherId,title:ns.title,theme:THEMES.find(x=>x.id===ns.theme)?.l||ns.theme,duration:d?.l,mode:ns.ai?"IA":"Lien",link:ns.link||null,ai:ns.ai,budget:studyCost,status:"active"})
  ...
  // Add recharge to transactions when PayPal redirect returns
  useEffect(()=>{
    const params=new URLSearchParams(window.location.search);
    if(params.get("payment")==="success"&&params.get("amount")){
      ...
      setTransactions(prev=>[{
        ...
        date:new Date().toLocaleDateString("fr-FR",{day:"2-digit",month:"short",year:"numeric"}),
        amount:`+${amt}€`,color:C.green,status:"Confirmé ✓"
      },...prev]);
    }
  },[]);
  ...
  const sideItems=[
    ...
    {id:"transactions",icon:"💳",label:"Transactions",badge:transactions.filter(t=>t.type==="payout"&&!t.seen).length},
  ...
  return(
    <div style={{minHeight:"100vh",display:"flex",flexDirection:"column",fontFamily:FONT}}>
        ...
        <main className="p-main" style={{flex:1,padding:"20px 16px",overflowY:"auto",background:C.bg,width:"100%",minWidth:0,boxSizing:"border-box",overflowX:"hidden"}}>
          ...
          {tab==="wallet"&&(
            <div style={{maxWidth:600}}>
              ...
              <Card style={{overflow:"hidden"}}>
                {transactions.length===0?<div style={{padding:"24px",textAlign:"center",color:C.muted,fontSize:14}}>Aucune transaction pour l'instant.</div>:transactions.map((t,i)=>(
                  <div key={t.id||i} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"14px 20px",borderBottom:`1px solid ${C.border}`}}>
                    <div>
                      <div style={{fontSize:14,fontWeight:600}}>{t.label}</div>
                      <div style={{fontSize:12,color:C.muted}}>{t.date}</div>
                    </div>
                    <div style={{display:"flex",gap:10,alignItems:"center"}}>
                      <Tag color={t.color||C.green}>{t.status}</Tag>
                      <span style={{fontWeight:800,color:t.color||C.green,fontSize:15}}>{t.amount}</span>
                    </div>
                  </div>
                ))}
              </Card>
          ...
          {tab==="transactions"&&(
            <div style={{maxWidth:760}}>
              <h1 style={{fontSize:22,fontWeight:800,marginBottom:4}}>Tableau de bord transactions</h1>
              <p style={{color:C.muted,fontSize:14,marginBottom:24}}>Suivi de tous les paiements envoyés aux participants.</p>
              ...
              <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:14,marginBottom:24}}>
                {[
                  ...
                  {l:"Recharges",v:transactions.filter(t=>t.type==="recharge").reduce((a,t)=>a+parseFloat(t.amount.replace(/[^0-9.]/g,"")||0),0).toFixed(2)+"€",c:C.accentLight,i:"⬆️"},
                ].map(s=>(
              ...
              {/* Pending participants to validate */}
              <div style={{marginBottom:24}}>
                <h3 style={{fontWeight:700,fontSize:14,marginBottom:12,display:"flex",alignItems:"center",gap:8}}>
                  ⏳ Participants à valider
                  <span style={{background:C.yellow+"22",color:C.yellow,fontSize:11,fontWeight:700,padding:"2px 8px",borderRadius:10}}>Validation manuelle requise</span>
                </h3>
                {studies.filter(s=>s.status==="active"&&s.joined>0).length===0?(
                  <Card style={{padding:"24px",textAlign:"center"}}>
                    <p style={{color:C.muted,fontSize:14}}>Aucun participant en attente de validation.</p>
                  </Card>
                ):studies.filter(s=>s.status==="active"&&s.joined>0).map(study=>(
                  <Card key={study.id} style={{padding:"18px 22px",marginBottom:10,border:`1px solid ${C.yellow}33`}}>
                    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                      <div>
                        <div style={{fontWeight:700,fontSize:14,marginBottom:4}}>{study.title}</div>
                        <div style={{fontSize:12,color:C.muted}}>{study.joined} participant(s) · {study.dur} · {study.budget}€/participant</div>
                      </div>
                      <Btn small onClick={()=>setShowValidateModal(study)} style={{background:C.yellow,color:"#000"}}>
                        Valider & Payer →
                      </Btn>
                    </div>
                  </Card>
              ...
              {/* All transactions */}
              <h3 style={{fontWeight:700,fontSize:14,marginBottom:12}}>📋 Toutes les transactions</h3>
              <Card style={{overflow:"hidden"}}>
                <div style={{display:"grid",gridTemplateColumns:"2fr 1fr 1fr 1fr",padding:"10px 20px",borderBottom:`1px solid ${C.border}`,fontSize:12,fontWeight:700,color:C.muted,textTransform:"uppercase",letterSpacing:.6}}>
                  <span>Description</span><span>Date</span><span>Statut</span><span style={{textAlign:"right"}}>Montant</span>
                </div>
                {transactions.length===0?<div style={{padding:"28px",textAlign:"center",color:C.muted,fontSize:14}}>Aucune transaction pour l'instant.</div>:transactions.map((t,i)=>(
                  <div key={t.id||i} style={{display:"grid",gridTemplateColumns:"2fr 1fr 1fr 1fr",padding:"14px 20px",borderBottom:`1px solid ${C.border}`,alignItems:"center",fontSize:13}}>
                    <div>
                      <div style={{fontWeight:600}}>{t.label}</div>
                      {t.batchId&&<div style={{fontSize:11,color:C.muted}}>Batch: {t.batchId.slice(0,16)}…</div>}
                    </div>
                    <span style={{color:C.muted}}>{t.date}</span>
                    <Tag color={t.color||C.green}>{t.status}</Tag>
                    <span style={{fontWeight:800,color:t.color||C.green,textAlign:"right",fontSize:15}}>{t.amount}</span>
                  </div>
                ))}
              </Card>
            </div>
      ...
      {showStudyModal&&(
        <Modal onClose={()=>setShowStudyModal(false)} title={`Nouvelle étude — Étape ${nsStep+1}/4`} wide>
          ...
          <div style={{display:"flex",justifyContent:"space-between",marginTop:24,paddingTop:16,borderTop:`1px solid ${C.border}`}}>
            ...
            {nsStep<3?<Btn onClick={()=>{setNsErr("");setNsStep(nsStep+1);}} disabled={(nsStep===0&&(!ns.title||!ns.theme))||(nsStep===1&&!ns.dur)||(nsStep===2&&!ns.mode)}>Continuer →</Btn>:<Btn onClick={publishStudy}>🚀 Publier l'étude</Btn>}
          </div>
        </Modal>
      )}
      ...
      {/* MODAL: Validate & Pay participant */}
      {showValidateModal&&(
...
function ParticipantDashboard({onLogout}){
  ...
  useEffect(()=>{
    const loadStudies=async()=>{
      const token=Storage.get("sb_token");
      try{
        const res=await fetch(`https://bwaoxwfkqqpqvtpynwzh.supabase.co/rest/v1/etudes?status=eq.active&select=*`,{
          headers:{"apikey":"eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImJ3YW94d2ZrcXFwcXZ0cHlud3poIiwicm9sZSI6ImFub24iLCJpYXQiOjE3ODA2OTEzMDEsImV4cCI6MjA5NjI2NzMwMX0.utS5lj2nI-Bs0efelpaj9LHT3B_MSib5Ro8ESIz1-q8","Authorization":`Bearer ${token||""}`}
        });
        const data=await res.json();
        if(Array.isArray(data)&&data.length>0){
          setStudies(data.map(s=>({
            ...
            desc:"Étude qualitative",status:"available"
          })));
        }
  ...
  const [profile,setProfile]=useState({first:"",last:"",email:"",paypal:"",prof:"",bio:"",age:"",region:"",themes:[]});
    ...
    const loadProfile=async()=>{
      ...
      try{
        ...
        if(user?.id){
          ...
          if(profiles?.[0]){
            const p=profiles[0];
            setProfile({first:p.first_name||"",last:p.last_name||"",email:p.email||"",paypal:p.paypal_email||"",prof:p.profession||"",bio:p.bio||"",age:p.age||"",region:p.region||"",themes:p.themes||[]});
          }
        }
        ...
        <main className="p-main" style={{flex:1,padding:"20px 16px",overflowY:"auto",background:C.bg,width:"100%",minWidth:0,boxSizing:"border-box",overflowX:"hidden"}}>
          ...
          {tab==="history"&&(
            <div>
              ...
              {["joined","pending_validation","completed"].map(st=>(
                <div key={st} style={{marginBottom:24}}>
                  <h3 style={{fontSize:14,fontWeight:700,color:C.muted,textTransform:"uppercase",letterSpacing:.8,marginBottom:10}}>
                    {st==="joined"?"En cours":st==="pending_validation"?"⏳ En attente de validation chercheur":"Terminées & Payées"}
                  </h3>
                  {studies.filter(s=>s.status===st).length===0?<p style={{color:C.dimmed,fontSize:14}}>Aucune.</p>:studies.filter(s=>s.status===st).map(s=>(
                    <Card key={s.id} style={{padding:"18px 22px",marginBottom:10,display:"flex",justifyContent:"space-between",alignItems:"center",border:st==="pending_validation"?`1px solid ${C.yellow}44`:undefined}}>
                      ...
                      <div style={{display:"flex",gap:12,alignItems:"center"}}>
                        ...
                        <Tag color={st==="joined"?C.accent:st==="pending_validation"?C.yellow:C.green}>
                          {st==="joined"?"En cours":st==="pending_validation"?"Validation en attente":"Payée ✓"}
                        </Tag>
          ...
          {tab==="settings"&&(
            <div style={{maxWidth:560}}>
              ...
              <Card style={{padding:24,marginBottom:16}}>
                <h3 style={{fontWeight:700,marginBottom:16,fontSize:15}}>Informations personnelles</h3>
                <div className="settings-name-grid" style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}><Inp label="Prénom" value={profile.first} onChange={e=>setProfile({...profile,first:e.target.value})}/><Inp label="Nom" value={profile.last} onChange={e=>setProfile({...profile,last:e.target.value})}/></div>
                ...
                <Sel label="Tranche d'âge" options={["18-24 ans","25-34 ans","35-44 ans","45-54 ans","55-64 ans","65 ans et +"]} value={profile.age} onChange={e=>setProfile({...profile,age:e.target.value})}/>
                <Inp label="Région" placeholder="Ex: Île-de-France" value={profile.region} onChange={e=>setProfile({...profile,region:e.target.value})}/>
                <Btn green onClick={async()=>{
                  const token=Storage.get("sb_token");
                  if(!token||!userId)return;
                  await fetch(`https://bwaoxwfkqqpqvtpynwzh.supabase.co/rest/v1/profiles?id=eq.${userId}`,{
                    method:"PATCH",
                    headers:{"apikey":"eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImJ3YW94d2ZrcXFwcXZ0cHlud3poIiwicm9sZSI6ImFub24iLCJpYXQiOjE3ODA2OTEzMDEsImV4cCI6MjA5NjI2NzMwMX0.utS5lj2nI-Bs0efelpaj9LHT3B_MSib5Ro8ESIz1-q8","Authorization":`Bearer ${token}`,"Content-Type":"application/json","Prefer":"return=representation"},
                    body:JSON.stringify({first_name:profile.first,last_name:profile.last,email:profile.email,profession:profile.prof,age:profile.age,region:profile.region})
                  });
              ...
              <Card style={{padding:24,marginBottom:16}}>
                ...
                <Btn green onClick={async()=>{
                  ...
                  await fetch(`https://bwaoxwfkqqpqvtpynwzh.supabase.co/rest/v1/profiles?id=eq.${userId}`,{
                    method:"PATCH",
                    headers:{"apikey":"eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImJ3YW94d2ZrcXFwcXZ0cHlud3poIiwicm9sZSI6ImFub24iLCJpYXQiOjE3ODA2OTEzMDEsImV4cCI6MjA5NjI2NzMwMX0.utS5lj2nI-Bs0efelpaj9LHT3B_MSib5Ro8ESIz1-q8","Authorization":`Bearer ${token}`,"Content-Type":"application/json","Prefer":"return=representation"},
                    body:JSON.stringify({paypal_email:profile.paypal})
                  });
                  alert("PayPal mis à jour !");
                }}>Mettre à jour PayPal</Btn>
              </Card>
              <Card style={{padding:24,marginBottom:16}}>
                <h3 style={{fontWeight:700,marginBottom:14,fontSize:15}}>Thèmes d'intérêt</h3>
                <p style={{fontSize:12,color:"#606880",marginBottom:12}}>Sélectionnez les thèmes qui vous correspondent pour recevoir des études pertinentes.</p>
                <div style={{display:"flex",gap:8,flexWrap:"wrap"}}>
                  {[{id:"ux",l:"🎨 UX"},{id:"mkt",l:"📣 Marketing"},{id:"tech",l:"🤖 Tech"},{id:"csr",l:"🛒 Conso"},{id:"hlth",l:"🏥 Santé"},{id:"fin",l:"💳 Finance"},{id:"edu",l:"📚 Éducation"},{id:"other",l:"✨ Autre"}].map(t=>(
                    <div key={t.id} onClick={()=>{const themes=profile.themes||[];const has=themes.includes(t.id);setProfile({...profile,themes:has?themes.filter(x=>x!==t.id):[...themes,t.id]});}} style={{background:(profile.themes||[]).includes(t.id)?"#1ec98a22":"#07080e",border:`1.5px solid ${(profile.themes||[]).includes(t.id)?"#1ec98a":"#1c2035"}`,borderRadius:20,padding:"6px 14px",cursor:"pointer",fontSize:13,fontWeight:600,color:(profile.themes||[]).includes(t.id)?"#1ec98a":"#606880",transition:"all .15s"}}>{t.l}</div>
                  ))}
                </div>
                <div style={{marginTop:12}}>
                  <button onClick={async()=>{const token=Storage.get("sb_token");if(!token||!userId)return;await fetch(`https://bwaoxwfkqqpqvtpynwzh.supabase.co/rest/v1/profiles?id=eq.${userId}`,{method:"PATCH",headers:{"apikey":"eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImJ3YW94d2ZrcXFwcXZ0cHlud3poIiwicm9sZSI6ImFub24iLCJpYXQiOjE3ODA2OTEzMDEsImV4cCI6MjA5NjI2NzMwMX0.utS5lj2nI-Bs0efelpaj9LHT3B_MSib5Ro8ESIz1-q8","Authorization":`Bearer ${token}`,"Content-Type":"application/json","Prefer":"return=representation"},body:JSON.stringify({themes:profile.themes||[]})});alert("Thèmes enregistrés !");}} style={{background:"#1ec98a",color:"#fff",border:"none",borderRadius:10,padding:"11px 22px",fontSize:14,fontWeight:700,cursor:"pointer"}}>Enregistrer les thèmes</button>
                </div>
              </Card>
              <Card style={{padding:24}}>
...
function LegalPage({type,onBack}){
  const content={
    ...
    legal:{title:"Mentions Légales",sections:[{t:"Éditeur du site",c:"Le site StudyReach est édité par la société StudyReach SAS, au capital de 1 000 €, immatriculée au RCS de Paris. Siège social : Paris, France. Directeur de la publication : le représentant légal de la société. Contact : contact@studyreach.io"},{t:"Hébergement",c:"Le site est hébergé par Vercel Inc., 340 Pine Street, Suite 1501, San Francisco, CA 94104, États-Unis. La base de données est hébergée par Supabase Inc."},{t:"Propriété intellectuelle",c:"L'ensemble des contenus présents sur le site StudyReach (textes, graphismes, logo, icônes) est la propriété exclusive de StudyReach SAS et est protégé par les lois françaises et internationales relatives à la propriété intellectuelle."},{t:"Responsabilité",c:"StudyReach SAS s'efforce d'assurer l'exactitude et la mise à jour des informations diffusées sur ce site. Toutefois, elle ne peut garantir l'exactitude, la précision ou l'exhaustivité des informations mises à disposition."},{t:"Contact",c:"Pour toute question relative aux présentes mentions légales, vous pouvez nous contacter à l'adresse : legal@studyreach.io"}]},
  };
