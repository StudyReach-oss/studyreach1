...
function ResearcherDashboard({onLogout,showOnboarding,onOnboardingDone}){
  ...
  const confirmCloseStudy=async()=>{
    ...
    if(!s)return;
    // Remboursement autoritatif déjà calculé à l'ouverture de la modale
    // (refundAmount = (max − validés − déjà_refusés) × budget). On le réutilise
    // tel quel → aperçu et débit identiques, et pas de double remboursement.
    const refund=Math.round((s.refundAmount||0)*100)/100;
    const token=Storage.get("sb_token");
