...
function ResearcherDashboard({onLogout,showOnboarding,onOnboardingDone}){
  ...
  const [ns,setNs]=useState({title:"",theme:"",dur:"",mode:"",link:"",ai:false,linkAi:false,ai_focus:"",ai_response_format:{audio:false,video:false,tts:false},studyType:"",meeting_address:"",meeting_notes:"",maxParticipants:null,description:"",prescreening:[],
  target_criteria:{
        ...
        <Modal onClose={()=>setShowStudyModal(false)} title={`Nouvelle étude — Étape ${nsStep+1}/7`} wide>
          ...
          {nsStep===2&&(
            <div>
              ...
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
                {STUDY_TYPES.map(t=>(
                  <div key={t.id} onClick={()=>setNs({...ns,studyType:t.id})} style={{background:ns.studyType===t.id?t.color+"22":C.bg,border:`1.5px solid ${ns.studyType===t.id?t.color:C.border}`,borderRadius:12,padding:"18px 14px",cursor:"pointer",textAlign:"center",transition:"all 0.15s"}}>
              ...
              {["inperson","inperson_group"].includes(ns.studyType)&&(
                <div style={{marginTop:18}}>
                  <Inp label="Adresse du lieu de rencontre *" placeholder="Ex: 12 rue de Rivoli, 75001 Paris" value={ns.meeting_address||""} onChange={e=>setNs({...ns,meeting_address:e.target.value})}/>
                  ...
                  <textarea value={ns.meeting_notes||""} onChange={e=>setNs({...ns,meeting_notes:e.target.value})} placeholder="Ex: Demander à l'accueil pour Sam, sonner au 2e interphone…" rows={2} style={{width:"100%",background:C.bg,border:`1px solid ${C.border}`,borderRadius:8,padding:"10px 12px",color:C.text,fontSize:13,outline:"none",boxSizing:"border-box",resize:"vertical",fontFamily:FONT}}/>
                </div>
              )}
            </div>
          )}
          {nsStep===3&&(
            <div>
              ...
              {["video","video_group","inperson","inperson_group"].includes(ns.studyType)&&!ns.ai&&!ns.linkAi&&(
          ...
          <div style={{display:"flex",justifyContent:"space-between",marginTop:24,paddingTop:16,borderTop:`1px solid ${C.border}`}}>
            ...
            {nsStep<6?<Btn onClick={()=>{setNsErr("");setNsStep(nsStep+1);}} disabled={(nsStep===0&&(!ns.title||!ns.theme||!ns.maxParticipants||ns.maxParticipants>500))||(nsStep===1&&!ns.dur)||(nsStep===2&&(!ns.studyType||(["inperson","inperson_group"].includes(ns.studyType)&&!(ns.meeting_address||"").trim())))||(nsStep===3&&(!ns.mode||(ns.mode==="link"&&!ns.link)))||(nsStep===4&&ns.mode==="ai"&&!(ns.ai_focus||"").trim())||(nsStep===5&&(ns.prescreening||[]).some(q=>!q.question||(q.acceptedAnswers||[]).length===0))}>Continuer →</Btn>:<Btn onClick={publishStudy}>🚀 Publier l'étude</Btn>}
          </div>
        </Modal>
      )}
...
function ParticipantDashboard({onLogout,showOnboarding,onOnboardingDone}){
  ...
  const availWithScore=studies
    .filter(s=>s.status==="available"&&inStudyZone(s,profile)&&(filterTheme?s.theme.includes(filterTheme):true)&&(filterDur?s.dur===filterDur:true)&&(filterType?s.studyType===filterType:true))
