...
function AiInterviewChat({study,profile,matchScore,onComplete,onClose,initialMessages,participationId}){
  ...
  // Ref to hold the accumulated mic transcript — avoids race with _shouldStop guard
  const micTranscriptRef=React.useRef("");
  ...
  const toggleRecording=()=>{
    ...
    // FIX 3: Stop path — flush ref value into input BEFORE setting _shouldStop
    if(recording){
      // Flush any accumulated transcript into the input field
      const accumulated=micTranscriptRef.current.trim();
      if(accumulated){
    ...
    const startRecognition=()=>{
      ...
      recognition.onresult=(event)=>{
        ...
        for(let i=event.resultIndex;i<event.results.length;i++){
          ...
          if(event.results[i].isFinal)micTranscriptRef.current+=transcript+" ";
          else interim+=transcript;
        }
        setInput((micTranscriptRef.current+interim).trim());
      };
  ...
  const send=()=>{
    ...
    // FIX: Stop mic cleanly — flush the ref transcript BEFORE setting _shouldStop
    let extraText="";
    if(recording){
      extraText=micTranscriptRef.current.trim();
      micTranscriptRef.current="";
    ...
    // Use extraText (from mic ref) as fallback if state hasn't updated yet
    const text=(input.trim()||extraText).trim();
      ...
      {finished?(
        ...
        <div>
          ...
          <div style={{display:"flex",gap:8}}>
            ...
            <Btn onClick={send} disabled={loading||(!input.trim()&&!recording)} style={{alignSelf:"flex-end"}}>Envoyer</Btn>
          </div>
        </div>
      )}
    </Modal>
...
function ResearcherDashboard({onLogout}){
  ...
  // FIX Bug 22: Poll participations every 30s so newly joined participants appear immediately
  useEffect(()=>{
    if(!researcherId)return;
    const pollParticipations=async()=>{
      const token=Storage.get("sb_token");
      if(!token)return;
      try{
        const studiesRes=await fetch(`${SUPA_URL}/rest/v1/studies?researcher_id=eq.${researcherId}&select=id,title,mode,budget,max_participants,ai,link_ai,prescreening,cost_per_participant,duration,status,global_synthesis,created_at,study_type,ai_response_format`,{
          headers:{"apikey":SUPA_KEY,"Authorization":`Bearer ${token}`}
        });
        const studiesData=await studiesRes.json();
        if(!Array.isArray(studiesData)||studiesData.length===0)return;
        const studyIds=studiesData.map(s=>s.id);
        const partRes=await fetch(`${SUPA_URL}/rest/v1/participations?study_id=in.(${studyIds.join(",")})&select=*,profiles(first_name,last_name,paypal_email,email)`,{
          headers:{"apikey":SUPA_KEY,"Authorization":`Bearer ${token}`}
        });
        const partData=await partRes.json();
        if(!Array.isArray(partData))return;
        // Update joined counts
        const counts={};
        partData.forEach(p=>{counts[p.study_id]=(counts[p.study_id]||0)+1;});
        setStudies(prev=>prev.map(s=>({...s,joined:counts[s.id]||s.joined||0})));
        // Load AI interviews for context
        const aiStudyIds=studiesData.filter(s=>s.ai).map(s=>s.id);
        let interviewsMap={};
        if(aiStudyIds.length>0){
          try{
            const intRes=await fetch(`${SUPA_URL}/rest/v1/interviews?study_id=in.(${aiStudyIds.join(",")})&select=*`,{
        ...
        // Merge new participations into transactions
        const mappedStudies=studiesData.map(s=>({
          id:s.id,title:s.title,budget:s.cost_per_participant||s.budget||20,
          maxParticipants:s.max_participants||10,ai:s.ai||false,linkAi:s.link_ai||false,
          prescreening:s.prescreening||[]
        }));
        const partTx=partData.map(p=>{
          const study=mappedStudies.find(s=>s.id===p.study_id);
        ...
        setTransactions(()=>partTx.sort((a,b)=>new Date(b.rawDate)-new Date(a.rawDate)));
      }catch(e){console.error("Poll participations error:",e);}
    };
    pollParticipations();
    const interval=setInterval(pollParticipations,30000);
...
function ParticipantDashboard({onLogout}){
    ...
    const loadStudies=async()=>{
      ...
      try{
        ...
        if(Array.isArray(data)&&data.length>0){
          ...
          // FIX Bug 21: also load the studies the participant has already joined
          // so they appear in "Mes participations" / dashboard counters.
          let joinedStudies=[];
          if(token&&userId&&joinedStudyIds.size>0){
            try{
              const joinedIdsArr=[...joinedStudyIds];
              const jRes=await fetch(`${SUPA_URL}/rest/v1/studies?id=in.(${joinedIdsArr.join(",")})&select=*`,{
                headers:{"apikey":SUPA_KEY,"Authorization":`Bearer ${token}`}
              });
              const jData=await jRes.json();
              if(Array.isArray(jData)){
                joinedStudies=jData.map(s=>{
                  const part=participationMap[s.id]||{};
                  return {
                    id:s.id,title:s.title,theme:s.theme||"",dur:s.duration||"",mode:s.mode||"Lien",
                    link:s.link||"",researcher:"Chercheur",company:"",
                    deadline:"30 juin 2026",pay:s.cost_per_participant?Math.round(s.cost_per_participant*0.9):27,
                    desc:s.description||"Étude qualitative",description:s.description||"",ai_focus:s.ai_focus||"",
                    // Use the real participation status
            ...
            }catch(e){console.error("Load joined studies error:",e);}
          }

          setStudies([...mapped,...joinedStudies]);
  ...
  const completeAiInterview=async(study,transcript,report,videoUrls)=>{
    // Guard against double execution (e.g. interview disconnected and resumed)
    ...
    try{
      ...
      // ✉️ Recap email to participant after completing the interview
